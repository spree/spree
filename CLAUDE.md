...
- `6.0-solid-queue-single-container.md` — Solid Queue (+ Solid Cache/Cable) replaces Sidekiq+Redis as the spree-starter default: one process (Solid Queue supervisor in-Puma), one container, Postgres-only. Single-DB default (atomic enqueue-with-transaction); scale-out = `SOLID_QUEUE_IN_PUMA=false` + `bin/jobs` worker from the same image, queue-DB split documented. Mission Control at `/jobs` replaces the Sidekiq UI; recurring jobs standardize on `config/recurring.yml`. Core audited adapter-clean (2026-07-18) — starter/CLI/docs change only; keep core free of Sidekiq/Redis-specific APIs.
