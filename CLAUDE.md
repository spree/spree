...
- `6.0-b2b-wholesale-shipping.md` — Wholesale/freight shipping (design finalized 2026-08-30; **phases 1–6 implemented 2026-09-01**, spree/spree#14539 — everything below the dashboard and storefront, plus the API surface; phases 7–9 deposits/dashboard/storefront pending): **`Spree::PackageType`** — store-scoped packaging vocabulary (`box|envelope|carton|pallet|container`, one default per store Channel-style, refusing deletion of the default) replacing the four `default_package_*` store preferences, which are **deleted outright with NO bridge** (recorded exception — they were added the same unreleased cycle; `spree:upgrade:package_types` carries their values into the default row first); deliberately NOT `Spree::Package` — `Stock::Package` is the in-memory packer); variant packing chain `carton_package_type_id` + `units_per_carton` + `carton_weight` + `cartons_per_pallet` (geometry on the shared carton row, packing facts on the variant; Unit → Carton → Pallet → CBM/weight); unit-aware `Spree::FreightSummary` (units/cartons/pallets/CBM/gross weight, `complete?` flag) built on the currently-unused `Stock::Package#volume` chain via a new `Spree::Measurement` conversion helper; shipment tiers = `DeliveryMethodRules::VolumeRule` (CBM min/max) + `CompanyRule` (`company_presence`) — **no new delivery profile kind** (the same product ships parcel retail and carton wholesale, and a product has one profile); **unpriced rates are first-class** (`unpriced` on Estimate + `spree_delivery_rates`, estimator skips markup, UIs render "quoted after review" — never a $0 rate that displays as Free) returned by `DeliveryRateProvider::Freight`, whose rate `metadata` snapshots the summary + deposit terms (frozen at completion — duties doctrine, never re-derived); **deposits are collected, not displayed**: deposit terms → `Cart#amount_due_at_checkout`, `Carts::Complete` validates payments against the deposit (base = order total at placement), order completes `partially_paid` with the existing `outstanding_balance` (2026-08-30 amendment: the method-level `deposit_percentage` is a *default source* feeding the order's `payment_terms` snapshot — see `6.0-6.1-b2b-payment-terms.md`); the forwarder's quote lands later as `Fee(kind: 'freight')` through `/admin/orders/:id/fees`. `Spree::Measurement` is the one conversion table (EasyPost's own copies were deleted); a package's tare and dimensions are read in the STORE's units via `PackageType#weight_in`/`#dimensions_in`, never raw; `Stock::Packer` stamps the cart/order onto every package it builds (splitters and order routing included) — nothing may read the buyer off a package built any other way; `Variant#volume`, `ContentItem#volume` and `Package#volume` all mean **cubic meters** now. **Constraints now:** all volume/CBM math goes through the unit-aware helper (raw dimension-column multiplication is forbidden — the `WeightRule` raw-number shortcut does not extend to volume); the `default_package_*` preferences are GONE — read the store's default `PackageType`; never model quote-later as a zero-cost priced rate; carton geometry never becomes variant columns; nothing may assume a completed order is paid in full (check `payment_status`/`outstanding_balance`); post-placement freight charges are Fees, never delivery-rate edits; freight tracking (PRO/container number) is a hand-entered `Spree::Delivery` row and its delivered state is staff-confirmed, while a bill of lading is never a `ShippingLabel` (cross-referenced 2026-09-03 from `6.0-shipping-labels-and-deliveries.md`).
