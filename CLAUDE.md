...
Shipped plans:
- `6.0-channel-markets.md` — Optional Channel→Markets allowlist (`spree_channel_markets` join, empty = sells into every market; **implemented 2026-09-02, PR #14518**). Enforced in market resolution (`set_market_from_country` reads the lazy memoized `current_channel` — that callback registers five slots before channel resolution, so it writes the channel's own default on rejection rather than falling through to `Spree::Current`, whose channel is still the store default), all three Store API `/store/markets` actions incl. `resolve`, and a `market must be served by its channel` validation in the shared `Purchase::Market` concern (Cart AND Order, shared-example battery). Channel gains nullable `default_market_id` (null = derived: store default when served, else first allowed; read via `resolved_default_market`, never the raw association — a pin can go stale against the allowlist). A narrowed channel's markets also bound its sellable locales/currencies, but only when it narrows them — an unrestricted channel keeps the store's own lists. **Constraints:** `serves_market?` carries its own cross-store check (purchase validation trusts the predicate, so it must not assume a scoped argument); a purchase's channel must belong to its store (`Purchase::Channel`, every write path); ship-to stays gated by delivery zones, never markets — no address-level market validation anywhere.
