...
- `6.0-tax-provider.md` — Per-Market TaxProvider, replaces TaxRate.adjust + Calculator; TaxRate gets direct `country_iso`/`state_abbr` string columns (amended 2026-08-07 — ISO codes, not FKs, per `6.0-drop-country-state-models.md`; tax decoupled from Zone; Zone model dropped in 6.0 via `6.0-delivery-zones.md`); reference provider: Avalara (decisions.md 2026-07-30)
- `6.0-delivery-rate-provider.md` — Per-DeliveryMethod DeliveryRateProvider wrapping Stock::Estimator (calculators stay — 2026-07-27 reversal), `store_id` on ShippingMethod, enriched DeliveryRate (carrier/service/date/metadata); monorepo ships one reference multi-carrier provider (Shippo/EasyPost, pick pending — decisions.md 2026-07-29); method rows stay regional (no cross-market method entity)
- `6.0-delivery-zones.md` — Zone → DeliveryZone with postal-code-range/prefix members (country-scoped); owns the full ~183-reference Zone consumer inventory; `Spree::Zone` dropped entirely by end of 6.0 (2026-07-27 — 6.0 is the breaking-change window). Member FKs convert to `country_iso`/`state_abbr` strings per `6.0-drop-country-state-models.md` (2026-08-07)
- `6.0-drop-country-state-models.md` — Delete `Spree::Country`/`Spree::State` AR models + tables in 6.0; constants survive as `countries`-gem-backed value objects (carmen removed), consumers store `country_iso` + `state_abbr` strings (always paired — subdivision codes aren't globally unique), Address validation semantics preserved (`STATES_REQUIRED` constants become runtime source of truth). Zero v3 API contract change (no `id` was ever exposed) and zero dashboard change. Amends tax-provider Phase 5 (TaxRate columns born as ISO strings, not FKs) and delivery-zone members. **Storage layer shipped (2026-08-07):** `Spree::IsoData` curated registry + drift-guard spec, ISO columns on addresses/delivery-zone-members/market-countries/stock-locations/stores with `spree:upgrade:migrate_country_state_isos` in the 5.6→6.0 manifest, every consumer matching on codes, admin FK params closed, country endpoints uncached, factories built from real ISO data. **Model removal blocked:** legacy `Spree::Zone`'s `has_many source_type: 'Spree::Country'` needs both to stay AR, and tax still runs through it — waits on tax-provider Phase 5 then delivery-zones Phase D (which also gates seed/carmen removal, since zone seeding resolves countries by ISO). **Constraint now:** no new `country_id`/`state_id` FKs or `belongs_to :country/:state` anywhere — new features store ISO strings (`Promotion::Rules::Country` is the template)
...
- `6.1-order-change-substrate.md` — `Spree::OrderChange` + `Spree::OrderChangeAction`: one preview-then-apply substrate behind every post-placement mutation (admin order edits, returns, exchanges, claims, draft-order amendments), replacing four per-domain draft models with one `begin → request → confirm → cancel` lifecycle. Actions are typed `kind` + concrete FKs (never polymorphic); **preview is computed in memory and never persisted**; confirm is a workflow with the balance settlement as an `external_step`. Deliberately 6.1 — new schema + new extension API, and 6.0 already carries the Cart/Order split, typed adjustments and the returns rework. 6.0 constraint: don't build a per-domain persisted preview/draft model, and expose post-placement money math through a service returning a value object so it can be re-pointed later.
