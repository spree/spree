...
- `6.0-analytics-semantic-layer.md` — Semantic metric/dimension registry (`Spree.reporting` — `Spree.analytics` is taken by storefront event tracking) + Cube-style query contract compiled at `POST /api/v3/admin/reporting/query`, with compiler-enforced store/currency/vendor forced scopes and pluggable storage adapters (live OLTP default — no new infra; incremental fact tables opt-in in 6.1; ClickHouse = Enterprise adapter). Implements on the current dashboard-home branch: registry + compiler + live adapter + the new home screen querying the contract directly (its hand-rolled `dashboard/analytics|rankings` endpoints get deleted, `dashboard/operations` stays), next: permission-filtered schema enrichment (labels, compatible metrics, filter values), time presets + week grain, market/country/category filters-as-dimensions, then saved reports + a picker-builder Reports page + CSV export, then separate agent tools (schema/query/explain/saved reports) CLI-first; AI layer is NL → query JSON via the registry-derived tool schema, never NL → SQL; seller forced scope now implementable since `Spree::Seller` landed. Replaces `Spree::Report` *authoring* for aggregates (CSV row-level export stays); **no new hand-rolled aggregate endpoints** — see the plan's "Constraints on Current Work".
