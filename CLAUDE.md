...
- `6.0-split-adjustments.md` — Replace polymorphic Adjustment with TaxLine, Discount, Fee. **Shipped to 6-0-dev (Waves 3+6):** typed rows + winner-only promotion adjuster + `Spree.tax_provider` live; admin `/orders/:id/{tax_lines,discounts,fees}` API + dashboard panels + `OrderUpdater#resum_typed_totals!` post-placement edit path; Wave 7 adjustments→typed data migration remains
