...
- `6.0-channel-markets.md` — Optional Channel→Markets allowlist (`spree_channel_markets` join, empty = all markets; **design finalized 2026-08-28**, ready to implement). Enforced in market resolution (`set_market_from_country` reads the lazy memoized `current_channel` — the market callback registers five slots before channel resolution, verified empirically), all three Store API `/store/markets` actions incl. `resolve`, and a `market must be served by its channel` validation in the shared `Purchase::Market` concern (Cart AND Order, shared-example battery). Channel also gains nullable `default_market_id` (null = derived: store default when served, else first allowed; read via `resolved_default_market`, never the raw association — it can go stale against the allowlist). Ship-to stays gated by delivery zones, never markets — no address-level market validation anywhere. Composes with `MarketRule` from the payment-method-rules plan; the reader trio copies shipped `serves_location?`.
