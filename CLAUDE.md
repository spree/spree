...
- `6.0-fulfillment-and-delivery.md` — Shipment→Fulfillment, ShippingMethod→DeliveryMethod, drop ShippingCategory, FulfillmentProvider strategy, pickup (merchant StockLocation) + pickup_point (third-party PickupPointProvider). **Shipped to 6-0-dev (Waves 2+4+6):** renames + deprecated twins, providers (Manual/Digital/Pickup/PickupPoint), dual-emit events, store pickup discovery endpoints, admin delivery_methods/delivery_zones CRUD + dashboard settings pages, FulfillmentMailer; Wave 7 shipping→delivery data migration remains. Pickup work beyond shipped code deferred to 6.1 (2026-08-06): contract hardening (`new(delivery_method)` ctor, `find_nearby` zipcode/query) + how-to guide; `PickupPointProvider::Base` stays **undocumented in 6.0** (v6 docs cover only the delivery rate provider interface) — never add pickup-provider docs in 6.0 work. Delivery mechanics live on provider classes (`pickup_point` provider ships but stays unregistered until 6.1; `local_delivery` cut = shipping + postal-code zone); the string fulfillment-type registry was deleted by `6.0-delivery-profiles.md`. **Partial fulfillment (2026-08-10, resolved question 12):** shipping a subset is `Spree::Fulfillments::Fulfill` (optional per-line-item `items:` + `tracking:` + `notify_customer:`; splits then fulfills in one transaction) — never a state-machine event, since split-then-ship in an `after_transition` is what service-workflows prohibits. **Side effects left the machine (2026-08-10, question 13):** `after_cancel`/`after_resume` are gone as transition callbacks — restock + carrier stand-down live in `Fulfillments::Cancel`/`::Resume` as inline steps (provider call is an `external_step`; `notify_provider: false` lets `Orders::Cancel` delegate and batch carrier I/O after its own transaction), `after_cancel`/`after_resume` are deprecated shells and the only trace left on the model — never add public model methods for workflows to call. **Status model (2026-08-11, question 14): the machine is REMOVED.** `status` = plain `HasStatus` `unfulfilled → fulfilled → delivered | canceled` (`pending`/`ready`/`ready_for_pickup` collapse — payment/stock gating is a `Fulfillments::Fulfill` validate guard, pickup renders modality-aware labels); `delivered` = confirmed receipt (`Fulfillments::MarkDelivered`, `delivered_at`, `fulfillment.delivered`) — the returns window + EU withdrawal anchor; carrier truth is the separate `tracking_status` axis (data, never a machine) written by `Fulfillments::UpdateTracking`, fed by EasyPost tracker webhooks (opt-in trackers for hand-entered numbers). Rollup domain `backorder | canceled | partial | unfulfilled | fulfilled | delivered`. **Phase 7 shipped 2026-08-11** (core + API + dashboard + EasyPost tracker webhook + `spree:migrate_fulfillment_statuses`); **label leads, fulfilled follows (2026-08-12):** `Fulfillments::PurchaseLabel` = explicit pre-ship label buy (loud failure, no email); `Fulfill` buys label BEFORE marking fulfilled (email always carries tracking; post-split failure keeps the split, never rolls it back); providers declare `generates_labels?` + idempotent `create_fulfillment`. **One tracking number per fulfillment (2026-08-11, question 15) — superseded 2026-09-03 by `6.0-shipping-labels-and-deliveries.md`:** the carrier axis moves whole onto `Spree::Delivery` rows in 6.0
- `6.0-shipping-labels-and-deliveries.md` — Labels and parcels become records (design finalized 2026-09-03, nothing built): **`Spree::ShippingLabel`** (prefix `lbl`, store-scoped, polymorphic `owner` = `Fulfillment` | `Return` per the 2026-08-22 tax-identifier ruling; `source` `purchased` | `uploaded`, `integration_id`, `external_id`, `carrier`/`service`, frozen `tracking_number`, `cost` + `currency` = what the merchant paid — admin-only, never shopper money, `format`, `has_status :purchased, :refund_requested, :refunded`, private-storage `file` fetched from the provider right after purchase and streamed by the API) and **`Spree::Delivery`** (prefix `dlv`, same polymorphic owner, `shipping_label_id` unique-nullable, `tracking_number` unique per owner, `tracking_url`, `carrier`, `status` = the carrier axis with `pending` replacing today's nil, `estimated_delivery_at`/`delivered_at`, `details`). The delivery **absorbs the entire carrier axis**: webhooks match a delivery scoped to the integration's store, `Deliveries::UpdateTracking` (renamed from `Fulfillments::`) writes one row and rolls the fulfillment to `delivered` when every sibling is; the fulfillment keeps `status`, `fulfilled_at`, `delivered_at` as lifecycle stamps and **only `tracking` + `tracking_url`** (5.6 API) as a permanent primary-delivery summary — `tracking_carrier`/`tracking_status`/`tracking_details`/`estimated_delivery_at` come out of their 20260811 migrations in place (unreleased). Workflows: `ShippingLabels::Purchase`/`Refund`/`Record`, thin `Fulfillments::PurchaseLabel` + `Returns::PurchaseLabel`; core owns idempotency (refuses on an active label — providers drop their `metadata` keys); provider contract `purchase_label(owner)` → `Spree::LabelPurchase` value object + `refund_label(label)`, `documents` no longer reports the label; EasyPost buys `is_return` shipments for returns. Admin API: nested `labels` (`POST` = buy, or record an uploaded label when `file` is given; `download` streamed; `refund`) and `deliveries` (CRUD + `mark_delivered`) under `/orders/:id/fulfillments/:id/` and `labels` under `/orders/:id/returns/:id/`; `purchase_label` stays one release as an alias; sellers upload but never purchase; Store API gains `fulfillment.deliveries[]` + a customer return-label download. **Constraints now:** never write `spree_fulfillments.tracking*`/`estimated_delivery_at`/`tracking_details` in new code (tracking is a `Delivery` via `Deliveries::Create` or the `tracking:` shortcut on `Fulfill`/`Update`); no provider reads or writes `fulfillment.metadata` for label state; a label's `cost` never feeds `fulfillment.cost` or a store serializer; webhook matching resolves a `Delivery`, never `Fulfillment.where(tracking:)`; return labels attach to the `Return` as owner — no `return_label_url`, no URL in `metadata`; a `Return` delivery never auto-receives; `Fulfillment#delivered` is a rollup no integration sets directly; `Delivery#carrier` is free text — never validated against `Spree.tracking_carriers` (a forwarder's PRO number must be enterable); freight paperwork (bill of lading, placards, packing lists) is never a `ShippingLabel` — it is `OrderDocument`/client prints (`6.1-b2b-order-documents.md`). **Name settled against the wholesale plan (2026-09-03):** `Parcel` was chosen and withdrawn because `6.0-b2b-wholesale-shipping.md` uses "parcel" as the antonym of freight; `Delivery` is the modality-neutral word (a parcel or three pallets under one PRO), and the two-meanings cost beside `DeliveryMethod`/`DeliveryRate` is accepted.
...
- `6.0-avalara-provider-gem.md` — First-party Avalara AvaTax provider gem (design finalized 2026-08-19; = tax-provider plan's Phase 6): **new monorepo gem `spree_avalara`/`SpreeAvalara` at `spree/providers/avalara`** (supersedes the `spree_tax_avalara` working name — api_type derives to `'avalara'`, matching `provider_id`), easypost-patterned. Estimate prices the **whole owner** per call (ignores the `items` subset, sweeps + rewrites only `provider_id: 'avalara'` rows, 5-min response cache absorbs single-item call sites) and **fails closed**; `taxability_reason` folds Avalara's structured reason fields (`nonTaxableType`/`exemptCertId`/`rateTypeCode`; only the `intra_community_supply` vs `reverse_charge` split derives from request-side facts). Idempotent commit via `create_or_adjust_transaction` (doc id in `order.metadata['avalara_transaction_id']` — no ledger table); refund = line-level ReturnInvoice scaled to the `amount:` actually refunded; standalone (no-return) refunds deliberately not filed in v1. Exemptions: the gem consumes core's `exemptions:` seam as-is (company certificates; **no Dependencies swap, no gem storage** — customer-level/no-Company exemptions are an open question with a prefer-extending-core suggestion; legacy `spree_users.vat_id` migrates to core `TaxIdentifier`, the legacy exemption columns survive as that question's migration source); address validation enforced at `carts.complete.validate` (fail-open on transport), never `Spree.validators.addresses`. Reads `Market#tax_inclusive` **destination-derived** — resolved through the market covering the tax address's country, falling back to the owner's market (the browsing market can diverge from the tax destination: currency-driven re-resolution, unvalidated bill addresses — the 5.x bug of spree_avatax_official#198); settles Phase 9's "wire or drop" — it stays. `spree_avalara:upgrade` retypes Integration rows, deletes synthesized 'AvaTax Official Tax Rate' rows, and points **only blank** `Market#tax_provider` at Avalara (never rewrites a set value). Avalara VCR cassettes are recorded only by the maintainer; specs are layered so everything else runs cassette-free.
...
- `5.5-6.0-order-cancellation-and-approval.md` — **Superseded 2026-09-02.** The 5.5 `OrderCancellation` + `OrderApproval` tables were write-only (nothing ever read a row) and are dropped in 6.0; 6.0 keeps `canceled_at`/`approved_at` on the order and adds a fourth merchant-owned reason vocabulary, `Spree::OrderCancellationReason` (`NamedType`, store-scoped, seeded, on the Settings → Reasons page), bound via `spree_orders.cancel_reason_id` alongside a `cancel_note` (stamped by `Orders::Cancel`, accepted as `cancel_reason_id` on the admin cancel endpoint; optional, like `Return#reason`). **Constraints now:** no cancellation/approval history tables; a new reason vocabulary is its own `NamedType` table, never a `kind` on a shared one; company approval flows are Enterprise.
...
| `spree/providers/avalara` | Ruby gem (`spree_avalara`, optional) — first-party Avalara AvaTax `TaxProvider`: credentials via `SpreeAvalara::Integration`, every AvaTax call through `SpreeAvalara::Client`. Being built out phase by phase — see `docs/plans/6.0-avalara-provider-gem.md`. |
| `spree/providers/easypost` | Ruby gem (`spree_easypost`, optional) — reference `DeliveryRateProvider` + `FulfillmentProvider`: live EasyPost rates, label purchase/refund, credentials via `SpreeEasyPost::Integration`. Provider gems live under `spree/providers/` (stripe/adyen land there too); gem names stay flat. |
