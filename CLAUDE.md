...
- `5.4-6.0-eu-legal-compliance.md` — GDPR, Omnibus, Consumer Rights. **Fully implemented 2026-09-02** (spree/spree#14560): `Spree::DataRequest` (async access/erasure requests, token-addressed download, pending request IS the rate limit), `Spree::Customers::Anonymize` (the ONLY erasure path — `scramble_email_and_names` is a deprecated shell; ledger survives, jurisdiction survives on order addresses, a schema tripwire spec fails when a PII column appears it does not reach), `Spree::ConsentRecord` (acceptance as events, polymorphic owner because guest checkout has no account, policy digest + locale snapshot), consent timestamp + source on customer, per-market `withdrawal_period_days` (separate from `return_window_days`, anchored on the LAST delivery). Store+admin endpoints both ship OSS; Enterprise keeps the request queue, Art. 12(3) deadline clock, audit trail and automated retention. **Constraints now:** a new table carrying customer-identifiable data extends the anonymizer in the same change; never record consent that was not given (terms only when the caller passes `terms_of_service`); the export download link is a credential and never enters an event payload or the delivery log; the withdrawal deadline is derived, never stored.
