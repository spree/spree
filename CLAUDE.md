...
- `6.0-analytics-semantic-layer.md` — Semantic metric/dimension registry (`Spree.analytics`) + Cube-style query contract compiled at `POST /api/v3/admin/analytics/query`, with compiler-enforced store/currency/vendor forced scopes and pluggable storage adapters (live OLTP default — no new infra; incremental fact tables opt-in in 6.1; ClickHouse = Enterprise adapter). Saved report = query JSON + viz config row; dashboard home + vendor dashboards become seeded saved reports; AI layer (6.1) is NL → query JSON via registry-derived tool schema, never NL → SQL. Replaces `Spree::Report` *authoring* for aggregates (CSV row-level export stays); **no new hand-rolled aggregate endpoints** — see the plan's "Constraints on Current Work".
