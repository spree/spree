...
- `6.0-digital-assets.md` — `Spree::Digital` → `Spree::DigitalAsset` (full table + FK rename with one-release bridges; `DigitalLink` keeps its name; design finalized 2026-08-10) plus the missing management/delivery surface: Admin API v3 (assets nested under products, link `reset`, resend; catalog: assets ride `products`, link management rides `orders`), dashboard product card + order links + Downloads settings, dedicated `Spree::DigitalAssetMailer` (`files_ready_email` / opt-in `file_updated_email`), customer `GET /store/account/digital_links` + store SDK resource, `digital_link.downloaded` domain event (no log table), signed-URL download transport (dead `digital_asset_link_expire_time` pref becomes the TTL), phantom `:digital` product param removed. Per-asset nullable `authorized_clicks`/`authorized_days` fall back to store settings — a recorded exception to the store-scoped-configuration no-fallback rule. **Constraints now:** new code uses `DigitalAsset`/`digital_assets` names only; nothing outside the model assumes an attachment is present (asset kinds + license-key pools groomed for 6.1); emailed download URLs keep resolving across renames one release.
