...
- `6.0-single-store-promotions-payment-methods.md` — Migrate `Spree::Promotion` + `Spree::PaymentMethod` from multi-store (`has_many :stores` via `spree_promotions_stores` / `spree_payment_methods_stores` join tables) to single-owner `belongs_to :store`, mirroring the 5.5 single-owner Product migration. Adds nullable `store_id` FK + backfill rake task, `LegacyMultiStoreSupport` deprecation bridges, deletes the `ResourceController` `store_ids=` seam; multi-store sharing moves to the `spree_multi_store` extension (join tables left intact). Paired with `6.0-channels-catalogs-b2b.md`.
