...
- `6.0-inventory-operations.md` — StockTransfer lifecycle (draft → ready_to_ship → in_transit → received with partial receive), new `Spree::PurchaseOrder` + `Spree::Supplier` (the procurement source; `Spree::Seller` is the marketplace seller — see decisions.md 2026-07-14 and 2026-08-17) replacing today's "external receive" hack, variant + stock-location stock history panels. Consumes the typed-movement primitives from `6.0-typed-stock-movements.md`.
...
- `6.0-maintenance-tasks.md` — Operator-run data work as a first-class primitive (design finalized 2026-08-15): `Spree::MaintenanceTask` (Shopify `maintenance_tasks` authoring contract — `collection`/`process`/`count`, ActiveModel attributes, `throttle_on`/`report_on`, opt-in `supports_dry_run` + `dry_run?`) run by a native `ActiveJob::Continuable` job (NO dependency on the gem / `job-iteration`), `Spree::MaintenanceTaskRun` row = cursor + progress + tallies + who/where/when (the audit; no destroy, lifecycle events fire), explicit registry `Spree.maintenance_tasks` (config array of class names — never subclass autodiscovery), one catalog resource `maintenance_tasks` in the `access` group (admins by default; nothing per task, nothing gated on the `admin` role by name), runs installation-wide with a context `store_id` loaded into `Spree::Current.store`, Admin API `/maintenance_tasks` + `/maintenance_task_runs` (pause/resume/cancel PATCH members), dashboard `/settings/maintenance-tasks` (audit group, polling), rake `spree:maintenance_tasks:perform` writes run rows too. **All 22 steps of the 5.6→6.0 upgrade manifest are rewritten as `Spree::MaintenanceTasks::Upgrade::*` classes and the manifest names `task_class:`** (older manifests keep rake steps; walker runs both inside run rows; `Upgrade::Walk` orchestrates with child runs via `parent_run_id`). Schema migrations never. **Shipped 2026-08-15 (all six phases):** core primitive + Admin API/SDK + dashboard page + all 22 upgrade steps as `Upgrade::*` classes named by `task_class:` in the manifest (released manifests keep `task:` rake steps, run through `UpgradeStep`; walker runs both kinds through `Start` so a shell walk is audited too) + `csv_collection` (uploaded blob attached to the run, row-number cursor) + `docs/v6/developer/how-to/maintenance-tasks.mdx`. **Constraints now:** new bulk/backfill work is a task (a rake entry may wrap `Start.call(inline: true)`), new 6.0 upgrade steps are born as task classes, system-triggered jobs (imports, product-type apply) stay plain Continuable jobs; NEVER load all engine `.rake` files to reach one task — re-loading a `.rake` file appends to existing tasks so they run twice (`load_engine_rake_tasks` greps for the defining file); task tallies must accumulate across executions, never replace.
...
Shipped plans:
- `5.5-6.0-display-on-to-boolean.md` — Collapse `display_on` tri-state to a single `storefront_visible` boolean on DeliveryMethod, CustomFieldDefinition and PaymentMethod. Real column on all three, `Spree::DisplayOn` concern deleted; `display_on` survives only as a deprecated accessor until 6.1. **Constraints:** never filter admin listings by `storefront_visible` (the backoffice sees every method); a new model needing storefront/admin visibility adds its own column plus `storefront_visible`/`admin_only` scopes rather than a shared concern.
