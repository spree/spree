...
- `6.0-b2b-storefront-purchasing.md` — Storefront company self-service (account section: members, invitations, address book, company orders; invitation acceptance page the mailer already links) + company-aware checkout (buying-for picker, company address book at the address step, ownership guard widened to company-or-ancestor book ids). **No polymorphic owner on Cart/Order** — a purchase keeps two buyer-side parties (`customer_id` + `company_id`), the industry dual axis; OSS ships plain machinery, the polished buyer portal is Enterprise.
