namespace :spree do
  namespace :upgrade do
    desc <<~DESC
      ...
      then re-points every polymorphic +user_type+ that named the old customer
      class (default +Spree::User+, override with SOURCE_USER_TYPE) to
      +Spree.customer_class+, and backfills +password_digest+ on
      ...
      detected. The legacy source table is left in place as a safety net.
    ...
    task migrate_users_to_customers: :environment do
      ...
      # than silently produce accounts nobody can sign into.
      if defined?(Devise) && Devise.respond_to?(:pepper) && Devise.pepper.present?
        ...
        puts "  Devise not loaded — assuming plain bcrypt digests (no pepper). If the source used a pepper, stop now."
      end
      ...
      copied = 0
      skipped_blank_email = 0

      source.where.not(id: customer_model.select(:id)).in_batches(of: batch_size) do |relation|
        rows = []
      ...
      puts
      puts "  Customers copied: #{copied} (skipped #{skipped_blank_email} blank-email row(s))."
      ...
      puts "  Re-pointed #{old_type} -> #{new_type}: " \
           ...
           "user_identities #{repointed[:user_identities]}, " \
