namespace :spree do
  namespace :taxons do
    ...
    task backfill_store_id: :environment do |_t, _args|
      ...
      # Mirror Spree::Category#ensure_store: taxonomy-less rows inherit their parent's
      ...
      loop do
        ...
        frontier = Spree::Category.unscoped.where.not(store_id: nil).
                   where(id: Spree::Category.unscoped.where(store_id: nil).select(:parent_id))
        frontier.pluck(:id, :store_id).group_by(&:last).each do |store_id, rows|
          rows.map(&:first).each_slice(1000) do |parent_ids|
            updated += Spree::Category.unscoped.where(store_id: nil, parent_id: parent_ids).
    ...
    backfill_products_count = lambda do
      Spree::Category.unscoped.in_batches(of: 1000) do |batch|
        Spree::Category.recalculate_products_count(batch.pluck(:id))
        print '.'
      end
    end
    ...
    task reset_counter_caches: :environment do |_t, _args|
      ...
      Spree::Category.find_each do |taxon|
