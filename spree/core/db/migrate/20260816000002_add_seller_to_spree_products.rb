class AddSellerToSpreeProducts < ActiveRecord::Migration[8.1]
  ...
  #
  # Replaces the guarded Spree::SellerConcern hook the Enterprise module used
  # to fill: the marketplace is core now, so the column is too.
  def change
    add_reference :spree_products, :seller, index: true
  end
end
