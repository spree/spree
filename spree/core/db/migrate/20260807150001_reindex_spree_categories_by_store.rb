class ReindexSpreeCategoriesByStore < ActiveRecord::Migration[8.1]
  ...
  # Categories are store-owned in 6.0, so uniqueness is scoped to the store
  # rather than the taxonomy. The old taxonomy-scoped indexes stop constraining
  # anything once spree:migrate_taxons_to_categories_and_collections nulls
  # taxonomy_id, because SQL treats NULLs as distinct in unique indexes.
  #
  ...
  #
  # Runs at db:migrate time, before the data task — the normal order. Nothing here
  # can conflict with pre-task data: rows still awaiting the task have store_id
  # IS NULL, which keeps them out of this index until the task backfills them.
  def up
