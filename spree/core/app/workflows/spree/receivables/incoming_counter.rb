module Spree
  module Receivables
    ...
    # toward incoming exactly while it is open and moving — `ordered`,
    ...
    module IncomingCounter
      ...
      # The destination level is created when the warehouse has never held
      # the SKU: a merchant who has ordered twenty of something new should see
      # them on their way.
      def move_incoming(receivable, sign)
        destination = receivable.destination_location

        receivable.items.each do |item|
          units = item.incoming
          next if units.zero?

          destination.stock_level_or_create(item.variant).adjust_incoming_count(sign * units)
        end
      end
    end
  end
