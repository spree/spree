module Spree
  module TaxIdValidator
    # Contract for checking a buyer's tax registration. Registered per kind in
    # {Spree.tax_id_validators}, because whether a number is well-formed and
    # whether it is really registered are questions for the jurisdiction that
    # issued it — not for the market's tax engine.
    #
    # Two halves, deliberately split by cost: the format check is synchronous,
    # offline and cheap enough to run on every save; the registry check makes a
    # network call and runs in a job.
    class Base
      ...
      #
      # A registry that cannot answer must return an +unavailable+ result rather
      # than raise: the number stays usable and the platform records that nobody
      # could answer.
      #
      # @param tax_identifier [Spree::TaxIdentifier] carries the kind, the value
      #   and its owner — reach the store through the owner when the registry
      #   needs credentials
      # @return [Spree::TaxIdentifier::ValidationResult]
