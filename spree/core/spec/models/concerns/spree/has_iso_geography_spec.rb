...
describe Spree::HasIsoGeography, type: :model do
  describe 'country_iso validation' do
    ...
    it 'guards country-only hosts too' do
      market_country = Spree::MarketCountry.new(market: create(:market), country_iso: 'ZZ')

      expect(market_country).not_to be_valid
      expect(market_country.errors[:country_iso]).to be_present
    end
  end

  describe 'state_code validation' do
    it 'accepts a subdivision of the country' do
      expect(build(:stock_location, country_iso: 'US', state_code: 'NY')).to be_valid
    end

    it 'accepts a retired code that resolves through its successor' do
      stock_location = build(:stock_location, country: nil, state: nil, country_iso: 'IN', state_code: 'OR')

      expect(stock_location).to be_valid
    end

    it 'rejects a code that is no subdivision of the country' do
      stock_location = build(:stock_location, country: nil, state: nil, country_iso: 'US', state_code: 'ZZ')
    ...
    it 'reports only the country when both codes are unrecognisable' do
      stock_location = build(:stock_location, country: nil, state: nil, country_iso: 'ZZ', state_code: 'QQ')
  ...
  describe 'historical rows' do
    ...
    it 'stays updatable when a stored code no longer resolves' do
      ...
      stock_location.update_columns(country_iso: 'ZZ', state_code: 'QQ')
    ...
    it 'validates again once the code itself changes' do
      stock_location = create(:stock_location)
      stock_location.update_columns(country_iso: 'ZZ')

      stock_location.reload
      expect(stock_location.update(country_iso: 'YY')).to be(false)
      expect(stock_location.errors[:country_iso]).to be_present
    end
  end
end
