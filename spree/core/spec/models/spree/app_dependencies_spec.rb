...
describe Spree::Core::Dependencies, type: :model do
  ...
  describe 'backwards compatibility' do
    it 'returns the default value as string' do
      expect(deps.cart_add_item_service).to eq('Spree::Cart::AddItem')
    end

    it 'allows to overwrite the value with a class' do
      deps.cart_add_item_service = MyCustomCreateService
      expect(deps.cart_add_item_service).to eq MyCustomCreateService
    end
    ...
    it 'works with constantize for string values' do
      expect(deps.cart_add_item_service.constantize).to eq Spree::Cart::AddItem
    end
  end

  describe '#<dependency>_class' do
    it 'returns the constantized class for string values' do
      expect(deps.cart_add_item_service_class).to eq Spree::Cart::AddItem
    end

    it 'returns the class directly when set as class' do
      deps.cart_add_item_service = MyCustomCreateService
      expect(deps.cart_add_item_service_class).to eq MyCustomCreateService
    end

    it 'memoizes the resolved class' do
      deps.cart_add_item_service_class
      expect(deps.instance_variable_get(:@cart_add_item_service_resolved)).to eq Spree::Cart::AddItem
    end

    it 'clears memoization when value changes' do
      deps.cart_add_item_service_class
      deps.cart_add_item_service = MyCustomCreateService
      expect(deps.instance_variable_defined?(:@cart_add_item_service_resolved)).to be false
    end
  end
  ...
  describe '#overrides' do
    ...
    it 'tracks overridden dependencies' do
      deps.cart_add_item_service = MyCustomCreateService
      expect(deps.overrides).to have_key(:cart_add_item_service)
    end

    it 'includes override metadata' do
      deps.cart_add_item_service = MyCustomCreateService
      override = deps.overrides[:cart_add_item_service]
  ...
  describe '#overridden?' do
    it 'returns false for non-overridden dependencies' do
      expect(deps.overridden?(:cart_add_item_service)).to be false
    end

    it 'returns true for overridden dependencies' do
      deps.cart_add_item_service = MyCustomCreateService
      expect(deps.overridden?(:cart_add_item_service)).to be true
    end
  ...
  describe '#override_info' do
    it 'returns nil for non-overridden dependencies' do
      expect(deps.override_info(:cart_add_item_service)).to be_nil
    end

    it 'returns override info for overridden dependencies' do
      deps.cart_add_item_service = MyCustomCreateService
      info = deps.override_info(:cart_add_item_service)
  ...
  describe '#current_values' do
    ...
    it 'marks non-overridden dependencies correctly' do
      cart_add_item = deps.current_values.find { |v| v[:name] == :cart_add_item_service }
      expect(cart_add_item[:overridden]).to be false
      expect(cart_add_item[:current]).to eq cart_add_item[:default]
    end
    ...
    it 'marks overridden dependencies correctly' do
      ...
      expect(cart_add_item[:overridden]).to be true
      expect(cart_add_item[:current]).to eq MyCustomCreateService
      expect(cart_add_item[:default]).to eq 'Spree::Cart::AddItem'
    end
  end

  describe '#validate!' do
    it 'raises Spree::DependencyError for invalid dependencies' do
      deps.cart_add_item_service = 'NonExistentClass'
      expect { deps.validate! }.to raise_error(Spree::DependencyError)
    end

    it 'includes dependency names in error message' do
      deps.cart_add_item_service = 'NonExistentClass'
      deps.cart_update_service = 'AnotherNonExistentClass'
      expect { deps.validate! }.to raise_error(Spree::DependencyError, /cart_add_item_service/)
    end
  end
end
...
describe 'Spree module dependency accessors' do
  ...
  describe 'Spree.<dependency>' do
    it 'returns the resolved class' do
      expect(Spree.cart_add_item_service).to eq Spree::Cart::AddItem
    end

    it 'responds to dependency methods' do
      expect(Spree.respond_to?(:cart_add_item_service)).to be true
      expect(Spree.respond_to?(:cart_add_item_service=)).to be true
    end
