  ...
  describe '#add' do
    ...
    context 'running promotions' do
      ...
      shared_context 'discount changes order total' do
        ...
        it { expect(subject.order.total).not_to eq variant.price_in(subject.order.currency).amount }
      end
      ...
      xcontext 'VAT for variant with percent promotion' do
  ...
  context 'update cart' do
    ...
    context 'submits item quantity 0' do
      ...
      it 'doesnt try to update unexistent items' do
        ...
        expect(subject.order).to receive(:update).with(filtered_params)
        subject.update_cart params
      end

      it 'does not filter if there is only one line item' do
        single_line_item_params = { line_items_attributes: { id: shirt.id, quantity: 0 } }
        expect(subject.order).to receive(:update).with(single_line_item_params)
