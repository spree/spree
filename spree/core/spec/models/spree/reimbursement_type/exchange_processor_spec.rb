...
module Spree
  describe ReimbursementType::ExchangeProcessor, type: :model do
    ...
    let(:exchange) { ReimbursementType::ExchangeProcessor.new(order, return_items) }
    ...
    describe '#perform!' do
      ...
      context 'when it cannot create shipments for all items' do
        before do
          StockItem.where(variant_id: return_item.exchange_variant_id).delete_all
        end

        it 'raises an UnableToCreateShipments error' do
          expect do
            subject
          end.to raise_error(Spree::ReimbursementType::ExchangeProcessor::UnableToCreateShipments)
        end
      end
    end

    describe '#to_key' do # for dom_id
      it { expect(ReimbursementType::ExchangeProcessor.new(nil, nil).to_key).to be_nil }
    end

    describe '.param_key' do # for dom_id
      it { expect(ReimbursementType::ExchangeProcessor.param_key).to eq 'spree_exchange' }
    end

    describe '.model_name' do # for dom_id
      it { expect(ReimbursementType::ExchangeProcessor.model_name).to eq Spree::ReimbursementType::ExchangeProcessor }
    end
  end
end
