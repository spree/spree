...
describe Spree::Order, type: :model do
  ...
  describe 'Scopes' do
    ...
    describe '.search' do
      ...
      it 'returns orders based on an email' do
        ...
        expect(described_class.search('jane.gone@example.com')).to eq([order_2])
        expect(described_class.search('johndoe@example.com')).to eq([order_4])
        expect(described_class.search('john_doe@example.com')).to eq([order_5])
        expect(described_class.search('mary.moe@')).to eq([])
      end

      it 'returns orders based on the first name' do
        expect(described_class.search('don')).to eq([order_1])
        expect(described_class.search('jan')).to eq([order_2])
        expect(described_class.search('greg')).to eq([])
      end

      it 'returns orders based on the last name' do
        expect(described_class.search('ro')).to eq([order_1])
        expect(described_class.search('moe')).to eq([order_3])
        expect(described_class.search('smith')).to eq([])
      end
  ...
  describe '#order_routing_strategy' do
    ...
    it 'lets a channel preference override the store preference' do
      ...
      Spree.order_routing.strategies << ChannelStrategy
      isolated_store = create(:store)
    ...
    it 'skips an unregistered channel preference and uses the store preference' do
      ...
      Spree.order_routing.strategies << StorePreferredStrategy
      isolated_store = create(:store)
  ...
  describe '#valid_promotions' do
    def create_discount(label, line_item, amount, action)
      create(:discount,
             order: order,
             line_item: line_item,
    ...
    let(:zero_promo) { create :promotion_with_order_adjustment, weighted_order_adjustment_amount: 0, starts_at: Time.now, code: 'Zero' }
    let(:order_promo) { create :promotion_with_order_adjustment, weighted_order_adjustment_amount: 10, starts_at: Time.now, code: 'Order1' }
    let(:line_item_promo) { create :promotion_with_item_adjustment, adjustment_rate: 10, starts_at: Time.now, code: 'LineItem' }
    ...
    context 'with promotions' do
      let!(:zero_discount) { create_discount('Zero discount', order.line_items.second, 0, zero_promo.actions.first) }
      let!(:order_discount) { create_discount('Discount', line_item, -50, order_promo.actions.first) }
      let!(:line_item_discount) { create_discount('Discount', order.line_items.third, -200, line_item_promo.actions.first) }

      before do
        [zero_promo, order_promo, line_item_promo].each do |promotion|
  ...
  describe '#cart_promo_total' do
    ...
    context 'with promotions' do
      ...
      context 'multiple promotions' do
        ...
        # The line-item promotion fully discounts every line, so the
        # order-level promotion has no remaining base to distribute over —
        # discounts are clamped so no line goes below zero (6.0 behavior).
