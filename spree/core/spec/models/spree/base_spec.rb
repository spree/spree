...
describe Spree::Base do
  ...
  describe '.json_api_columns' do
    ...
    it { expect(Spree::Address.json_api_columns).to contain_exactly('address1', 'address2', 'alternative_phone', 'city', 'company', 'country_iso', 'created_at', 'deleted_at', 'firstname', 'label', 'lastname', 'phone', 'state_abbr', 'state_name', 'updated_at', 'zipcode', 'metadata', 'quick_checkout', 'latitude', 'longitude') }
    it { expect(Spree::Address.json_api_columns).not_to include('country_id') }
  end

  describe '.json_api_permitted_attributes' do
    it { expect(Spree::Address.json_api_permitted_attributes).to contain_exactly('firstname', 'lastname', 'address1', 'address2', 'city', 'zipcode', 'phone', 'country_iso', 'state_abbr', 'state_name', 'alternative_phone', 'company', 'state_id', 'country_id', 'created_at', 'updated_at', 'customer_id', 'deleted_at', 'label', 'metadata', 'quick_checkout', 'latitude', 'longitude') }
  end
end
