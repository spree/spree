...
describe Spree::PromotionActions::CreateLineItems, type: :model do
  let(:order) { create(:order) }
  let(:action) { Spree::PromotionActions::CreateLineItems.create!(promotion: promotion) }
  ...
  context '#perform' do
    ...
    context 'order is eligible' do
      ...
      it 'only adds the delta of quantity to an order' do
        Spree::Carts::AddItem.call(order: order, variant: shirt)
        action.perform(payload)
