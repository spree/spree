...
describe Spree::Taxonomy, type: :model do
  ...
  context '#destroy' do
    ...
    it 'destroys all associated taxons' do
      @taxonomy.destroy
      expect { Spree::Category.find(@root_taxon.id) }.to raise_error(ActiveRecord::RecordNotFound)
      expect { Spree::Category.find(@child_taxon.id) }.to raise_error(ActiveRecord::RecordNotFound)
    end
  end
