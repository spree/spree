...
RSpec.describe Spree::Event do
  ...
  describe '#store' do
    ...
    it 'returns nil when store_id is blank' do
      allow(Spree::Current).to receive(:store).and_return(nil)
      event = described_class.new(name: 'order.placed', store_id: nil)
  ...
  describe '#resource_type' do
    it 'extracts the resource type from the event name' do
      event = described_class.new(name: 'order.placed', payload: {})
  ...
  describe '#action' do
    it 'extracts the action from the event name' do
      event = described_class.new(name: 'order.placed', payload: {})
      expect(event.action).to eq('placed')
    end
  ...
  describe '.matches?' do
    it 'matches exact names' do
      expect(described_class.matches?('order.placed', 'order.placed')).to be true
      expect(described_class.matches?('order.placed', 'order.canceled')).to be false
    end

    it 'matches wildcard patterns' do
      expect(described_class.matches?('order.placed', 'order.*')).to be true
    ...
    it 'matches global wildcard' do
      expect(described_class.matches?('order.placed', '*')).to be true
  ...
  describe '#attributes' do
    it 'returns a hash representation with string keys' do
      ...
      expect(hash['name']).to eq('order.placed')
  ...
  describe '#inspect' do
    it 'returns a readable string representation' do
      event = described_class.new(name: 'order.placed', payload: {})
      expect(event.inspect).to match(/Spree::Event id="[0-9a-f-]{36}" name="order.placed" store_id=#{store.id} created_at=/)
    end
  end
end
