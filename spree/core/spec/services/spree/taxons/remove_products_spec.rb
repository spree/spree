  ...
  describe Taxons::RemoveProducts do
    ...
    describe '#call' do
      ...
      it 'updates products_count on taxons' do
        expect { subject }.to change { taxons.first.reload.products_count }.from(products.size).to(0)
          .and change { taxons.second.reload.products_count }.from(products.size).to(0)
      end

      it 'updates categories_count on products' do
        expect { subject }.to change { products.first.reload.categories_count }.from(taxons.size).to(0)
          .and change { products.second.reload.categories_count }.from(taxons.size).to(0)
      end
    end
  end
end
