...
RSpec.describe Spree::Events::Registry do
  ...
  describe '#register' do
    it 'registers a subscriber for a pattern' do
      registry.register('order.placed', subscriber_class)

      expect(registry.size).to eq(1)
      expect(registry.registered?('order.placed')).to be true
    end

    it 'allows multiple subscribers for the same pattern' do
      registry.register('order.placed', subscriber_class)
      registry.register('order.placed', another_subscriber)

      expect(registry.size).to eq(2)
    end

    it 'stores subscription options' do
      registry.register('order.placed', subscriber_class, async: false)

      subscription = registry.subscriptions_for('order.placed').first
      expect(subscription.options[:async]).to be false
    end

    it 'returns the subscription' do
      subscription = registry.register('order.placed', subscriber_class)

      expect(subscription).to be_a(Spree::Events::Registry::Subscription)
      expect(subscription.pattern).to eq('order.placed')
  ...
  describe '#unregister' do
    ...
    it 'removes a specific subscriber' do
      result = registry.unregister('order.placed', subscriber_class)

      expect(result).to be true
      expect(registry.size).to eq(1)
      expect(registry.subscriptions_for('order.placed').map(&:subscriber)).not_to include(subscriber_class)
    end
    ...
    it 'keeps other subscribers intact' do
      registry.unregister('order.placed', subscriber_class)

      expect(registry.subscriptions_for('order.placed').map(&:subscriber)).to include(another_subscriber)
    end
  end
  ...
  describe '#subscriptions_for' do
    ...
    it 'returns subscriptions for exact match' do
      subscriptions = registry.subscriptions_for('order.placed')
    ...
    context 'with global wildcard' do
      ...
      it 'matches all events' do
        expect(registry.subscriptions_for('order.placed').size).to eq(3)
  ...
  describe '#patterns' do
    it 'returns unique patterns' do
      ...
      expect(registry.patterns).to contain_exactly('order.placed', 'order.*')
    end
  end
  ...
  describe '#registered?' do
    ...
    it 'returns false for unregistered patterns' do
      expect(registry.registered?('order.placed')).to be false
    end
  end
