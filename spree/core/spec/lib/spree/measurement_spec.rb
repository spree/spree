...
RSpec.describe Spree::Measurement do
  ...
  # Two lists of the same vocabulary can drift, and a unit a merchant may
  # store but this cannot convert is silently read as the default — a pound
  # counted as a kilogram, an inch as a centimeter.
  describe 'the units it covers' do
    it 'converts every unit a variant may be measured in' do
      expect(described_class::LENGTH_IN_CENTIMETERS.keys).to match_array(Spree::Variant::DIMENSION_UNITS)
      expect(described_class::WEIGHT_IN_KILOGRAMS.keys).to match_array(Spree::Variant::WEIGHT_UNITS)
    end
  end
end
