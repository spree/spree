...
describe Spree::CurrencyHelper, type: :helper do
  ...
  describe '#supported_currency_options' do
    it { expect(supported_currency_options).to contain_exactly(['PLN — Polish Złoty', 'PLN'], ['GBP — British Pound', 'GBP'], ['EUR — Euro', 'EUR']) }
  end
  ...
  describe '#currency_presentation' do
    it { expect(currency_presentation('EUR')).to eq(['EUR — Euro', 'EUR']) }
  end
