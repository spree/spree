...
RSpec.describe Spree::Events::ShipmentSerializer do
  ...
  describe '#as_json' do
    it 'includes identity attributes' do
      expect(subject[:id]).to eq(shipment.prefixed_id)
    ...
    it 'includes foreign keys' do
      expect(subject[:order_id]).to eq(shipment.order&.prefixed_id)
      expect(subject[:stock_location_id]).to eq(shipment.stock_location&.prefixed_id)
    end
