  ...
  module Api
    module V3
      ...
      module SellerAuthentication
        ...
        # Rejects an admin or storefront token before any seller is resolved.
        def expected_audience
          Spree::Api::V3::JwtAuthentication::JWT_AUDIENCE_SELLER
        end

        # Capability comes from the roles the seller holds *on this seller*.
        # Passing the resource is what stops a store role — or the store's
        # `admin` super-role — being read here (see Spree::Ability).
        def ability_options
          { store: current_store, resource: current_seller }
        end
