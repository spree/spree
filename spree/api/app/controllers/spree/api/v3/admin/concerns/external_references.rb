      ...
      module Admin
        module Concerns
          ...
          module ExternalReferences
            ...
            #
            # Several controllers permit different keys on create and update —
            # orders take +shipping_address+ on create and +ship_address+ on
            # update — so a payload key the update action does not name would
            # be silently dropped on a replay. +normalize_upsert_params!+ is
            # where such a controller translates; the default is a no-op.
