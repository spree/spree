          ...
          class PasswordResetsController < Store::BaseController
            ...
            def update
              ...
              if user.update(password: params[:password], password_confirmation: params[:password_confirmation])
                ...
                refresh_token = Spree::RefreshToken.create_for(
                  user,
                  audience: JWT_AUDIENCE_STORE,
                  request_env: { ip_address: request.remote_ip, user_agent: request.user_agent&.truncate(255) }
                )
