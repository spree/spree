  ...
  module Api
    module V3
      module Vendor
        ...
        class AuthController < Vendor::BaseController
          include Spree::Api::V3::Vendor::AuthCookies
          ...
          rate_limit to: Spree::Api::Config[:rate_limit_login],
                     ...
                     store: Rails.cache, only: :create, name: 'vendor_login',
                     with: -> { render_rate_limited(limit: Spree::Api::Config[:rate_limit_login]) }
          rate_limit to: Spree::Api::Config[:rate_limit_refresh],
                     within: Spree::Api::Config[:rate_limit_window].seconds,
                     store: Rails.cache, only: :providers, name: 'vendor_providers',
                     with: -> { render_rate_limited(limit: Spree::Api::Config[:rate_limit_refresh]) }
          rate_limit to: Spree::Api::Config[:rate_limit_refresh],
                     within: Spree::Api::Config[:rate_limit_window].seconds,
                     store: Rails.cache, only: [:refresh, :logout], name: 'vendor_refresh',
                     with: -> { render_rate_limited(limit: Spree::Api::Config[:rate_limit_refresh]) }

          # Signing in is what establishes which vendors the caller may act as,
          # so these actions run before any vendor is known.
          skip_before_action :set_current_vendor_context, only: [:create, :refresh, :logout, :providers]
          skip_before_action :authenticate_vendor!, only: [:create, :refresh, :logout, :providers]

          # POST /api/v3/vendor/auth/login
          def create
            ...
            # not enough: without a role on some vendor there is no panel to
            # sign in to, and minting a token would hand out an audience the
            # holder can do nothing with.
            return render_authentication_failed(Spree.t(:vendor_membership_required)) unless user.vendor_member?

            set_refresh_cookie(
              Spree::RefreshToken.create_for(
                user,
                audience: Spree::Api::V3::JwtAuthentication::JWT_AUDIENCE_VENDOR,
          ...
          # GET /api/v3/vendor/auth/providers
          def providers
            render json: { providers: Spree.vendor_authentication_strategies.describe }
          end

          # POST /api/v3/vendor/auth/refresh
          def refresh
            ...
            # be exchangeable for a vendor one.
            refresh_token = Spree::RefreshToken.active.
                            for_audience(Spree::Api::V3::JwtAuthentication::JWT_AUDIENCE_VENDOR).
            ...
            unless user.vendor_member?
              refresh_token.destroy
              clear_refresh_cookie
              return render_error(
                code: ErrorHandler::ERROR_CODES[:access_denied],
                message: Spree.t(:vendor_membership_required),
                status: :forbidden
              )
            end
          ...
          # POST /api/v3/vendor/auth/logout
          def logout
            refresh_token_value = refresh_token_from_cookie
          ...
          def auth_response(user)
            {
              token: generate_jwt(user, audience: Spree::Api::V3::JwtAuthentication::JWT_AUDIENCE_VENDOR),
              user: Spree.api.admin_user_serializer.new(user, params: { store: user.vendors.first&.store }).to_h,
              vendors: serialized_vendors(user)
            }
          end

          # The panel needs these to choose an X-Spree-Vendor-Id before it can
          # make any other request.
          def serialized_vendors(user)
            user.vendors.map { |vendor| { id: vendor.prefixed_id, name: vendor.name, status: vendor.status } }
          end

          def authentication_strategies
            Spree.vendor_authentication_strategies
          end
