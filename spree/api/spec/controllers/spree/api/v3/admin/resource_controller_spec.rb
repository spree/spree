...
# Unit spec for `Spree::Api::V3::Admin::ResourceController#scope` — CanCanCan
# lives on the admin branch only. The scope infers the ability action from
# `request.method` and narrows the relation via `accessible_by`, but only when
# there's no `@parent` (nested resources defer to their parent's
# authorization).
RSpec.describe Spree::Api::V3::Admin::ResourceController, type: :controller do
  ...
  describe '#scope with @parent' do
    ...
    # is deliberately skipped so the parent's permissions win.
