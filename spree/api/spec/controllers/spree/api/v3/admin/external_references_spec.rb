...
RSpec.describe Spree::Api::V3::Admin::OrdersController, 'upsert by external id', type: :controller do
  ...
  # Orders permit `shipping_address` on create and `ship_address` on update.
  # A replayed feed row must keep the address it sent, not have it dropped by
  # being reinterpreted through the other action's params.
