...
RSpec.describe Spree::Api::V3::Admin::MeController, type: :controller do
  ...
  describe 'GET #show' do
    ...
    context 'as a staffer with a limited role' do
      let(:staffer) do
        create(:admin_user, :without_admin_role).tap do |user|
          create(:role_user, user: user, role: create(:role, name: 'viewer', permissions: %w[write_orders], resource: store))
        end
      end
      let(:headers) do
    ...
    context 'as a staffer with different roles on different stores' do
      ...
      let(:staffer) do
        create(:admin_user, :without_admin_role).tap do |user|
          create(:role_user, user: user, role: create(:role, name: 'a_orders', permissions: %w[write_orders], resource: store))
          create(:role_user, user: user, role: create(:role, name: 'b_products', permissions: %w[read_products], resource: store_b))
        end
      end
      let(:headers) do
        { 'Authorization' => "Bearer #{Spree::Api::V3::TestingSupport.generate_jwt(staffer, audience: Spree::Api::V3::JwtAuthentication::JWT_AUDIENCE_ADMIN)}" }
