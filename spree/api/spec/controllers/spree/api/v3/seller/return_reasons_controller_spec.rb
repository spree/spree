...
RSpec.describe Spree::Api::V3::Seller::ReturnReasonsController, type: :controller do
  render_views
  ...
  describe 'GET #index' do
    let!(:reason) { create(:return_reason, store: store, name: 'Damaged') }

    it 'lists the marketplace’s reasons' do
      get :index, as: :json

      expect(response).to have_http_status(:ok)
      expect(json_response['data'].map { |row| row['name'] }).to include('Damaged')
    end
    ...
    it 'leaves out retired reasons' do
      create(:return_reason, store: store, name: 'Obsolete', active: false)
    ...
    it 'leaves out another store’s reasons' do
      ...
      create(:return_reason, store: other_store, name: 'Elsewhere')
