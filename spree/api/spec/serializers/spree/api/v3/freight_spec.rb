...
RSpec.describe 'freight serialization' do
  ...
  describe Spree::Api::V3::CartSerializer do
    ...
    it 'reports the load the selected freight rate was quoted on' do
      create(:line_item, cart: cart, order: nil, variant: variant, quantity: 24)
      select_freight_rate(cart, variant_id: variant.prefixed_id, units: 24, cartons: 2, pallets: 1, complete: true)
    ...
    # A buyer is told how big their shipment is, not which SKU accounts for
    # which part of it — that breakdown is the warehouse's, and lives on the
    # admin shape.
    it 'sends totals without the per-product breakdown' do
      create(:line_item, cart: cart, order: nil, variant: variant, quantity: 24)
      select_freight_rate(cart, variant_id: variant.prefixed_id, sku: 'SECRET-SKU', units: 24, cartons: 2, complete: true)

      summary = described_class.new(cart.reload, params: { store: store }).to_h['freight_summary']

      expect(summary['total_cartons']).to eq(2)
      expect(summary).not_to have_key('lines')
    end

    # Measured goods are not a freight shipment until a freight rate says so:
    # a retail basket reports nothing, however it is packed.
    it 'reports nothing before a freight rate is selected' do
      create(:line_item, cart: cart, order: nil, variant: variant, quantity: 24)

      expect(described_class.new(cart.reload, params: { store: store }).to_h['freight_summary']).to be_nil
    end
  end
