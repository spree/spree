      ...
      class ProductsController < ResourceController
        ...
        def edit_variants
          ...
          @stock_location = if params[:stock_location_id].present?
                              Spree::StockLocation.find(params[:stock_location_id])
                            else
                              default_stock_location_for_product(@product)
                            end
          @currency_symbol = ::Money::Currency.find(@currency).symbol
  
          @q = @product.variants.includes(:prices, :stock_items, option_values: :option_type).ransack(params[:q])
          @variants = @q.result(distinct: true)
        end
  
        def update_variants
          if @product.update(permitted_resource_params)
            flash[:success] = Spree.t(:successfully_updated, resource: Spree.t(:variants))
            redirect_to edit_admin_product_path(@product)
          else
            flash[:error] = @product.errors.full_messages.join(', ')
            redirect_to edit_variants_admin_product_path(@product)
          end
        end
