namespace :db do
  ...
  task :bootstrap  => :environment do
    ...
    Dir.glob(File.join(FooExtension.root, "db", 'sample', '*.{yml,csv}')).each do |fixture_file|
      Fixtures.create_fixtures("#{FooExtension.root}/db/sample", File.basename(fixture_file, '.*'))
    end
