      ...
      class Product < Base
        ...
        def search
          ...
          unless Spree::Config.show_products_without_price
            @products = @products.merge(Spree::Price.amount_present).merge(Spree::Price.with_currency(current_currency))
          end
        ...
        def get_products_conditions_for(base_scope, query)
          query.blank? ? base_scope : base_scope.like_any([:name, :description], query.split)
        end

        def prepare(params)
          @params[:taxon] = Spree::Taxon.find(params[:taxon]) unless params[:taxon].blank?
