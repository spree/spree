    ...
    module ControllerHelpers
      module Order
        ...
        def find_order_by_token_or_user(options={}, with_adjustments = false)
          ...
          incomplete_orders = Spree::Order.incomplete.includes(line_items: [variant: [:option_values, :product]])
          order = if with_adjustments
                    incomplete_orders.includes(:adjustments)
                  else
                    incomplete_orders
                  end.lock(options[:lock]).find_by(current_order_params)
