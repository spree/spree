...
describe Spree::LocaleHelper, type: :helper do
  ...
  describe '#all_locales_options' do
    it { expect(all_locales_options).to contain_exactly(['English (Default)', 'en'], ['Deutsch (DE)', 'de'], ['Français (FR)', 'fr']) }
  end
  ...
  describe '#available_locales_options' do
    ...
    it { expect(available_locales_options).to contain_exactly(['English (Default)', 'en'], ['Deutsch (DE)', 'de']) }
  end
