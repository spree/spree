...
describe Spree::Country do
  it "can find all countries group by states required" do
    ...
    states_required[country_states_required.id.to_s].should be true
    states_required[country_states_not_required.id.to_s].should be false
  end

  it "returns that the states are required for an invalid country" do
    Spree::Country.states_required_by_country_id['i do not exit'].should be true
  end
end
