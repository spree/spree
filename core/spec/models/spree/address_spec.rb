...
describe Spree::Address, type: :model do
  describe 'clone' do
    it 'creates a copy of the address with the exception of the id, label, updated_at and created_at attributes' do
      state = create(:state)
      ...
      expect(cloned.label).not_to eq(original.label)
    end
  end
