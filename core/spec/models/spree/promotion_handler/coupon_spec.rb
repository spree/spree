...
module Spree
  module PromotionHandler
    describe Coupon, type: :model do
      ...
      context 'coupon code promotion with apply_collectively set to true' do
        ...
        let(:calculator) { Calculator::Promotion::FlatRate.new(preferred_amount: 10, preferred_apply_collectively: true) }
      ...
      context 'existing coupon code promotion with apply_collectively set to false' do
        let!(:promotion) { Promotion.create name: 'promo', code: '10off' }
        let!(:action) { Promotion::Actions::CreateItemAdjustments.create(promotion: promotion, calculator: calculator) }
        let(:calculator) { Calculator::Promotion::FlatRate.new(preferred_amount: 10, preferred_apply_collectively: false) }
          ...
          context 'right coupon given' do
            context 'with correct coupon code casing' do
              ...
              it 'successfully activates promo' do
                ...
                # Ensure that applying the adjustment actually affects the order's total!
                expect(order.reload.total).to eq(100.00)
              end
