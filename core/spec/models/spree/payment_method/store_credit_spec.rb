...
describe Spree::PaymentMethod::StoreCredit do
    ...
    context 'with a valid request' do
      ...
      context 'with an originator' do
        ...
        it 'passes the originator' do
          expect_any_instance_of(Spree::StoreCredit).to receive(:authorize).with(anything, anything, action_originator: originator)
          subject
        end
      end
    end
  ...
  context "#purchase" do
    ...
    it "captures a purchase if it can find a pending credit for the correct amount" do
      ...
      expect(response.message).to include Spree.t('store_credit_payment_method.successful_action', action: Spree::StoreCredit::CAPTURE_ACTION)
    end
  end
  ...
  context "#credit" do
    ...
    context 'with an originator' do
      ...
      it 'passes the originator' do
        allow_any_instance_of(Spree::StoreCredit).to receive(:credit).with(anything, anything, anything, action_originator: originator)
        subject
      end
    end
  end
