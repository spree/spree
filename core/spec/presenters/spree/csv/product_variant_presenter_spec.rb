...
RSpec.describe Spree::CSV::ProductVariantPresenter do
  ...
  describe '#call' do
    ...
    it 'returns array with correct values' do
      ...
      expect(subject[29]).to eq Rails.application.routes.url_helpers.rails_blob_url(variant.images[0].attachment, host: store.formatted_url)
      expect(subject[30]).to eq Rails.application.routes.url_helpers.rails_blob_url(variant.images[1].attachment, host: store.formatted_url)
      expect(subject[31]).to eq Rails.application.routes.url_helpers.rails_blob_url(variant.images[2].attachment, host: store.formatted_url)
    end
