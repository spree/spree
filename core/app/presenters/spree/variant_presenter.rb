module Spree
  class VariantPresenter
    ...
    def backorderable_variant_ids
      @_backorderable_variant_ids ||= Spree::Variant
        .joins(:stock_items)
        .where(id: @variants.map(&:id))
        .where(spree_stock_items: { backorderable: true })
        .merge(Spree::StockItem.with_active_stock_location)
        .distinct
        .ids
    end
  end
