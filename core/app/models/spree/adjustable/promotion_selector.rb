  ...
  module Adjustable
    class PromotionSelector
      ...
      def select!
        ...
        set_only_best_promotion_adjustments_eligible
      ...
      def find_other_adjustables_to_update
        @other_adjustables = adjustments_to_update.map { |a| [a.adjustable_type, a.adjustable_id] }
        @other_adjustables.delete(adjustables_type_and_id.values)
      end
      ...
      def needs_updating?(adjustment)
        adjustment.eligible != best_promotion_adjustments.include?(adjustment)
      end

      def best_promotion_adjustments
        @best_promotion_adjustments ||= promotions_adjustments(@best_promotion.try(:id))
      end
      ...
      def update_other_adjustables_promo_totals
        @other_adjustables.each do |(type, id)|
      ...
      def update_adjustable_promo_total(type, id)
        adjustments = where(best_promotion_adjustments, adjustable_type: type, adjustable_id: id)
      ...
      def set_only_best_promotion_adjustments_eligible
        Spree::Adjustment.where(id: best_promotion_adjustments.map(&:id)).update_all(eligible: true)
      end

      def adjustable_promo_total
        where(best_promotion_adjustments, adjustables_type_and_id).sum(&:amount)
      end
    end
  end
end
