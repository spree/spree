...
module Spree
  class Adjustment < Spree::Base
    ...
    scope :with_complete_order, -> { joins(:order).merge(Spree::Order.where.not(completed_at: nil)) }
    scope :with_incomplete_order, -> { joins(:order).merge(Spree::Order.where(completed_at: nil)) }
