...
module Spree
  module Calculator::Returns
    class DefaultRefundAmount < ReturnsCalculator
      ...
      def weighted_order_adjustment_amount(return_item, inventory_unit)
        inventory_unit.order.adjustments.eligible.non_tax.sum(:amount) * percentage_of_order_total(return_item, inventory_unit)
      end
      ...
      def weighted_line_item_pre_tax_amount(return_item, inventory_unit)
        inventory_unit.line_item.pre_tax_amount * percentage_of_line_item(return_item, inventory_unit)
      end
