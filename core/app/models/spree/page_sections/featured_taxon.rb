module Spree
  module PageSections
    class FeaturedTaxon < Spree::PageSection
      ...
      def products(currency, include_params)
        @products ||= begin
          finder_params = {
            store: store,
          ...
          products_finder.new(scope: store.products.includes(include_params), params: finder_params).execute.limit(preferred_max_products_to_show)
        end
      end
