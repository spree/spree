Then /^I should see listing orders tabular attributes with created_at descending$/ do
  ...
  data = output[1]
  data[0].should == Spree::Order.limit(1).order('completed_at desc').to_a.first.created_at.strftime('%Y-%m-%d')
end

Then /^I should see listing orders tabular attributes with created_at ascending$/ do
  output = tableish('table#listing_orders tr', 'td,th')
  data = output[1]
  data[0].should == Spree::Order.limit(1).order('completed_at asc').to_a.first.created_at.strftime('%Y-%m-%d')
end
...
Then /^I should see listing orders tabular attributes with completed_at descending$/ do
  ...
  data = output[1]
  data[0].should == Spree::Order.limit(1).order('created_at desc').to_a.first.created_at.strftime('%Y-%m-%d')
end

Then /^I should see listing orders tabular attributes with completed_at ascending$/ do
  output = tableish('table#listing_orders tr', 'td,th')
  data = output[1]
  data[0].should == Spree::Order.limit(1).order('created_at asc').to_a.first.created_at.strftime('%Y-%m-%d')
end

Then /^I should see listing orders tabular attributes with order number ascending$/ do
  output = tableish('table#listing_orders tr', 'td,th')
  data = output[1]
  data[1].should == Spree::Order.limit(1).order('number asc').to_a.first.number
end

Then /^I should see listing orders tabular attributes with order number descending$/ do
  output = tableish('table#listing_orders tr', 'td,th')
  data = output[1]
  data[1].should == Spree::Order.limit(1).order('number desc').to_a.first.number
end
