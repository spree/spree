      ...
      def apply_coupon_code
        if @order.coupon_code.present?
          ...
          if @order.adjustments.promotion.eligible.detect { |p| p.originator.promotion.code == @order.coupon_code }.present?
            return { :coupon_applied? => true, :notice => t(:coupon_code_already_applied) }
          else
            ...
            if promotion.present?
              if promotion.expired?
                return { :coupon_applied? => false, :error => t(:coupon_code_expired) }
              end

              if promotion.usage_limit_exceeded?
                return { :coupon_applied? => false, :error => t(:coupon_code_max_usage) }
              end
              ...
              if promo.present? and promo.eligible
                return { :coupon_applied? => true, :success => t(:coupon_code_applied) }
                true
              elsif previous_promo.present? and promo.present?
                return { :coupon_applied? => false, :error => t(:coupon_code_better_exists) }
              elsif promo.present?
                return { :coupon_applied? => false, :error => t(:coupon_code_not_eligible) }
              else
                # if the promotion was created after the order
                return { :coupon_applied? => false, :error => t(:coupon_code_not_found) }
              end
            else
              return { :coupon_applied? => false, :error => t(:coupon_code_not_found) }
            end
