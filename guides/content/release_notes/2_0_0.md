...
There are 3 main components that make up split shipments: Stock Locations, Stock Items, and Stock Movements.
...
For more information on the classes introduced by split shipments and how to work with them programatically, see the [Shipments Guide](http://edgeguides.spreecommerce.com/developer/shipments.html#split-shipments).
...
configuration was confusing and lead to invalid data being inserted for sample data.
...
* Fix issue where `params[:keyword]`, not `params[:keywords]` was acknowledge on taxons/show.html.erb. <%= issue 2258 %> <%= issue 2270 %>
...
* Scale and precison have been corrected in split_prices_from_variants migration. <%= issue 2358 %>
...
* Allow Address#phone validation to be overriden. <%= issue 2394 %>
...
* option values in `Variant#options_text` are now sorted in a predicatable order. <%= issue 2432 %>
...
* Allow for specially overriden attributes for line items in the API. <%= issue 2916 %>
