...
h4. Pre-compiling Assets

Rail's already supports pre-compiling of assets which is intended to offload the overhead of generating and serving assets from the application server in production environments. To pre-compile assets for production  you would normally execute the following rake task on the production server.
