...
INFO: While the upgrade instructions that follow briefly cover the new asset pipeline features we suggest you review the guides above as part of our upgrade preparation.

h3. Promotions

Promotions have been refactored to include new <tt>activator</tt> support, which can now fire or activate a promotion for a variety of different user actions. 
...
spree_site.rb file is no longer required so it's important to migrate any existing code out of this file and into the correct location.
...
h5. Remove spree_site from config/applicaiton.rb
...
NOTE: If you are using a library like Jammit solely for concatenating and minify your javascript and stylesheet files then we suggest you remove it from your application as part of the Rails 3.1 upgrade.
...
To include your previously defined old style theming hooks in <tt>lib/site_hooks.rb</tt> add the following to the bottom of <tt>config/application.rb</tt>
