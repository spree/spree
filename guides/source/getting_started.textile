...
INFO. If you have an existing Rails application with customizations (as opposed to just the starter app that we've shown here) you will want to make a note of any files that this generator changes.  Its possible that it may alter your javascript or stylesheet files in undesirable ways.  You should check the differences via +git diff+ or some other process and manually merge the needed changes if necessary.
