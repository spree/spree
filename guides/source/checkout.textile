...
The default state machine for the +Spree::Order+ model is defined in +app/models/spree/order.rb+ of spree_core as follows:
...
INFO: This completely overwrites the default +Spree::Order+ +state_machine+.  This means it is necessary to explicitly define all of the events, transitions, callbacks etc associated with the +Spree::Order+ +state_machine+. In this example we ommited everything except the definition of the +next+ event.
