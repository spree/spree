...
We have moved the 'spree' command line tool to its own gem. It will add Spree to an existing Rails application. The tool will add the Spree gem, copy migrations, initializers and generate sample data.
...
If you have previous versions of Spree installed, you may have conflicts with the name 'spree'. We have included an alternative binary named 'spree_cmd'. The recomendation is to remove older versions of Spree.
