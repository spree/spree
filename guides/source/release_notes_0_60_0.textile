...
The 0.60.0 release contains a significant change to all controllers within Spree as they have been refactored to remove the use of the "resource_controller":http://github.com/jamesgolick/resource_controller gem. This release may have an impact on existing extensions or site customizations that leverage some of resource_controllers features, so it's important to read the details on it's removal below and review any code which might be affected.
...
WARNING: Always be sure to perform a complete database backup before performing any operations.
...
WARNING: Use of these generic helpers is discouraged in favor of the default Rails helper urls.

h5. Instance Variables

* +@object+
* +@collection+

WARNING: Use of these variables is strongly discouraged.  Use the actual Rails standard variable names instead.

h4. Unsupported Functionality
...
INFO: See the "Customization":customization.html guide for more details on +respond_override+

h5. Deprecated Methods
...
NOTE: While the use of resource_controller has been completely removed from all of Spree's core controllers, the dependency on resource_controller gem will remain for a while to allow extension authors to update their projects.

h3. Miscellaneous Changes

There are also a series of minor bug fixes and improvments.  Please see the "Github compare":http://github.com/spree/spree/compare/v0.50.2...v0.60.0 for more details.
