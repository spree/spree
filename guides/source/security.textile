...
Spree uses a third party authentication library for Ruby known as "Devise":https://github.com/plataformatec/devise.  This library provides a host of useful functionality that is in turn available to Spree, including the following features:
...
h4. Encryption Algorithm

By default, Spree supports the same encryption scheme as the one used by the "Authlogic":https://github.com/binarylogic/authlogic gem.  This encryption scheme is sufficient but its not the best choice currently available.  We have chosen it to allow existing Spree sites (based on 0.11.x and earlier) to more easily migrate to newer versions of Spree.

If you have a brand new site (and password migration is not a concern) you may want to consider the more powerful "bcrypt":http://bcrypt.sourceforge.net/ algorithm.

INFO. Currently this requires modifying the +spree_auth+ gem directly.  We're working with the Devise team to come up with a more convenient solution for handling this and other customizations to Devise.
...
h5. Resource Controller

Since most controllers in Spree are based on "resource controller":https://github.com/jamesgolick/resource_controller, Spree is able to use a short cut that means you do not have to explicitly check resource access in most cases.  The +spree_auth+ gem introduces the following code in +resource_controller_decorator.rb+:
...
This overrides the default behavior of +resource_controller+ and enforces an authorization check against CanCan before proceeding.

INFO. All Rails controllers that include +resource_controller+ are automatically protected.  By default, only the admin role will be able to access them.  You will need to add your own custom CanCan rules to grant access.

h4. Custom Authorization Rules

We have modified the original CanCan concept to make it easier for extension developers and end users to add their own custom authorization rules.  For instance, if you have an "artwork extension" that allows users to attach custom artwork to an order, you will need to add rules so that they have permissions to do so.  Recall that the use of +resource_controller+ will automatically deny access for all operations to non admin users until you explicitly allow it.
