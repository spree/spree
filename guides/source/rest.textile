...
You will need an authentication token to access the API. These keys can be generated on the user edit screen within the admin interface. To make a request to the API, pass a key parameter along with your request:

<shell>
curl http://example.com/api/products.json?key=YOUR_KEY_HERE
</shell>
...
<plain>
GET /api/v1/products.json?page=1
</plain>
...
<plain>
GET /api/v1/products/a-product.json
</plain>
...
<plain>
POST /api/v1/products/create.json
</plain>

For instance, a request to create a new product called "Headphones" with a price of $100 would look like this:

<plain>
POST /api/v1/products/create.json?product[name]=Headphones&product[price]=100
</plain>
...
<plain>
PUT /api/v1/products/a-product.json
</plain>

For instance, to update a product's name, send it through like this:

<plain>
PUT /api/v1/products/a-product.json?product[name]=Headphones
</plain>
...
<plain>
DELETE /api/v1/products/a-product.json
</plain>
