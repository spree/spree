...
"preferences guide":preferences.html#persisting-modificationsto-preferences for more 
info.
...
use the +find_or_create+ technique, with something like this:
...
declare them in advance, or use +find_or_create+. You'll probably find it easier to 
...
v = Variant.create :product => p, :sku => sku, :price => price

v.option_values = opts.split(/\s*&\s*/).map do |nm| 
  OptionValue.find_or_create_by_name_and_presentation_and_option_type_id(nm,nm, color_type.id)
end
</ruby>
...
or by alphabetical order. The +OptionValue+ model uses +acts_as_list+ for setting the 
...
<ruby> 
color_type = OptionType.create :name => "Color", :presentation => "Color"
color_options = %w[Red Blue Green].split.map {|n| OptionValue.create :name => n, :presentation => n, :option_type => color_type}
</ruby>
...
<ruby>
color_type.option_values.sort_by {|o| o.name}.each_with_index do |val,pos| 
...
"Steph Powell":http://github.com/stephp has produced a useful blog post on
...
attachments and their various size formats.  (See the "theming 
guide":theming.html#configuringyourthemesimagesizes for info on altering the image formats.)
