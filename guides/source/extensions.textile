...
Spree is moving at a rapid pace. Code is constantly changes and new releases are being pushed out. Amid all these changes sometimes it becomes difficult to keep track of which version of Spree is compatible with which version of spree extension. To solve this problem spree extension authors are encouraged to have a file called "Versionfile".

h5. What is Versionfile

Just like Gemfile Versionfile is a plain text file which keeps information about which version of spree extension is compatible with which version of Spree. Given below is a sample Versionfile.
...
Above Versionfile is saying that if you are using Spree version "0.50.x" then use "master" branch. If you are using Spree version "0.40.x" then you should use tag "v1.0.0".
...
There are four differt ways of specifying how to get extension code.
...
h5. How do I use Versionfile

Versionfile is not directly consumed by the users. There is a crawler which crawls Versionfile of all the registered extensions once every day. So any changes you make to Versionfile should be captured within 24 hours.

After Versionfile info has been captured if you visit Spree extension information page then under "Stable releaseses" and "Dev releases" you will see a link called "Show". Clicking that link will show the users the exact line they need to copy of their Gemfile. For example "Click here":http://spreecommerce.com/extensions/110-email-to-friend to see an example in action.

h5. I updated Versionfile but the information is not being captured

While parsing the lines gathered from Versionfile if a line is invalid then that line is skipped. So make sure that entries in Versionfile is invalid. Soon we would be releasing a tool that would verify the Versionfile and would instantly provide a feedback if something is not right.
