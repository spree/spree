...
By default Spree will only load +/stylesheets/screen.css+ into the main layout. Rather than 
override the layout template you can load your custom stylesheet by setting the +:stylesheets+
option in an extension initializer, e.g. add the following to 
+SITE_EXTENSION_DIR/config/my_theme.rb+.
