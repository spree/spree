...
This step will make sure any extra necessary gems are properly installed on the production server. Next, we will setup the database, assuming a clean database. To specify the Ruby on Rails environment, append the RAILS_ENV variable to your Rake tasks, e.g., RAILS_ENV=production.
...
You can load the seed data (like countries, etc.) into your database.
...
If you do not wish to use SSL in production, or if you wish to enable SSL in development mode, you will have to change the +:allow_ssl_in_production+ configuration setting.  This can be done via the admin interface (+Configuration | General Settings+) as shown below, but it is usually better to use an initializer (see the "Extensions guide":extensions.html#extensioninitializers).
