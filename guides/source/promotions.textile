...
* **Products** The order must include the chosen products, or products from the chosen product group
* **User** The promotion is only available to the specified users
...
To configure the outcome of a promotion, Promotion Actions are added in the same manner as rules. This means a promotion could result in mulitple actions being performed such as a discount adjustment created on the order, an email being delivered or a predefined set of items being added to the cart. Currently the only action available is 'Create adjustment' but it is easy to add new action types by defining a subclass of *PromotionAction* which implements the *perform* method. Call *register* on your subclass to make it available to choose in the admin interface. See Promotion::Actions::CreateAdjustment for an example.
