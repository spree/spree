...
Authentication is disabled by default within Spree as of this release, with the application supposed to be providing its own authentication. If you have an application that depended on this functionality within Spree existing, please add "spree_auth_devise":https://github.com/spree/spree_auth_devise to your Gemfile and then put this line inside +config/initializers/spree.rb+ as well:
