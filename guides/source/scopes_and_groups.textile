...
* +keywords+ - defaults to search in the product names and descriptions, though you can modify or extend this via the "Searcher API":#searcher-api
...
This is a related issue, for providing powerful full-text searching and facets on the product database.
There are several possible search engines to use, such as Xapian, Solr, Sphinx. Spree interfaces for these are being developed as extensions, and the code should be consulted for useful examples. 
The Spree core contains an API and a simple base implementation which accesses the main database directly.

To set the current searcher implementation, assign a suitable object to +Spree::Config.searcher+. The default is the simple base implementation.

TODO. document the main methods and integration
