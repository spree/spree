...
NOTE. You can enable collection of extra _shipping instructions_ by setting the option +Spree::Config.set(:shipping_instructions => true)+. This is turned off by default. Such instructions will be shown in an order's shipment admin page. 
...
Your calculator should accep a shipmend return a cost. Your calculator can look at 
any reachable data, but typically uses the address and order in a shipment, plus 
the information from variants.
...
which a calculator could extract imposed prices (and not go through its usual 
calculations).

h4. Variant configuration

Variants can be specified with weight and dimension information. 
