...
Spree supports SSL and contains a special filter to require SSL for certain sensitive pages (checkout and admin-related.)  It also has the ability to redirect SSL requests that do not require SSL back to standard unencrypted HTTP.  The code for this is built right into Spree and is based on the "ssl_requirement":https://github.com/rails/ssl_requirement/tree/master by David Heinemeier Hansson.
