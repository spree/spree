...
payment information. For a developer's viewpoint, a gateway supports five core 
...
* authorize and capture - immediate approval of transfer, with actual transfer 
                          of funds occurring at a later date
...
responses. See XYZ to see how to use it in testing. 
...
Active Merchant currently supports around 40 payment gateways. One of these
...
from scratch you should handle such numbers.
...
The gateway-specific fields are stored in the GatewayOptions table, with 
...
INFO. Contributions of new gateways are always welcome. Please advertise 
new code or developments on the Spree mailing list (or the Active Merchant 
list). 
...
in development mode, to using a test mode 
...
be turned off in production use.
