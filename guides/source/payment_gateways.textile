...
|Environment|The Rails environment for which this gateway will be used.  Typical choices are: +DEVELOPMENT+, +TEST+ and +PRODUCTION+|
...
Gateway configurations can also be temporarily deactivated.  This would allow you to configure a production environment gateway with all of the live settings and then disable it until your ready to launch.  Only one gateway can be active per environment.   
...
INFO. It is important that you use the exact spelling of the Active Merchant gateway option value that are trying to configure.
...
INFO. Recurring payments are not yet supported in Spree although they are plans to provide this in the near future.
