...
The Spree command line tool helps you quickly add Spree to your
exisiting rails application.
...
INFO: For security reasons, the +CheckoutController+ will not update the +Order+ once the checkout process is complete.  It is therefore impossible for an order to be tampered with (ex. changing the quantity) after checkout.
