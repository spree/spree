...
I believe (can someone confirm?)  you will want to ensure the installed spree
gem is up-to-date.  For example, if you are using Spree edge, then get the 
latest edge source, and run +rake spree:gem:install+.

Run this command from inside RAILS_ROOT (your application directory) and your 
application will be upgraded.

The update process is also now less “destructive” than in previous versions of Spree.
