...
To use this framework, you should over-ride and extend +lib/product_filters.rb+ and define a suitable
...
The upgrade process is also now less "destructive" than in previous versions of Spree.  Instead of silently replacing crucial files in your application, Spree now checks the content of files it needs to replace, and if the old version differs, it will be saved with a +~+ suffix.  

This makes it easier to see when and how some file has changed - which is often useful if you need to update a customized version. The upgrade command will also no longer copy the +routes.rb+ file - the original version just loads the core Spree routes file, so has no need to change. (Recall that you can define new routes in your extensions.)
