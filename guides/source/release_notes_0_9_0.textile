...
Upgrade now checks the content of files it needs to replace, and if the old version differs, it will be saved with a +~+ suffix. This makes it easier to see when and how some file has changed - which is often useful
if you need to update a customized version. Upgrade will no longer copy the +routes.rb+ file - the original version just loads the core routes file, so has no need to change. (Recall that you can define new routes in your extensions.)
