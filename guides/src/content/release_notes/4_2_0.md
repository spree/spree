...
**Spree 4.2** is a collaborative work of **34 amazing individuals** from around the globe who contributed **over 1300 commits**  to **over 1400 fies** in the code repository since the last release.
...
Spree 4.2 can be run on Ruby 2.5, 2.6, 2.7 and recently released 3.0. Spree 4.2 supports **both Rails 6.0 and 6.1** out of the box, so you don't need to upgrade to Rails 6.1 straight away, you can firstly move to Spree 4.2 and later perform the Rails upgrade minimizing the risk of any issues. Please see [Rails 6.1 announcement](https://weblog.rubyonrails.org/2020/12/9/Rails-6-1-0-release/) to check out new amazing features that are packed in that release.
...
* `Checkout Payment Methods` - which Payment Methods should be available in each Store [Spark Solutions](https://github.com/spree/spree/pull/10457)
...
* Admin users able to set which Option Types should be shown as filter option in Storefront [Spark Solutions](https://github.com/spree/spree/pull/10536)
