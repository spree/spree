...
module Spree
  module Resources
    module Helpers
      ...
      def issue(num)
        "<small><a href='https://github.com/spree/spree/issues/#{num}'>##{num}</a></small>"
      end
    ...
    IMAGE ||=
      { "id" => 1,
        ...
        "large_url" => "/spree/products/1/large/file.png?1370533476" }.freeze
    ...
    NEW_ORDER_WITH_LINE_ITEMS ||= NEW_ORDER.merge("line_items" => [LINE_ITEM])
    ...
    temp = SHIPMENT.merge("tracking" => "UPS1234566",
    ...
    ORDER_SHOW ||= ORDER.merge("line_items" => [LINE_ITEM],
                               "payments" => [],
                               "adjustments" => [])

    ORDER_SHOW2 ||= ORDER.merge("line_items" => [LINE_ITEM2],
                                "payments" => [PAYMENT],
                                "shipments" => [SHIPMENT],
    ...
    ORDER_SHOW_ADDRESS_STATE ||= ORDER.merge("state" => "address",
                                             "line_items" => [LINE_ITEM])

    ORDER_SHOW_DELIVERY_STATE ||= ORDER.merge("shipments" => [SHIPMENT],
                                              "state" => "delivery")

    ORDER_SHOW_PAYMENT_STATE ||= ORDER.merge("payment_methods" => [PAYMENT_METHOD],
                                             "state" => "payment")

    ORDER_SHOW_CONFIRM_STATE ||= ORDER.merge("state" => "confirm")

    ORDER_SHOW_COMPLETE_STATE ||= ORDER.merge("state" => "complete")
    ...
    COUNTRY_STATE ||= { "state" => ADDRESS_STATE }.freeze
