...
Ta sekcja jest backendowym handoffem dla zadania dopasowania `KakaowySklepikFront/lib/spree` do realnego Store API v3. W tym repozytorium nie ma katalogu frontendu ani plików `lib/spree`, a próba pobrania `pawelekbyra/KakaowySklepikFront` z GitHuba w środowisku agenta zakończyła się błędem `CONNECT tunnel failed, response 403`. Z tego powodu nie zmieniano kodu adaptera w tej gałęzi backendu.

### Co należy zmienić w `lib/spree`

- bazową ścieżkę produktowego Store API z `/api/v2/storefront` na `/api/v3/store`,
- listę produktów na `GET /api/v3/store/products`,
- szczegół produktu na `GET /api/v3/store/products/{id-or-slug}`; backend potwierdza slug oraz prefiksowane ID `prod_`,
- parametr relacji z `include=...` na `expand=default_variant,variants,media,primary_media,option_types`, z zastrzeżeniem, że relacje muszą zostać potwierdzone realnym requestem na uruchomionym backendzie,
- nagłówek publishable key z `X-Spree-Storefront-Token` na `X-Spree-Api-Key`, przy zachowaniu nazwy env `SPREE_PUBLISHABLE_KEY` po stronie frontendu,
- search po nazwie z `filter[name]` na `q[name_cont]`,
- sortowanie defensywnie do potwierdzonych wartości `price`, `-price`, `name`, `-name`, `available_on`, `-available_on`; `updated_at` nie powinien być wysyłany jako bezpieczny sort,
- mapowanie produktów z JSON:API v2 `data.attributes.relationships` na płaski format API v3,
- mapowanie obrazów z `images` na `media`, `primary_media` i `thumbnail_url`,
- mapowanie cen z obiektu `price`, szczególnie `amount`, `amount_in_cents`, `display_amount` i `currency`,
- mapowanie wariantów tak, żeby `ProductVariant.id` było prefiksowanym ID wariantu z API v3.

### Założenia v2 do usunięcia

Adapter frontendu nie powinien już zakładać:

- endpointów `/api/v2/storefront/products`,
- parametru `include`,
- relacji `images`,
- nagłówka `X-Spree-Storefront-Token`,
- filtra `filter[name]`,
- sortowania po `updated_at`,
- JSON:API v2 jako głównego kształtu odpowiedzi produktowej,
- top-level `currency` albo stringowego `price` na produkcie,
- stałej zgodności hosta mediów z `SPREE_API_URL`.

### Co nadal wymaga uruchomionego backendu

- realny request listy produktów z `expand=default_variant,variants,media,primary_media,option_types`,
- realny request szczegółu po slugu i po prefiksowanym `prod_...`,
- potwierdzenie, czy `primary_media` jest obsługiwane jako wartość `expand` w tej samej formie co `media`,
- potwierdzenie pełnego kształtu `option_values` dla strukturalnego `selectedOptions`,
- potwierdzenie rzeczywistego hosta URL-i mediów generowanych przez `cdn_image_url`,
- potwierdzenie, czy odpowiedzi produktów zawierają wystarczające dane wymiarów obrazów; aktualnie serializer mediów ich nie potwierdza.

### Czego nie ruszano

- nie zmieniano backendowego Store API,
- nie zmieniano core Spree,
- nie implementowano koszyka ani checkoutu,
- nie zmieniano płatności, zamówień ani line itemów,
- nie dodawano frontendowych hostów obrazów bez realnych URL-i z działającego środowiska,
- nie tworzono sztucznego `lib/spree` w repo backendu.

Najlepszy następny krok to wykonać te korekty w repo `pawelekbyra/KakaowySklepikFront`, a następnie uruchomić `pnpm test`, opcjonalnie `pnpm build`, oraz realne requesty do działającego backendu `sklepik` dla listy i szczegółu produktu.
