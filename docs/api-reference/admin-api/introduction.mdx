...
| **Permissions** | API key scopes (API key authentication) or the staff member's role permissions — the same `read_*`/`write_*` keys | Customer can only read/modify their own data |
...
```bash
spree api get /orders -q status_eq=placed --limit 25
```
