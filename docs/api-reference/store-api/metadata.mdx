...
<Note>
Metadata is different from [custom fields](/developer/core-concepts/metafields). Metadata is simple JSON storage (untyped, no schema), while custom fields are structured, typed, and schema-defined. Use metadata for integration data and internal tracking. Use custom fields for customer-facing custom attributes.
</Note>
...
Spree has two permanent, complementary systems for custom data — **metadata for machines, custom fields for humans**.
...
Both systems are here to stay. For more details on custom fields, see the [Custom Fields guide](/developer/core-concepts/metafields).
