...
Send monetary values back as strings too, so a value reads and writes in the same type:

```json
{ "prices": [{ "currency": "USD", "amount": "29.99", "compare_at_amount": "39.99" }] }
```

Strings are also the only way to express a localized amount — Spree parses the active locale's separators (e.g. `"1.299,00"` in a `de` context), which a JSON number literal cannot represent.
