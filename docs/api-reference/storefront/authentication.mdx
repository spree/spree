...
[Cart](/api-reference/storefront/cart/create-a-cart) and [Checkout](/api-reference/storefront/checkout/update-checkout) endpoints paths also allow interactions without the bearer token to allow creating and managing guest checkouts.
...
For users who have an account in your store, you will need to generate oAuth tokens to authenticate requests to endpoints such as [Account](/api-reference/storefront/account/retrieve-an-account), [Cart](/api-reference/storefront/cart/create-a-cart) and [Checkout](/api-reference/storefront/checkout/update-checkout).
