...
Modern payment gateways use a two-phase approach: first a **Payment Session** is created with the gateway, then after the customer completes payment on the frontend, Spree creates a **Payment** record to track the result. This ensures full PCI compliance, 3D Secure support, and compatibility with all payment method types.
...
```typescript
const updated = await client.orders.paymentSessions.update(
  cart.id, session.id,
  { amount: '149.99' },
  options
)
