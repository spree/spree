...
```bash cURL
# Discover the rule kinds this marketplace has
curl 'https://api.mystore.com/api/v3/admin/commission_rates/rule_types' \
  -H 'X-Spree-API-Key: sk_xxx'

# Attach rules to a rate
curl -X PATCH 'https://api.mystore.com/api/v3/admin/commission_rates/crate_xxx' \
  -H 'X-Spree-API-Key: sk_xxx' \
  -H 'Content-Type: application/json' \
  -d '{
    "rules": [
...
Commission lines say what the marketplace charged. They don't move money, and they don't say what a seller is owed.

That is the [payout ledger's](/developer/core-concepts/sellers#payouts) job: when an order is fulfilled, a seller transfer is written for the sale *less these lines*, and payouts settle those transfers on the seller's schedule — by the built-in provider, which keeps the books and leaves the operator to pay by bank, or by Stripe Connect, which performs the transfer. A commission line is an input to that ledger, never a row in it.

<Note>
Refund clawbacks and netting across settlements, reconciliation and seller tax reporting are Spree Enterprise.
</Note>

## Related

- [Sellers](/developer/core-concepts/sellers) — marketplaces, order splitting, and the payout ledger commission feeds
