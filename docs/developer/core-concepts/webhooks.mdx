...
Use `constructEvent` from `@spree/sdk/webhooks`. It verifies the signature **and** returns the parsed, fully-typed event in one call — because the verified event is the only thing it returns, you can't accidentally act on an unverified payload. It throws `WebhookVerificationError` on a bad, missing, or stale signature, or invalid JSON. Pass the **raw** request body (parsing then re-stringifying changes the bytes and breaks the signature) and the request headers (a plain object or a `Headers` instance).

Narrowing on `event.name` types `event.data` to the matching Store API resource — `order.completed` gives you an `Order`, `product.updated` a `Product`:
...
If you only need the boolean (e.g. custom routing), the lower-level `verifyWebhookSignature(body, signature, timestamp, secret)` is also exported.
