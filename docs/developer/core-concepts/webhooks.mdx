...
Each webhook delivery sends a JSON payload with the following structure. The `data` object uses the same [Store API V3 serializers](/api-reference/store-api/introduction) as the REST API, so webhook payloads and API responses share the same schema:
...
| `data` | Serialized resource data (V3 API format with [prefixed IDs](/api-reference/store-api/introduction)) |
