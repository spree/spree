...
The Store is the top-level tenant in Spree. Every resource — products, orders, channels, markets, categories — belongs to exactly one store. A store owns its [channels](/developer/core-concepts/channels) (online, POS, wholesale, …), its [markets](/developer/core-concepts/markets) (region/currency/locale), and its [product catalog](/developer/core-concepts/products).
...
| [**Shipping Methods**](/developer/core-concepts/fulfillments) | A shipping method belongs to one store |
...
`db:seed` gives every store its starting data: tax categories, the Wholesale channel and customer group, the admin role, digital delivery, store credit, product types, return and cancellation reasons, the default commission rate, seller requirements, API keys, saved reports and allowed origins. A platform that creates stores as merchants sign up can seed only the new store, so the work does not grow with the number of stores already in the install:

```ruby server/app/services/create_merchant_store.rb
Spree::Seeds::StoreResources.call(store: store)

Spree::Stores::ProvisionDefaults.call(store: store, country: country, locale: locale, currency: currency)
```

It is safe to run again on the same store. Run it before `Spree::Stores::ProvisionDefaults`, which builds the market, warehouse, delivery zones and pickup from the merchant's country and currency.
