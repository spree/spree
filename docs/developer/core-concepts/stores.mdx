---
title: Stores
description: The Store is Spree's top-level multi-tenant boundary — learn how products, orders, channels, markets, and admin users are scoped to each store.
---
...
- **Multiple stores in one app** — the legacy multi-store pattern, where each store is fully independent (catalog, admin, branding). In Spree 5.5+ this requires the [spree_multi_store](/developer/multi-store/quickstart) extension. Core Spree treats each product as belonging to a single store.

For most multi-brand operations, the channel pattern is simpler and more flexible. Reach for [spree_multi_store](/developer/multi-store/quickstart) only when stores need genuinely independent catalogs, branding, and admin teams.
