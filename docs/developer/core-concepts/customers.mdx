...
erDiagram
    ...
    Customer ||--o| Address : "default ship/bill"
    Customer ||--o{ Wishlist : "has many"
    Customer ||--o{ StoreCredit : "has many"
    Customer ||--o{ GiftCard : "holds"
    Customer ||--o{ CompanyMembership : "buys for"
    CompanyMembership }o--|| Company : "belongs to"
    Customer }o--o{ CustomerGroup : "segmented by"
...
<Note>
The Store API flow above is **customer self-registration** — the customer sets a password and gets a JWT back. To [create a customer record from the back office](/developer/sdk/admin/resources) (importers, CRM sync, phone orders), use the [Admin SDK](/developer/sdk/admin/quickstart) instead. No password is set, so it's a managed record rather than a sign-up:
...
Refresh an expiring token to keep the session alive. For how `client.auth.login` and `client.auth.refresh` fit into configuring the SDK for guest vs customer (JWT) auth, see the [SDK authentication guide](/developer/sdk/authentication):
...
- [Account (Store SDK)](/developer/sdk/store/account) — Task-oriented walkthrough of registration, login, profile, addresses, and order history
...
- [Authentication](/developer/providers/sso) — Custom authentication setup
