...
| `preferred_order_routing_strategy` | Optional per-channel override of the store's [Order Routing](/developer/core-concepts/fulfillments#order-routing) strategy | `Spree::OrderRouting::Strategy::Rules` |
...
This attribution drives reporting (best-selling by channel, revenue per channel) and per-channel order routing — see [Order Routing](/developer/core-concepts/fulfillments#order-routing).

### Storefront Access Gating

A channel's `storefront_access` decides what a visitor may see. The first three postures gate only **anonymous** visitors — a request with no authenticated customer; the fourth also gates signed-in customers until they belong to a company:
...
| `approval_required` | Yes | No — for guests **and** customers without an active [company](/developer/core-concepts/companies) | A trade store with a visible range: anyone browses, but prices and checkout need a company account. |
...
- **`approval_required`** — reads succeed for everyone, but money fields are `null` unless the customer has standing over an active company, and completing checkout is refused (requirement code `company_activation_required`) until the cart resolves to one. See [company self-registration](/developer/core-concepts/companies#self-registration-and-activation) for the registration flow and what "active" means.

Whenever prices are withheld, every payload that nulls them also carries a **`pricing_access`** reason code — `login_required` (sign in to see prices) or `company_required` (register or join a company) — so a storefront renders the right prompt from the code instead of guessing at bare nulls. The field is `null` when prices are visible.
...
- [Order Routing](/developer/core-concepts/fulfillments#order-routing) — Channels can override the store's routing strategy
