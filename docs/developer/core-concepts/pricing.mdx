...
Each variant has one or more `Price` records — one per currency. The API automatically returns the correct price based on the current currency and [Market](/developer/core-concepts/markets) context. See [monetary amounts](/api-reference/store-api/monetary-amounts) for how money fields (decimal, cents, and display string) are formatted across the API.
...
Base prices are set per variant per currency. Use the [Admin API](/api-reference/admin-api/introduction) — via the [Admin SDK](/developer/sdk/admin/quickstart) — to set them individually or in bulk (`bulk_upsert` matches on variant + currency):
...
`?expand=variants.prices` returns every price a variant has, Price List amounts
and quantity breaks included; each row carries its `price_list_id`, which is
`null` for a base price. Writing `prices` back on a variant sets **base prices
only** — a currency left out is removed — so filter the rows you read before
sending them back. To write what a Price List charges, use `/prices`.
...
| **Channel Rule** | Matches based on the [Channel](/developer/core-concepts/channels) the customer is buying through | App-only or in-store pricing |
| **Volume Rule** | Matches based on quantity purchased | Bulk discounts, tiered pricing |

*Who* gets a price list is not a rule — it's a [Catalog](/developer/core-concepts/catalogs): attach the list to a catalog assigned to the customer groups or companies it's for.

<Note>
The **User Rule** and **Customer Group Rule** kinds still work and are not going away, but they're no longer offered when adding a rule — targeting an audience through a catalog is the one way to do it now. Nothing needs migrating; rules you already have keep matching exactly as before.
</Note>
...
| Channel | The API credential in use | The [Channel](/developer/core-concepts/channels) for channel-based rules |
| Customer | Authentication | The signed-in customer, for resolving their [Catalogs](/developer/core-concepts/catalogs) |
| Quantity | Cart line item | The quantity for volume-based rules |
| Date | Current time | For time-based Price List scheduling |

The Store API automatically builds this context from the [request headers](/api-reference/store-api/localization) (`X-Spree-Currency`, `X-Spree-Country`) and authentication state. You don't need to construct it manually — just make API requests and the correct price is resolved.
...
- [Store SDK: Products](/developer/sdk/store/products) — Fetching product and variant prices with the Store SDK
