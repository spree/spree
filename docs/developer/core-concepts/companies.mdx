...
```bash cURL
curl -X POST 'https://api.mystore.com/api/v3/store/companies/comp_xxx/members' \
  -H 'X-Spree-API-Key: pk_xxx' \
  -H 'Authorization: Bearer $CUSTOMER_JWT' \
  -H 'Content-Type: application/json' \
  -d '{ "customer_email": "buyer@acme.com" }'
```
...
```bash cURL
# Reading an invitation needs no sign-in — just the emailed token
curl 'https://api.mystore.com/api/v3/store/company_invitations/TOKEN' \
  -H 'X-Spree-API-Key: pk_xxx'

curl -X POST 'https://api.mystore.com/api/v3/store/company_invitations/TOKEN/accept' \
  -H 'X-Spree-API-Key: pk_xxx'
```
...
```bash cURL
curl -X POST 'https://api.mystore.com/api/v3/store/companies/comp_xxx/addresses' \
  -H 'X-Spree-API-Key: pk_xxx' \
  -H 'Authorization: Bearer $CUSTOMER_JWT' \
  -H 'Content-Type: application/json' \
  -d '{ "label": "Northern Warehouse", "first_name": "Goods", "last_name": "Inwards" }'
```
...
```bash cURL
# Roots only
curl 'https://api.mystore.com/api/v3/admin/companies?q[parent_id_null]=1' \
  -H 'X-Spree-API-Key: sk_xxx'
```
