...
The following are descriptions of the currently available calculators in Spree. If you would like to add your own, please see the [Creating a New Calculator](calculators#creating-a-new-calculator) section.
...
The Percent Per Item calculator works in a near-identical fashion to the [Per Item Calculator](calculators#per-item), with the exception that rather than providing a flat-rate per item, it is a percentage.
