...
erDiagram
    ...
    User {
        string email
        integer ship_address_id
        integer bill_address_id
    }

    Address }o--|| Country : "belongs to"
    Address }o--o| State : "belongs to"
    Address }o--o| User : "belongs to"
    User ||--o{ Address : "has many"
    User }o--o| Address : "default ship"
    User }o--o| Address : "default bill"
    Order }o--|| Address : "ship_address"
    Order }o--|| Address : "bill_address"
    Country ||--o{ State : "has many"
    Zone ||--o{ ZoneMember : "has many"
    ZoneMember }o--o| Country : "member"
    ZoneMember }o--o| State : "member"
```

**Key relationships:**
- **Address** stores shipping/billing information for [Orders](/developer/core-concepts/orders) and Users
- **Country** and **State** provide geographic data for addresses
- **Zone** groups countries or states for [tax](/developer/core-concepts/taxes) and [shipping](/developer/core-concepts/shipments) rules
- **Zone Members** link zones to their geographic components
...
Users can have multiple addresses and can manage them on the Checkout and Account page (or via API). Each `Spree::Address` object has a `user_id` column which links to the User.

Additionally in `Spree.user_class` there are `ship_address_id` and `bill_address_id` columns which indicate the default addresses for this User.
...
Every order has a "tax zone", which indicates if a user should or shouldn't be taxed when placing an order. For more information, please see the [Taxes](taxes) guide.
...
- [Users](/developer/core-concepts/users) - User accounts and address books
- [Taxes](/developer/core-concepts/taxes) - How addresses affect taxation
- [Shipments](/developer/core-concepts/shipments) - How addresses affect shipping
