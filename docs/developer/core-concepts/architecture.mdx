...
3. **Checkout** — The Order collects [Addresses](/developer/core-concepts/addresses), calculates [Shipping](/developer/core-concepts/fulfillments) options, and processes [Payments](/developer/core-concepts/payments)

4. **Fulfillment** — [Shipments](/developer/core-concepts/fulfillments) are created from Stock Locations, tracking individual [Inventory Units](/developer/core-concepts/inventory#inventory-units)

5. **Pricing & Adjustments** — [Taxes](/developer/core-concepts/taxes) and [Promotions](/developer/core-concepts/promotions) create [Adjustments](/developer/core-concepts/taxes-discounts-fees) that modify order totals
...
Both APIs share the same list shapes, filtering, querying, and pagination, and offer fully typed TypeScript clients — [@spree/sdk](/developer/sdk/quickstart) for the Store API and [@spree/admin-sdk](/developer/sdk/admin/quickstart) for the Admin API.

Events from both APIs can trigger [Webhooks](/developer/core-concepts/webhooks) to notify external systems in real time.

## Multi-Store Architecture

Spree supports running [multiple storefronts](/developer/multi-tenant/quickstart) from a single installation. Each Store can have:
...
| **Decorators** | Modify existing core models & classes behavior (use sparingly) | [Decorators Guide](/developer/customization/decorators) |
...
| Package | Purpose |
|---------|---------|
| [`@spree/sdk`](/developer/sdk/quickstart) | TypeScript SDK for the Store API |
| [`@spree/admin-sdk`](/developer/sdk/admin/quickstart) | TypeScript SDK for the Admin API |
| [`@spree/cli`](/developer/cli/quickstart) | Command line interface for managing local environment and using the Admin API from the terminal |
...
- [Shipments](/developer/core-concepts/fulfillments) — Shipping and fulfillment
