...
Store credits carry no category and no type — both were retired in Spree 6.0
because neither answered a question anything asked. Two fields cover it
instead:
...
<Note>
`Spree::StoreCreditCategory` and `Spree::StoreCreditType` survive as deprecated
shells for one release and are removed in 6.1. Nothing writes them.
</Note>
...
- [Admin SDK Quickstart](/developer/sdk/admin/quickstart) — Set up and authenticate the Admin SDK
