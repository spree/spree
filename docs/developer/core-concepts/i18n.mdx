...
[Spree extensions](/developer/extending-spree/3rd-party-extensions) can contain their own `config/locales` directory where developers can include YAML files for each language they wish to support.
