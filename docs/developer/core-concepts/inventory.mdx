...
// Eight of the ten made it. The transfer stays open for the rest.
await adminClient.stockTransfers.receive(transfer.id, {
  items: [{ id: 'sti_xxx', quantity_received: 8, discrepancy_reason: 'damaged_in_transit' }],
})
```
...
await adminClient.purchaseOrders.receive(order.id)
```

Ordered stock is never available stock. Nothing reaches `count_on_hand` until a delivery is received, because a merchant who has ordered goods does not have them — and suppliers under-ship often enough that receiving in parts is the ordinary case.
