...
When a store has more than one location, [order routing](/developer/how-to/custom-order-routing) decides which one ships each item — at checkout and for orders built by staff alike. By default it prefers the location chosen for the order (a customer's pickup counter, or one staff picked), then the location that can cover the most of the order on its own, then the `default` one.
