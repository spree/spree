...
Stock Reservations are a time-limited soft hold on stock during checkout. When a customer enters checkout, Spree creates a reservation against each line item's stock so other shoppers see reduced availability immediately. Two customers can no longer both pass the availability check on the same last unit and only fail at order completion.

### What changes for the storefront

Availability now subtracts active reservations:

```
purchasable = physical stock − units already allocated to shipments − active reservations
```

When you ask whether a variant is in stock — whether through the Storefront API, the v2 platform API, or directly via the variant model — you get back the post-reservation number automatically. There's nothing for the storefront to compute. Physical stock counts on each `StockItem` are never modified by reservations; reservations are an independent layer that's consulted at read time and cleaned up by background jobs.
...
When a cart mutation in checkout would push the order beyond available stock, the operation is rejected up front. The Storefront API surfaces this as a validation error so the storefront can show the customer immediately, instead of letting them progress through payment only to fail at the final submit.

### Background expiry

Abandoned checkouts leave behind expired reservation rows. Spree provides a job to clean them up but does **not** auto-schedule it — your application's job runner needs to invoke it periodically (every minute is typical). See the [5.4 to 5.5 upgrade guide](/developer/upgrades/5.4-to-5.5#schedule-the-stock-reservations-expiry-job) for sidekiq-cron, solid_queue, and good_job snippets.
...
### Looking ahead to 6.0

The availability formula above mentions "units already allocated to shipments" — in Spree 5.5 this number is always zero, so availability collapses to `physical stock − active reservations`. Spree 6.0 will introduce typed stock movements that track allocation explicitly (units committed to a shipment but not yet shipped). When that lands, the formula becomes meaningful without any change to the API contract or storefront integration — the existing fields keep their names and meanings.
