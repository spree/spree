...
Each movement points at what caused it — the order, the fulfillment, the transfer, the purchase order — so an audit trail reads as a sequence of business events rather than a list of numbers that changed.
...
const transfer = await adminClient.stockTransfers.create({
  ...
  items: [{ variant_id: 'var_xxx', quantity_shipped: 10 }],
})
...
// Eight of the ten made it intact and two were crushed. Each delivery is a
// stock receipt: this delivery's counts, not a running total.
...
Every delivery is a **stock receipt** — numbered, dated, carrying the packing-slip reference — with what it accepted and what it refused per line. Refused units are recorded but never stocked. A transfer is `received` once everything that left has arrived, in whatever state; `over_received` if more arrived than was sent; and it can be **closed short** when the missing units are not going to turn up:

```typescript Admin SDK
await adminClient.stockTransfers.close(transfer.id, { reason: 'Two fell off the van' })
```

A transfer that is packed but not yet sent can go back to draft (`markDraft`) if the plan changes.
...
const order = await adminClient.purchaseOrders.create({
  ...
  items: [{ variant_id: 'var_xxx', quantity_ordered: 100, unit_cost: '12.50' }],
})
...
Ordered stock is never available stock. Nothing reaches `count_on_hand` until a delivery is received, because a merchant who has ordered goods does not have them — and suppliers under-ship often enough that receiving in parts is the ordinary case. Unlike a transfer, a supplier's refused units are still owed: the order stays `partially_received` until they are replaced, or the merchant closes it short (`close`, with a reason) because the balance is not coming. A supplier that sends more than was ordered leaves the order `over_received`. An order that was placed but has had no delivery can go back to draft (`markDraft`).

Orders carry an optional `cancel_by` date — the day after which the goods are no longer wanted — and the dashboard lists filter on *Overdue* (expected date passed) and *Past cancel-by*. Nothing cancels automatically. Purchase orders can be exported to CSV, one row per line, and imported from one: rows sharing a `reference` become one draft order, naming the supplier, warehouse and SKU by name.
