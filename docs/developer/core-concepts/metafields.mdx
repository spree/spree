...
```mermaid
erDiagram
    spree_metafield_definitions ||--o{ spree_metafields : "defines schema for"
    spree_products ||--o{ spree_metafields : "has many"
    spree_variants ||--o{ spree_metafields : "has many"
    spree_orders ||--o{ spree_metafields : "has many"
    ...
    spree_metafields {
        bigint id PK
        bigint metafield_definition_id FK
        string type
        text value
        string resource_type
        bigint resource_id
...
<Note>
The `display_on` attribute is intentionally excluded from Store API responses for security reasons.
</Note>
...
| **Recommended For** | New projects | Integration data, tracking | Legacy projects only |
| **Structure** | Strongly typed, schema-defined | Stripe-style flat key-value | Product-specific properties |
| **Validation** | Type-specific validation | None | Name/value pairs |
| **Visibility** | Configurable (front/back/both) | Write-only in Store API, readable in Admin API | Always public |
| **API Access** | Auto-included in API | Writable on cart, line items, orders | Auto-included in API |
| **Admin UI** | Dedicated management UI | Viewable in JSON preview | Dedicated UI |
...
- Storing external system IDs (Stripe, ERP, CRM)
- Tracking attribution data (UTM parameters, referral source)
- Write-and-forget data for backend systems
- Integration payloads that don't need validation or admin UI
