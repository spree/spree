...
| Record | Statuses |
|---|---|
| **Return** | `requested` → `approved` → `received` → `partially_refunded` → `refunded`, or `canceled` |
...
You can refund a return in more than one step — for example, part to store credit and the rest to the card, or some now and the rest after inspection. A refund for less than the return is still owed leaves it `partially_refunded`, and it becomes `refunded` once nothing is left owing. No refund can go above what is still owed. Each refund publishes `return.refunded`.
