...
```bash cURL
# What profiles the store has
curl 'https://api.mystore.com/api/v3/admin/delivery_profiles' \
...
```bash cURL
curl -X POST 'https://api.mystore.com/api/v3/admin/delivery_zones' \
  -H 'X-Spree-API-Key: sk_xxx' \
  -H 'Content-Type: application/json' \
  -d '{
    "name": "London same-day",
...
leaves on. How those cartons and pallets roll up into a quote is
[freight](/developer/core-concepts/freight).
...
`dimensions_unit` and `weight_unit` are optional too. A row that names neither
is read in whatever the store's own units are, so a store trading in inches and
pounds reads inches and pounds. A row that names them keeps them, which is how
one store can mix units — a carton measured in centimetres sits beside a pallet
measured in feet. Variant dimensions work the same way.
...
```bash cURL
# The seller's own packaging
curl 'https://marketplace.example.com/api/v3/seller/package_types?owner=mine' \
  -H 'Authorization: Bearer $SELLER_JWT' \
  -H 'X-Spree-Seller-Id: sel_xxx'

# Theirs plus the marketplace's shared rows
curl 'https://marketplace.example.com/api/v3/seller/package_types' \
  -H 'Authorization: Bearer $SELLER_JWT' \
  -H 'X-Spree-Seller-Id: sel_xxx'
```
...
- [Freight](/developer/core-concepts/freight) — cartons, pallets and wholesale shipping
