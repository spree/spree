...
Orders flow through a state machine, beginning at a `cart` state and ending up at a `complete` state. The intermediary states can be configured using the [Checkout Flow API](../customization/checkout#the-checkout-flow-dsl).
...
For more on this please go to [Shipment States page](shipments#overview).
...
For more on this please go to [Payment States page](payments#overview).
