...
const taxonFilters = await client.products.filters({
  taxon_id_eq: 'txn_xxx',
})
```
...
```typescript SDK
// List taxonomies
const { data: taxonomies } = await client.taxonomies.list()

// List taxons (categories)
const { data: taxons } = await client.taxons.list()

// Get a taxon by permalink
const taxon = await client.taxons.get('categories/clothing')

// List products in a taxon
const { data: products } = await client.taxons.products.list('categories/clothing', {
  limit: 12,
})
```
