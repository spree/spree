...
const detailed = await client.store.products.get('spree-tote', {
  expand: ['variants', 'images', 'option_types', 'taxons'],
})
...
```bash cURL
curl 'https://api.mystore.com/api/v3/store/products/spree-tote?expand=variants,images,option_types,taxons' \
  -H 'Authorization: Bearer spree_pk_xxx'
```
...
Every product response includes a `thumbnail_url` field — the URL to the first image, ready to use without any expands. Similarly, each variant includes a `thumbnail` URL and an `image_count` counter.
...
<Warning>
Avoid using `?expand=images` on listing pages. This loads **all** images for every product in the response, which is unnecessary when you only need a thumbnail. Use `thumbnail_url` instead and only expand full images on the product detail page.
</Warning>

### All Images

On the product detail page, expand `images` and `variants` to get the full set of images. Images are ordered by `position`:
...
product.variants?.forEach(variant => {
  ...
  variant.images       // full image array (only when ?expand=images)
})
```
...
# List products in a taxon
curl 'https://api.mystore.com/api/v3/store/taxons/categories/clothing/products?limit=12' \
  -H 'Authorization: Bearer spree_pk_xxx'
```
