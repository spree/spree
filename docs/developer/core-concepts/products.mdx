...
const detailed = await client.products.get('spree-tote', {
  expand: ['variants', 'images', 'option_types', 'categories'],
})
...
```bash cURL
curl 'https://api.mystore.com/api/v3/store/products/spree-tote?expand=variants,images,option_types,categories' \
  -H 'Authorization: Bearer spree_pk_xxx'
```
