...
<Warning>
  **You have to schedule these yourself** — a new project does not ship them, so
  a marketplace that skips this step never pays a seller. Add both to your
  scheduler; with Solid Queue that is `server/config/recurring.yml`:
