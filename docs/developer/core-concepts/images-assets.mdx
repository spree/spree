...
Product images are included in Store API responses when you pass `includes=images`:

```typescript
const product = await client.store.products.get('red-t-shirt', {
  includes: ['images'],
})
```

Each image in the response contains the original URL and pre-generated URLs for each named variant size:
