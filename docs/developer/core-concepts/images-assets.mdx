...
Every product response includes a `thumbnail_url` field — ready to use without any includes. Similarly, each variant includes a `thumbnail` URL and an `image_count` counter.
...
products.forEach(product => {
  product.thumbnail_url // "https://cdn.../tote-front.webp" — no includes needed
})
```
...
<Warning>
Avoid using `?includes=images` on listing pages. This loads **all** images for every product in the response. Use `thumbnail_url` instead and only include full images on product detail pages.
</Warning>

### Full Images (On Demand)

On the product detail page, include `images` and `variants` to get the full set of images with all named variant URLs:
...
product.variants?.forEach(variant => {
  ...
  variant.images       // full image array (only with ?includes=images)
})
```
...
- **Use `thumbnail_url`** on listing pages — avoid loading full images via includes
