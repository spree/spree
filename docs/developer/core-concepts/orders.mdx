...
// On a split checkout, give back this order's share of the shared payment —
// all of it, or the part named by refund_amount
await adminClient.orders.cancel('or_xxx', {
  cancel_reason_id: reason.id,
...
curl -X PATCH 'https://api.mystore.com/api/v3/admin/orders/or_xxx/cancel' \
  ...
  -d '{ "cancel_reason_id": "ocr_xxx", "refund_payments": true, "refund_amount": "25.00" }'
```

</CodeGroup>

Canceling always puts stock back — the goods are not going out either way. Money
is settled at the gateway: an uncaptured authorization is released, and a
captured payment is refunded in full.

`refund_payments` and `refund_amount` matter on an order paid through a split
checkout, where one payment is shared across several orders. There the amount is
this order's own share, drawn from each payment in turn until it is met, so a
partial refund never has to name a payment. Leave `refund_amount` out to give
back the whole share, or set it to keep part back — a restocking fee, say.
Without `refund_payments`, a shared authorization is written down but nothing is
handed back.
...
Tax, discounts and fees are kept as separate records, so you can ask what tax was charged without picking through a mixed list. See [Order totals](/developer/core-concepts/order-totals).
