...
// Add an item
const updatedOrder = await client.store.orders.lineItems.create(cart.id, {
  variant_id: 'var_xxx',
  quantity: 2,
})

// Update quantity
await client.store.orders.lineItems.update(cart.id, 'li_xxx', {
  quantity: 3,
})

// Remove an item
await client.store.orders.lineItems.delete(cart.id, 'li_xxx')
```
...
// Advance to delivery
await client.store.orders.next(orderId)

// Get shipments and select a shipping rate
const { data: shipments } = await client.store.orders.shipments.list(orderId)
await client.store.orders.shipments.update(orderId, shipments[0].id, {
...
const session = await client.store.orders.paymentSessions.create(orderId, {
  payment_method_id: 'pm_xxx',
})
// session.external_data.client_secret => use with Stripe.js

// Complete the payment session after provider confirmation
await client.store.orders.paymentSessions.complete(orderId, session.id)

// Complete the order
await client.store.orders.complete(orderId)
```
...
```typescript SDK
// Apply a coupon
await client.store.orders.couponCodes.apply(orderId, 'SAVE20')

// Remove a coupon
await client.store.orders.couponCodes.remove(orderId, 'promo_xxx')
```
...
```typescript SDK
// List past orders
const { data: orders } = await client.store.customer.orders.list()

// Get a specific order with details
const order = await client.store.orders.get('or_xxx', {
  includes: 'line_items,shipments,payments',
})
```
...
curl 'https://api.mystore.com/api/v3/store/orders/or_xxx?includes=line_items,shipments,payments' \
  -H 'Authorization: Bearer <jwt_token>'
```
