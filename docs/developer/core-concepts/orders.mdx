...
is a separate decision: pass `refund_payments` to give back what the order has
been paid, optionally capped by `refund_amount`. Without it, captured payments
are canceled at the gateway and any authorization is released, but nothing is
refunded.
