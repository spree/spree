...
curl -X PATCH 'https://api.mystore.com/api/v3/admin/orders/or_xxx/cancel' \
  ...
  -d '{ "cancel_reason_id": "ocr_xxx", "cancel_note": "Supplier could not deliver in time" }'
```

</CodeGroup>

Canceling puts stock back and settles any payments. It records who canceled, and
optionally why: the reason comes from a list you manage yourself under
Settings, so it can say what your team actually means. A staff-facing note can
go alongside it.
