...
// Cancel and give the money back in the same call
await adminClient.orders.cancel('or_xxx', {
  cancel_reason_id: reason.id,
...
Canceling always puts stock back — the goods are not going out either way. An
authorization the gateway never drew on is always released.

Money that *was* taken is a separate decision, as it is on every other major
platform: pass `refund_payments: true` to return it. Without it the captured
payment is left alone for an operator to refund deliberately.
