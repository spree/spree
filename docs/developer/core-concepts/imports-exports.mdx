---
title: Bulk CSV imports and exports in Spree
description: "Import and export large catalogs in Spree using CSV files, field mapping, background job processing, and admin progress tracking for products and orders."
---
...
Exports support [Ransack filtering via `search_params`](/api-reference/admin-api/querying):
...
Import and export lifecycle events such as [`import.completed` and `export.created`](/api-reference/webhooks-events) can be delivered as webhooks to react when an asynchronous import or export finishes.
...
- [Admin imports/exports endpoints](/api-reference/admin-api/endpoints) — REST routes and required scopes to trigger imports and exports programmatically.
- [Admin SDK resources](/developer/sdk/admin/resources) — typed TypeScript client for driving these back-office operations.
