...
```bash cURL
curl 'https://api.mystore.com/api/v3/store/products?q[multi_search]=tote+bag&per_page=12' \
...
# Filter by taxon ID
curl 'https://api.mystore.com/api/v3/store/products?q[taxon_id_eq]=txn_xxx' \
  -H 'Authorization: Bearer spree_pk_xxx'
```
...
Get available filter options for building a faceted search UI. Returns option values, price ranges, and taxons with counts:
...
```bash cURL
curl 'https://api.mystore.com/api/v3/store/products?page=1&per_page=24' \
  -H 'Authorization: Bearer spree_pk_xxx'
```
