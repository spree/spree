...
- Flexible sorting options (including sortable product custom fields via `sort=cf_{namespace.length}_{namespace}_{key}`)
...
// Plus any product custom field marked sortable, e.g. cf_6_custom_label / -cf_6_custom_label
```
