...
## Extending Ransackable Configuration

If you need to add custom ransackable attributes, associations, or scopes to Spree's built-in models without using decorators, use the `Spree.ransack` configuration API.

### Adding Custom Configuration

Add the following to your initializer (e.g., `config/initializers/spree.rb`):
...
Rails.application.config.after_initialize do
  ...
  # Add multiple configurations for Orders
  Spree.ransack.add_attribute(Spree::Order, :custom_reference)
  Spree.ransack.add_scope(Spree::Order, :high_value)
  Spree.ransack.add_association(Spree::Order, :sales_rep)
end
```

<Info>
The `Spree.ransack` API only allows adding new searchable fields. It does not support removing default configurations to prevent accidentally breaking core functionality.
</Info>
...
|--------|-------------|---------|
| `add_attribute` | Add a searchable attribute | `Spree.ransack.add_attribute(Spree::Product, :vendor_id)` |
| `add_scope` | Add a searchable scope | `Spree.ransack.add_scope(Spree::Product, :featured)` |
| `add_association` | Add a searchable association | `Spree.ransack.add_association(Spree::Product, :vendor)` |

### Complete Example

Here's an example of extending Products with a vendor relationship:
...
# db/migrate/xxx_add_vendor_to_spree_products.rb
class AddVendorToSpreeProducts < ActiveRecord::Migration[7.0]
  def change
    add_reference :spree_products, :vendor
  end
end
...
Now you can search products by vendor:

```bash
# Filter by vendor_id
GET /api/v2/platform/products?filter[vendor_id_eq]=123

# Filter by vendor name (through association)
GET /api/v2/platform/products?filter[vendor_name_cont]=acme
```
...
- [Admin Tables](/developer/admin/tables) - Customize admin list views with sorting and filtering
- [Decorators](/developer/customization/decorators) - Add searchable attributes without decorators
