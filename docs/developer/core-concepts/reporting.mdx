...
- `format` is `:money`, `:integer`, `:decimal` or `:percent`. A `:money` metric
  ...
  Amounts are **never converted**. A query answers in one currency — the one
  it names, or the store's default — and every base a money metric reads must
  filter by it, or two currencies end up added together under one symbol. A
  store selling in several therefore has as many answers as it has
  currencies, which is why the home screen offers a currency alongside its
