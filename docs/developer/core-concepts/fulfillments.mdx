...
Which stock location sources each item is decided by [order routing](/developer/how-to/custom-order-routing) — the channel's routing rules, or a custom strategy. Storefront checkout and orders built by staff are routed the same way, and a placed order keeps the fulfillments its checkout proposed.
