...
const product = await client.store.products.get('prod_86Rf07xd4z')
...
```typescript SDK
// English
const product = await client.store.products.get('red-shoes')

// French (with locale header)
const product = await client.store.products.get('chaussures-rouges', {
  locale: 'fr',
})
```
