...
Spree generates SEO-friendly URLs for resources like products, taxons, stores, and more. Instead of accessing resources via database IDs (e.g., `/products/123`), clean, readable URLs based on resource names are used (e.g., `/products/spree-tshirt`).
...
Slugs are primarily used in both the Store API and Admin APIs which accept both slugs or IDs interchangeably for resource lookups

<Info>
Spree supports internationalized slugs and handles slug cleanup on soft-deletion automatically.
</Info>
...
Slugs are automatically generated from resource attributes using automatic URL conversion. This:
...
"Spree T-Shirt".to_url  # => "spree-t-shirt"
"Café & Restaurant".to_url      # => "cafe-and-restaurant"
```

### Slug Candidates

When generating a slug, Spree tries multiple "candidate" patterns to ensure uniqueness. If the first candidate is taken, it tries the next pattern, and so on.
...
# Example progression:
# 1st attempt: "spree-tshirt"
# 2nd attempt: "spree-tshirt-TSHIRT-001" (includes SKU)
# 3rd attempt: "spree-tshirt-a1b2c3d4-e5f6-..." (includes UUID)
```

### Reserved Words

Spree prevents using certain reserved words as slugs that would conflict with application routes:
...
product.slug # => "spree-t-shirt"

# Accessing by slug
Spree::Product.friendly.find('spree-t-shirt')

# Setting a custom slug
product.update!(slug: 'custom-rails-shirt')

# Translated slugs
product.set_friendly_id('t-shirt-spree', :fr)
product.save!
```
...
Other Spree models that support slugs:
...
Spree supports multi-language slugs, generating locale-specific URLs for each translation.
...
The slug history feature preserves old slugs when they change, ensuring existing URLs continue to work. This is critical for:
...
Spree maintains a separate `friendly_id_slugs` table for slug history:
...
Spree's slug system handles soft-deleted records gracefully. When a resource is deleted:
...
```ruby
# Find by slug
product = Spree::Product.friendly.find('spree-tshirt')
...
product.slug # => "spree-t-shirt" (regenerated from name)
```
...
To add slugs to your custom models, inherit from `Spree::Base` which provides the base slug functionality.
...
<Warning>
Always use `:id` as the route parameter, not `:slug`. This allows the `.friendly` finder to accept both numeric IDs and slugs, maintaining backward compatibility.
</Warning>
...
```ruby
# Loading a product by slug or ID
def load_product
...
Both the Store API and Admin API accept slugs or IDs:

```ruby
# Finding a product by slug or ID in an API controller
...
# API Examples:
# GET /api/v3/store/products/123
# GET /api/v3/store/products/spree-tshirt
```
...
- `core/config/initializers/friendly_id.rb` - Global slug configuration
- `core/lib/friendly_id/paranoia.rb` - Soft-delete integration
