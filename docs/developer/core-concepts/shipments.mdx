...
During checkout, after the customer provides a shipping address, Spree calculates available shipping rates for each shipment. The customer must select a rate before proceeding. See the [Cart & Checkout SDK](/developer/sdk/store/cart-checkout) guide for the full storefront cart and fulfillment flow.
