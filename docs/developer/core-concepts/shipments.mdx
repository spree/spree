...
const order = await client.store.orders.get(orderId, {
  includes: 'shipments',
})
...
await client.store.orders.shipments.update(orderId, shipment.id, {
...
```bash cURL
# Get shipments
curl 'https://api.mystore.com/api/v3/store/orders/or_xxx?includes=shipments' \
