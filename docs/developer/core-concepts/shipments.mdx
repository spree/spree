...
- **Inventory Units** track individual items within each shipment
...
| `stock_location_id`| The ID of the [Stock Location](/developer/core-concepts/inventory#stock-locations) where the items for this shipment will be sourced from.         | 3                            |
...
* 1 [Stock Location](/developer/core-concepts/inventory#stock-locations): You are shipping all items from the same location, so you can use the default.
...
Consider you sell products to a single zone US and you ship from 2 [Stock Locations](/developer/core-concepts/inventory#stock-locations):
...
* 2 [Stock Locations](/developer/core-concepts/inventory#stock-locations) (Configuration -> Stock Locations): New York, Los Angeles
...
The `Spree::Stock::Coordinator` is the starting point for determining shipments when calling `create_proposed_shipments` on an order. Its job is to go through each [Stock Location](/developer/core-concepts/inventory#stock-locations) available and determine what can be shipped from that location.
...
A `Spree::Stock::Packer` object is an important part of the `create_proposed_shipments` process. Its job is to determine possible packages for a given [Stock Location](/developer/core-concepts/inventory#stock-locations) and order. It uses rules defined in classes known as `Splitters` to determine what packages can be shipped from a [Stock Location](/developer/core-concepts/inventory#stock-locations).
...
If you want to add different splitters for each [Stock Location](/developer/core-concepts/inventory#stock-locations), you need to decorate the `Spree::Stock::Coordinator` class and override the `splitters` method.
...
2. Override the `prioritize_packages` method in the `Spree::Stock::Coordinator`, passing in your custom adjuster class to the `Prioritizer` initializer.

<Warning>
  Decorators should be used as a last resort as they can make upgrades difficult. See [Decorators](/developer/customization/decorators) for more information and alternatives.
</Warning>

For example, if your adjuster was called `Spree::Stock::CustomAdjuster`, you would do the following in `app/models/spree/stock/coordinator_decorator.rb`:
...
- [Events](/developer/core-concepts/events) - Subscribe to shipment events
