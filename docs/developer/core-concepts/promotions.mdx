...
const order = await client.orders.couponCodes.apply('or_abc123', 'SUMMER20', {
  orderToken: '<token>',
})

// Remove a promotion from the order
await client.orders.couponCodes.remove('or_abc123', 'promo_abc123', {
  orderToken: '<token>',
})
```
...
```bash cURL
# Apply a coupon code
curl -X POST 'https://api.mystore.com/api/v3/store/orders/or_abc123/coupon_codes' \
