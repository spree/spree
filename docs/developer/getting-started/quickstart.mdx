...
```
my-store/
├── server/                 # The Spree API — models, endpoints, jobs, specs
├── apps/
...
**`server/` is the only part that is Spree's.** You own the apps under `apps/` outright — they are ordinary Vite and Next.js projects that talk to the API over HTTP, so you can restyle, restructure or replace them without forking anything.

Each app carries its own instructions: `server/CLAUDE.md`, `apps/dashboard/AGENTS.md` and so on. Read the one for the directory you are working in — that is also what a coding agent picks up.
...
In a hurry? Jump straight to the [tutorial](/developer/tutorial/introduction) to build a feature end to end, the [Next.js Storefront](/developer/storefront/nextjs/quickstart) to put a storefront in front of it, or the [CLI reference](/developer/create-spree-app/quickstart) for what the scaffold generated.
