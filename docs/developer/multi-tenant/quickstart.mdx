...
<Info>
After you set `Spree.root_domain` you must **restart the Rails server** for the change to take effect. The **Settings → Domains** tab in the admin dashboard — where tenants connect their own custom domains — only appears once `Spree.root_domain` is configured and the app has been restarted. See [Custom domains](/developer/multi-tenant/configuration#custom-domains) for details.
</Info>
