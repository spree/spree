...
<Info>
  If you need individual seller/supplier/vendor accounts but shared product listings under one site you should use [multi vendor recipe](/developer/how-to/build-a-marketplace) instead.
</Info>
...
1. Add the following code to your `server/Gemfile`:
...
2. Configure the Docker image build to authenticate against the Keygen source.
    ...
    Wrap **every** `bundle install` in your `server/Dockerfile` with the secret mounts. There
...
```ruby server/config/environments/development.rb
...
```ruby server/app/models/spree/user.rb
...
```ruby server/app/models/spree/admin_user.rb
