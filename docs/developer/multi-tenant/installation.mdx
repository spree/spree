...
3. Run generators:
    ...
    ```bash
    bundle exec rails spree_enterprise:install
    bundle exec rails spree_multi_tenant:install
    ```

    <Info>
      This will copy and run migrations for `spree_enterprise` and `spree_multi_tenant` gem.
    </Info>
