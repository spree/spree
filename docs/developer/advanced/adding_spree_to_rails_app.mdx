...
This installs core models, API, CLI, and the gem-owned authentication models (`Spree::Customer` and `Spree::AdminUser`):

```bash
bin/rails g spree:install
```

To authenticate against your own user model instead of the defaults, pass `--user_class` (see [Authentication](/developer/customization/authentication)).

**Core installation options:**

| Option | Description |
|--------|-------------|
| `user_class` | The customer model class (default: `Spree::Customer`) |
| `admin_user_class` | The admin model class (default: `Spree::AdminUser`) |
