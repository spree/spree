...
description: This guide will show you how to add quick checkout to your custom storefront using Stripe and Spreee API
---
...
Spree Storefront with `spree_stripe` gem already has quick checkout built in, but if you like to add quick checkout to your custom storefront then this guide will help you with that
...
After you have a code which will handle those events, here's what you should do in each event:

#### Address change
Here we need to send some basic information about the customer's shipping address, and we should move the order to the delivery step (don't forget to replace the placeholders with customer's data):
...
This will return you an order response, and you can update the payment element with new total and shipping rates
...
Subsequently, update the payment element with a new total taken from the response
...
2. You should have a lot more information about the customer (in previous events, Stripe probably will not provide you information such as customer address), so send them to backend:
```sh
curl -X PATCH <SHOP URL>/api/v2/storefront/checkout \
  -H 'Authorization: Bearer <Token>' \
  -H 'Content-Type: application/json' \
...
If you like to redirect the user to the Spree's order summary page, then set the `return_url` when confirming the payment intent to `<SHOP URL>/stripe/payment_intents/<PAYMENT INTENT ID>`. This will check if the payment intent is confirmed, move the order to the complete state, and redirect the user to the order summary.
...
curl -X POST <SHOP URL>/api/v2/storefront/stripe/payment_intents/<PAYMENT INTENT ID>/confirm \
  -H 'Authorization: Bearer <Token>'
```
TODO: ADD documentation about this enpoint when PR is merged
...
If at any point user cancels the payment (closes the quick checkout sheet/modal) then clear the shipping and billing address from the order
