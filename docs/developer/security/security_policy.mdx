...
- Configuring [Active Record encryption](/developer/deployment/environment_variables#active-record-encryption) keys in production — without them, webhook signing secrets, payment gateway customer IDs and OAuth tokens are stored in plain text
