...
Spree.order_routing_rules << 'AcmeFresh::OrderRouting::RefrigeratedRule'.constantize
```
...
```ruby
# config/initializers/spree.rb
Spree.order_routing_strategies << 'Acme::Oms::Strategy'.constantize

# Optionally drop the legacy escape hatch:
# Spree.order_routing_strategies.delete(Spree::OrderRouting::Strategy::Legacy)
```
