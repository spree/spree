...
The three methods bracket the lifecycle: allocation → sale, or allocation → release. Whatever your algorithm reserves at allocation time, the other two are where you settle or give it back.
...
- **`for_allocation` runs on every checkout recalculation.** A cart rebuilds its delivery options whenever the address, pickup location, items or market change, so keep the call fast. If your algorithm calls an outside system, cache its answer for the cart's contents rather than calling on every rebuild.
