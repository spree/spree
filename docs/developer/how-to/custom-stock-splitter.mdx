...
The example above relies on `variant.refrigerated?`. In a real plugin you'd back that with a [Custom Field](/developer/core-concepts/metafields) on `Spree::Variant` — say a boolean metafield with key `refrigerated` — and define the predicate as a thin reader:
