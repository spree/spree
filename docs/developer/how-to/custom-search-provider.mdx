...
Products are indexed as **one document per market × locale** combination. Your provider decides its own document shape — write a presenter that returns the fields your engine needs, and point `Spree::Dependencies.search_product_presenter` at it.

If the `spree_meilisearch` gem is installed, its presenter is a working starting point you can reuse or subclass:
...
Provided by the `spree_meilisearch` gem; your own provider may define its own shape instead.
