...
A class-name string can be registered at the top level of the file. Registering the class itself needs `Rails.application.config.after_initialize`, because application classes cannot be loaded while initializer files are still running.
...
- [Digital products](/developer/how-to/sell-digital-products) — what the feature does for merchants
