...
Add `reporting.counters.orders_on_hold.label` (and optionally `.description`)
to your locale file; without an entry the name is humanized. Pass `nav:` with
a sidebar entry's key to badge that entry with this count. Declare `link`
only when the list filter shows exactly the rows counted — the dashboard
resolves `resource` to one of its lists (`orders`, `returns`, `products`) and
appends the channel filter itself for order lists. A counter whose meaning
depends on a store setting can pass `description:` as a lambda receiving the
store, which is how the built-in low stock counter names its threshold.
