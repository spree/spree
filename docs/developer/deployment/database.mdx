...
You can also configure the database in `config/database.yml`. The connection pool is sized to cover web request threads plus [background job](/developer/deployment/background_jobs) threads, since jobs run inside the web container by default:
