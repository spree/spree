...
- **New projects** — `create-spree-app` generates the keys into `.env`, and the [Render Blueprint](/developer/deployment/render) generates them for production.
...
<Warning>
Never change or lose the keys once data is encrypted — records encrypted with them become unreadable. To rotate, add the new keys and keep the old ones as [previous keys](https://guides.rubyonrails.org/active_record_encryption.html#rotating-keys) rather than replacing them.
</Warning>
...
Apps created from an older `spree-starter` also need `config/application.rb` to read the env vars — see the [5.6 to 6.0 upgrade guide](/developer/upgrades/5.6-to-6.0#enable-active-record-encryption).
