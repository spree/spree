...
We also ship [agent skills](/developer/agentic/agent-skills) (`npx skills add spree/agent-skills`), a [docs MCP server](/developer/agentic/mcp), and [LLM-ready documentation](/developer/agentic/llm-docs) — see the [Agentic Development docs](/developer/agentic/overview) for setup across Claude Code, Cursor, Copilot, and other tools.
