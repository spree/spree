...
The resource map grows with your store. The [`spree generate api_resource`](/developer/tutorial/model-and-api) generator scaffolds new Admin API endpoints (model, serializers, controllers, routes, specs) in one command — then call them from the SDK with a [custom fetch or raw request](/developer/sdk/extending), or generate a typed client for them.
