...
Four small pieces — each one unlocks a specific frontend capability. In a host app these live in `server/` (or `api/`); in a distributed plugin, in the Rails engine half.
...
import { LeadTimeCard } from './slots/lead-time-card'
...
```tsx
// src/slots/lead-time-card.tsx
import { Card, CardContent, CardHeader, CardTitle, Field, FieldError, FieldLabel, Input, useHostForm } from '@spree/dashboard'
