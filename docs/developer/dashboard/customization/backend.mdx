...
The dashboard never reaches into Rails models or fetches HTML. All data flows through `@spree/dashboard-sdk` — a typed Admin API client. This page covers the patterns: calling existing endpoints, adding new ones, handling errors, and wrapping it all in React-Query hooks for caching.
...
```ts
import { SpreeError } from '@spree/dashboard-sdk'
