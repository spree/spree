...
import { resourceSearchSchema } from '@spree/dashboard'
...
```tsx
import type { ResourceSearch } from '@spree/dashboard'
...
Custom routes inherit the dashboard's `_authenticated` + `$storeId` layout (auth guard, sidebar, top bar, etc.) — your component renders inside the `<Outlet />`. Use `<ResourceLayout>` to get the same header/main/sidebar grid as core pages:
