...
If your customization introduces a new model on the backend, register it as a permission catalog resource so its keys become grantable to roles (and mintable as API-key scopes):

```ruby
# in your engine's initializer (host app or extension)
Spree.permissions.register_resource(:reports, group: :analytics, subjects: -> {
  [MyApp::Report]
})
```

After that, any role granted `read_reports` makes `permissions.can('read', 'MyApp::Report')` resolve in the dashboard exactly like a first-party check — the abilities ship to the dashboard with the current-user response (`GET /api/v3/admin/me`) at sign-in, alongside `permission_keys`, the flat key list. For record-level rules the catalog can't express, register a plain `CanCan::Ability` class via `Spree::Ability.register_ability` — see [Permissions](/developer/customization/permissions).
