...
The component receives two pieces of context merged into a single props object:

1. **Slot-specific context** — what the host page passes via `<Slot name="..." context={{ product, /* etc */ }} />`. The shape varies per slot — `page.actions_dropdown` passes `{ resource }`, the order-detail page might pass `{ order }`, and so on.
2. **Ambient context** — `{ permissions, store, user }` injected by the providers, available to every slot. Useful for the `if` predicate and for components that only need the current user.

Both shapes are documented per slot in the [Slots catalog](/developer/dashboard/slots-catalog).
...
registerSlot<ProductSlotContext>('product.form_sidebar', {
  ...
  component: ({ product, permissions }) => {
    if (!permissions?.can('read', 'Spree::Brand')) return null
    return <BrandCard productId={product.id} brandId={product.brand_id} />
  },
  position: 50,
})
```
...
The `if` predicate runs on every render with the merged context:

```tsx
slots: {
  'page.actions_dropdown': [{
    id: 'admin-only-thing',
    component: AdminOnlyMenuItem as never,
    if: (ctx) => ctx.permissions?.can('manage', 'Spree::User') ?? false,
  }],
}
```

When it returns `false`, the entry is skipped entirely (no DOM nodes mounted). Useful for entries that should depend on dynamic state — feature flags, A/B buckets, or context the user can change without reloading.
