...
<Info>
The React Dashboard is currently a **Developer Preview** — APIs may still change between releases. It is becoming the default Admin interface.
</Info>

The React Dashboard is a React single-page application (SPA) that talks to the [Spree Admin API](/api-reference/admin-api/introduction). It can be distributed with the Spree API or as a standalone npm package, and it can be extended with plugins. The dashboard is built with modern React libraries and patterns, including:
...
Either way: `cd apps/dashboard && pnpm dev`, open http://localhost:5173, and sign in with your admin email and password. No API keys to configure — admins authenticate interactively.
...
- [Classic Admin extension model](/developer/dashboard/customization/quickstart) — for Spree 5.x
