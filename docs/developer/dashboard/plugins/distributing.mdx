...
1. **Install the npm package** — `pnpm add @your-scope/your-plugin`
2. **Import it in `main.tsx`** — so its side-effects (`defineDashboardPlugin`) run
3. **Apply any backend changes** — usually adding a Rails gem, running migrations
...
import '@/i18n-setup'
import '@your-scope/your-plugin'  // ⬅ step 2
...
**No `vite.config.ts` edit.** As long as your plugin's `package.json` declares the `spree.dashboard.plugin` marker (the CLI scaffold does this for you — see [Publishing](/developer/dashboard/publishing#tell-the-vite-plugin-where-to-find-you)), `spreeDashboardPlugin()` walks the host's dependencies at build time and finds it automatically. Tailwind picks up the plugin's class names without any extra wiring.
...
import '@your-scope/your-plugin'
```

Simple, no dependencies. The consumer has to call `configure` **before** importing the plugin (or the plugin's `defineDashboardPlugin` runs with defaults).
