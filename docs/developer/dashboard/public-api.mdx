...
The shared `@spree/dashboard-sdk` instance configured for the host's API URL. Use this everywhere instead of constructing your own.
...
The deployable app. **You normally don't import from here in a plugin.** The exception is when you're composing a custom dashboard variant (e.g., a vendor portal) and want to reuse a resource hook:
...
Importing from `@spree/dashboard` pins your code to the deployable app's version. If you only need the underlying primitives, prefer `@spree/dashboard-core`.

## What about `@spree/dashboard-sdk`?

The SDK is a separate package and a public dependency. Import types and `SpreeError` from it directly:
...
- [`packages/dashboard-sdk/src/index.ts`](https://github.com/spree/spree/blob/main/packages/dashboard-sdk/src/index.ts)
