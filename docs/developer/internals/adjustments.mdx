...
An `Adjustment` object tracks an adjustment to the price of an [Order](orders.md), an order's [Line Item](orders.md#line-items), or an order's [Shipments](shipments.md) within a Spree Commerce storefront.
...
* `mandatory`: Indicates if this adjustment is mandatory; i.e that this adjustment _must_ be applied regardless of its eligibility rules.
...
The _source_ is the source of the adjustment. Typically a `Spree::TaxRate` object or a `Spree::PromotionAction` object.

The _adjustable_ is the object being adjusted, which is either an order, line item or shipment.
...
* `charge`: Adjustments which _increase_ the price of their adjustable.
* `credit`: Adjustments which _decrease_ the price of their adjustable.
