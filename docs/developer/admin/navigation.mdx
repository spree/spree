...
sidebar_nav.add :brands,
  ...
  icon: 'award',
  position: 35,
  active: -> { %w[brands brand_categories].include?(controller_name) },
  if: -> { can?(:manage, Spree::Brand) } do |brands|

  brands.add :all_brands,
    label: 'All Brands',
    url: :admin_brands_path,
    position: 10,
    active: -> { controller_name == 'brands' }

  brands.add :brand_categories,
