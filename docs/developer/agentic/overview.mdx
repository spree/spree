...
- **A guided project workflow** — [create-spree-app](/developer/create-spree-app/quickstart) scaffolds projects with a generated `AGENTS.md` and `CLAUDE.md`, local docs, and the [Spree CLI](/developer/cli/quickstart), so an agent can boot, migrate, generate code, and run tests without bespoke setup.
...
<CardGroup cols={2}>
  ...
  <Card title="Admin MCP Server" icon="store" href="/developer/agentic/admin-mcp">
    Let an agent run your store — merchandise the catalog, answer reporting questions, manage orders and setup — under the same permissions as the Admin API.
  </Card>
  ...
  <Card title="Spree CLI" icon="square-terminal" href="/developer/cli/quickstart">
    A predictable command surface (`spree dev`, `spree generate`, `spree migrate`) plus `spree api` — call the Admin API directly to inspect and drive the store, with zero-config credentials in local dev. Agents debug and act end-to-end.
  </Card>
</CardGroup>
