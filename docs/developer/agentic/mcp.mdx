...
<Note>
  This server is about Spree's documentation, not your store. To let an agent
  work on your own catalog, orders and settings, see the
  [Admin MCP server](/developer/agentic/admin-mcp) — the same protocol, hosted
  by your application and authenticated with one of your API keys.
</Note>
...
| **[Admin MCP server](/developer/agentic/admin-mcp)** | Not docs at all — operating a real store: its catalog, orders, reports and settings |
