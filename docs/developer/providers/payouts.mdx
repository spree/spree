...
Register inside `after_initialize`: the provider class lives in your application, and Rails cannot load application classes while initializer files are still running. Spree keeps anything registered earlier, so a class-name string such as the integration also works at the top level of the file.
...
<Note>
  `available` is what `available_for_store?` answers. A provider that returns
  false still appears in the picker, greyed out, with the dashboard explaining
  what the operator has to connect first — so implement it to mean "usable
  today", not "installed".
</Note>
