...
<Info>
  Node.js is only needed for the scaffolding tool and the Next.js storefront. The Spree backend itself runs inside Docker — you don't need Ruby installed locally.
</Info>
...
- **Spree** — runs the `ghcr.io/spree/spree:latest` image on port `3000`
...
If you used `--no-start`, you can start services manually:
...
```bash
docker compose exec spree bin/rails spree:load_sample_data
```
