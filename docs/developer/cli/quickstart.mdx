...
First-run setup. Starts Docker services, seeds the database, generates API keys, deploys the project's `spree.config.yml` (see [`spree config`](#spree-config)), and prints the one-time setup link where you create the admin account and can load sample data.

```bash
spree init
spree init --no-open                                   # Skip opening browser
spree init --admin-email ops@example.com \
           --admin-password '********' --no-sample-data  # Scripted install: seed the admin, skip sample data
```
...
Switch from the prebuilt Docker image to building from your local `server/` directory. This lets you customize the Rails app — add gems, override models, add migrations, etc.

```bash
spree eject
```

After ejecting, the Docker image is built from `server/Dockerfile`. Edit files in `server/` and run `spree dev` to rebuild and restart with your changes.
...
Describe the store in `spree.config.yml` (every new project ships one) and deploy it anywhere through the Admin API: channels, markets, tax, delivery zones and methods, warehouses, categories, products, customers. Records match on natural keys, so one file works against local, staging and production. See [Store Configuration as Code](/developer/cli/configurator).

```bash
spree config validate                    # schema check, offline
spree config diff                        # what deploy would change
spree config deploy --fail-on-delete     # apply; refuse any delete (CI guard)
spree config introspect --out store.yml  # capture the live store into a file
```
