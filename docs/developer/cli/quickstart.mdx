...
Not appropriate for Gemfile changes (use `spree bundle install` then `spree stop && spree dev`), Dockerfile / `.ruby-version` changes (use `spree build`), or compose file changes (use `spree stop && spree dev`).
...
Run a Rails generator inside the container. Same arguments as `bin/rails generate`.

```bash
spree generate model Brand name:string slug:string
spree generate migration AddPositionToProducts position:integer
spree generate spree:api_resource Brand name:string slug:string  # The Spree v3-conformant generator
```

See [Generators](/developer/customization/generators) for the Spree-specific generators (`spree:model`, `spree:api_resource`) that extend Rails' built-in ones with Spree conventions.

### `spree migrate`

Install pending Spree migrations from gems, then run `db:migrate`.

```bash
spree migrate
```

Equivalent to `spree rails spree:install:migrations db:migrate` — a shortcut for the canonical post-update sequence.

### `spree db:reset`

Drop, recreate, and seed the database. **Destructive** — wipes everything. Asks for confirmation.
...
Open a database console (`bin/rails dbconsole`) connected to the dev Postgres.
