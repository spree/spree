...
If your Spree initializer (`config/initializers/spree.rb`) sets `Spree.searcher_class`, remove that line. The `Spree::Core::Search::Base` class and the `searcher_class` configuration option have been removed. Use `Spree::Products::Find` finder instead.
