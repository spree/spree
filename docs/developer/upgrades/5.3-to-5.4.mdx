...
Spree 5.4 improves security by masking real database IDs with Stripe-style prefixed IDs, enhancing data privacy and preventing enumeration attacks. Prefixed IDs are also easier to read and understand, eg. Product ID is `prod_86Rf07xd4z` and order ID is `or_k5nR8xLq02`.

Prefixed IDs are computed on the fly using [Sqids](https://sqids.org/) encoding of integer primary keys. No database migration or backfill is required — they work automatically for all existing records.
