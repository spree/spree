...
### Remove `Spree.searcher_class` from your initializer

If your Spree initializer (`config/initializers/spree.rb`) sets `Spree.searcher_class`, remove that line. The `Spree::Core::Search::Base` class has been removed and the `searcher_class` configuration option is deprecated and will be removed in Spree 5.5.
