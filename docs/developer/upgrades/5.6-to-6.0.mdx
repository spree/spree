...
<Warning>
Never change or lose the keys once data is encrypted — the encrypted records become unreadable.
</Warning>

**3. Encrypt existing rows.** Rows written before the keys were set are plain text. OAuth tokens on user identities stay readable and are encrypted on their next write. Webhook endpoint secrets and gateway customer IDs can't be read in plain text once encryption is on — follow [Enabling encryption on an existing installation](/developer/deployment/environment_variables#enabling-encryption-on-an-existing-installation): turn on `support_unencrypted_data` and `extend_queries`, deploy with the keys, run `find_each(&:encrypt)` over `Spree::WebhookEndpoint`, `Spree::GatewayCustomer` and `Spree::UserIdentity`, then turn the two settings off again.
...
The `Create line items` promotion action now discounts the items it adds down to zero. It used to add them at their full price and leave the discounting to you.

<Warning>
  **Delete the second action if you paired one with it.** The old guidance was to add a whole-order discount set to the gift's price to pay for the gift. That discount still applies, so a promotion configured the old way now discounts twice — the gift is free *and* the order discount comes off. Open each promotion using `Create line items`, and remove the accompanying `Create whole-order adjustment` that exists only to cancel out the gift.
</Warning>
...
`Spree::Stock::Coordinator` itself stays — the `Rules` strategy packs through it, and exchanges and claims still use it directly.

### Storefront checkout follows order routing

In 5.x a cart built its delivery options straight from `Spree::Stock::Coordinator`, so routing rules and custom strategies only affected orders built by staff. Carts now go through the same strategy as orders — the channel's `preferred_order_routing_strategy`, falling back to the store's. A customer's items ship from the locations your routing rules prefer, and a customer's chosen pickup location is preferred by the `preferred_location` rule.

If you wrote a custom strategy, it now also runs during checkout, where `order` is a `Spree::Cart`. It runs on every checkout recalculation, so keep it fast.

**BREAKING — `for_cancellation` is removed from the strategy contract.** Spree never called it, and orders cannot be canceled after they ship; returns handle restocking. `for_sale(fulfillment:)` is now called after a fulfillment ships, and `for_release` after an order is canceled. Both run once the change is saved and are for telling an outside system — Spree writes the stock movements itself. A `for_cancellation` method left on your strategy is simply never called.
...
**Where each question goes now.** Aggregates — revenue by channel, product performance, anything you would have written a report class for — are [reporting](/developer/core-concepts/reporting) queries, composed from registered metrics and dimensions rather than a class per question. Row-level CSV of records is a `Spree::Export` subclass, which is what [imports and exports](/developer/core-concepts/imports-exports) covers, and unlike `Spree::Report` it has both an Admin API and a dashboard page.
