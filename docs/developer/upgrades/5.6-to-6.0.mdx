...
Core registers the default validator before your initializers load, so the swap works at the top level of an initializer. The same validator also guards `exchanges.create.validate`; unregister it there too if your policy should cover exchanges.
