...
staff **or an API key**, so each of those columns gained a companion naming
which kind it is. Rows written before the upgrade carry only an id; this fills
in the kind. Until it runs they still read as the staff member they always
