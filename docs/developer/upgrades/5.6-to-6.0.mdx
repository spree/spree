...
| Custom permission sets with record-level rules | `Spree::Ability.register_ability(MyAbility)` — a plain `CanCan::Ability` class |
...
Storefront customers are not part of this system: the old `:default` role and `DefaultCustomer` set became internal ability code, and customers never resolve roles. Extensions register their resources into the catalog once, which makes them grantable to roles **and** mintable as API-key scopes:
