...
Category and collection descriptions, policy bodies, and order and customer internal notes move from `action_text_rich_texts` into text columns on their own tables. Per-locale rows land in the model's translation table. Run this **after** the categories step, which re-points the rows from `Spree::Taxon` to `Spree::Category` so this one can still find them.

Content is sanitized on the way in, and **the 6.0 allowlist is much narrower than 5.6's**: it permits only what the dashboard's editor emits — paragraphs, headings, `strong`/`em`/`s`/`u`/`code`, `pre`, `blockquote`, lists, `hr`, `br` and links. Tables, images, `div`/`span`, inline `style` and arbitrary `class` attributes are no longer permitted. Text inside a stripped tag survives; its formatting does not.
...
Reads are unchanged — the plain field still returns text and `*_html` still returns markup — but writes move to the `*_html` name, so reading and writing finally use the same field:
...
A write that still sends the plain name is **ignored rather than stored** — it is no longer a permitted parameter, so nothing 422s and the value silently doesn't persist. Audit any integration that writes descriptions or internal notes.
