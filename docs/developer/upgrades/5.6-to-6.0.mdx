...
| `Spree::WishedItem`, `Wishlist#wished_items`, `#wished_items_count` | `Spree::WishlistItem`, `#wishlist_items`, `#wishlist_items_count` (table renamed; the prefixed ID change is a **breaking** change — see below) |
...
### Breaking: wishlist item IDs change prefix

`Spree::WishlistItem` prefixed IDs move from `wi_` to `wli_`, and unlike the renames above **this one has no bridge** — ID decoding rejects a prefix that does not match, so a previously issued `wi_...` ID returns 404 on `PATCH`/`DELETE /store/wishlists/:id/items/:item_id`.

Prefixed IDs are computed from the primary key rather than stored, so nothing needs migrating: the same record answers to its new ID immediately. What breaks is a client holding an old one. If your storefront persists wishlist item IDs (a cached list, an optimistic update, a deep link), clear them on upgrade and re-read the wishlist. Every 6.0 prefix change follows this rule — `Spree::StockLevel` moved `si_` → `sl_` the same way.
