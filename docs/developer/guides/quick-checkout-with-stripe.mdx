...
<ParamField body="bill_address_id" type="string" required>
  Set it to `CLEAR` to clear existing billing address (this prevents order to accidentaly advance to the `payment` step)
</ParamField>
</Accordion>
...
If you receive response code other than `200` then something changed in the order and it can't be moved to the payment state yet
</Accordion>
