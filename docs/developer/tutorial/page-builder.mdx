...
module Spree
  module PageSections
    class BrandGrid < Spree::PageSection
      ...
      preference :columns, :integer, default: 4
      ...
      def brands
        store.brands.active.order(:name)
      end
    end
  end
end
...
<div class="form-group">
  <%= f.label :preferred_columns, Spree.t(:columns) %>
  <%= f.select :preferred_columns,
      options_for_select([2, 3, 4, 5, 6], @page_section.preferred_columns),
      {},
      { class: 'custom-select', data: { action: 'auto-submit#submit' } } %>
</div>

<div class="custom-control custom-checkbox mb-3">
  <%= f.check_box :preferred_show_description,
      class: 'custom-control-input',
      data: { action: 'auto-submit#submit' } %>
  <%= f.label :preferred_show_description,
      Spree.t(:show_description),
...
<div class="custom-control custom-checkbox mb-3">
  <%= f.check_box :preferred_show_logo,
      class: 'custom-control-input',
      data: { action: 'auto-submit#submit' } %>
  <%= f.label :preferred_show_logo,
      Spree.t(:show_logo),
      class: 'custom-control-label' %>
</div>
...
<% content_for(:design_tab) do %>
  <div class="form-group">
    <%= f.label :preferred_layout, Spree.t(:layout) %>
    <%= f.select :preferred_layout,
        options_for_select([
...
<% cache_unless page_builder_enabled?, spree_storefront_base_cache_scope.call(section) do %>
  <div style="<%= section_styles(section) %>">
    <div class="page-container">
      ...
      <% if brands.any? %>
        <div class="grid grid-cols-2 md:grid-cols-<%= section.preferred_columns %> gap-6">
