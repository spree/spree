...
<Info>
  This guide assumes you've completed the previous tutorials and have a working `Spree::Brand` model with admin and storefront controllers.
</Info>
  ...
  factory :brand, class: Spree::Brand do
    ...
    trait :with_products do
      ...
      after(:create) do |brand, evaluator|
        create_list(:product, evaluator.products_count, brand: brand, stores: [brand.store])
      end
    end
  end
end
...
brand = create(:brand, name: 'Nike', active: true)
...
RSpec.describe Spree::Brand, type: :model do
  ...
  describe 'validations' do
    subject { build(:brand, store: store) }

    it { is_expected.to validate_presence_of(:name) }

    describe 'slug uniqueness' do
      let!(:existing_brand) { create(:brand, slug: 'nike', store: store) }

      it 'validates uniqueness of slug within store scope' do
        brand = build(:brand, slug: 'nike', store: store)
      ...
      it 'allows same slug in different stores' do
        ...
        brand = build(:brand, slug: 'nike', store: other_store)
  ...
  describe 'scopes' do
    describe '.active' do
      let!(:active_brand) { create(:brand, active: true, store: store) }
      let!(:inactive_brand) { create(:brand, active: false, store: store) }

      it 'returns only active brands' do
        expect(described_class.active).to include(active_brand)
        expect(described_class.active).not_to include(inactive_brand)
      end
    end
  end
  ...
  describe 'FriendlyId' do
    it 'generates slug from name' do
      brand = create(:brand, name: 'Nike Sportswear', store: store)
      expect(brand.slug).to eq('nike-sportswear')
    end

    it 'handles duplicate names by appending UUID' do
      create(:brand, name: 'Nike', store: store)
      brand = create(:brand, name: 'Nike', store: store)
      expect(brand.slug).to match(/nike-[a-f0-9-]+/)
    end
  end

  describe '#image' do
    let(:brand) { create(:brand, :with_logo, store: store) }
...
RSpec.describe Spree::Admin::BrandsController, type: :controller do
  ...
  describe 'GET #index' do
    let!(:brand1) { create(:brand, name: 'Adidas', store: store) }
    let!(:brand2) { create(:brand, name: 'Nike', store: store) }
  ...
  describe 'POST #create' do
    context 'with valid params' do
      let(:valid_params) do
        { brand: { name: 'New Brand', active: true } }
      end
  ...
  describe 'PUT #update' do
    let(:brand) { create(:brand, name: 'Old Name', store: store) }
...
RSpec.describe Spree::BrandsController, type: :controller do
  ...
  describe 'GET #index' do
    ...
    it 'assigns only active brands' do
      get :index
      expect(assigns(:brands)).to include(active_brand)
      expect(assigns(:brands)).not_to include(inactive_brand)
    end
  end
...
RSpec.feature 'Admin Brands', type: :feature do
  ...
  describe 'listing brands' do
    let!(:brand1) { create(:brand, name: 'Nike', store: store) }
    let!(:brand2) { create(:brand, name: 'Adidas', store: store) }
  ...
  describe 'editing a brand' do
    let!(:brand) { create(:brand, name: 'Nike', store: store) }
...
RSpec.feature 'Storefront Brands', type: :feature do
  ...
  describe 'brands listing page' do
    let!(:nike) { create(:brand, name: 'Nike', active: true, store: store) }
    let!(:adidas) { create(:brand, name: 'Adidas', active: true, store: store) }
    let!(:inactive) { create(:brand, name: 'Inactive Brand', active: false, store: store) }
    ...
    it 'displays active brands' do
      ...
      expect(page).to have_content('Nike')
      expect(page).to have_content('Adidas')
      expect(page).not_to have_content('Inactive Brand')
    end
  ...
  describe 'brand detail page' do
    let(:brand) { create(:brand, :with_description, name: 'Nike', store: store) }
...
it 'creates brand with all attributes', :aggregate_failures do
  brand = create(:brand, name: 'Nike', active: true, store: store)

  expect(brand.name).to eq('Nike')
  expect(brand.active).to be(true)
