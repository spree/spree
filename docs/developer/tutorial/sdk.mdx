...
In this tutorial, we'll use the `@spree/sdk` TypeScript SDK to consume the Brand API endpoints we created in the [API tutorial](/developer/tutorial/api), and work with the extended Product data that now includes brand information.

<Info>
  This guide assumes you've completed the [API](/developer/tutorial/api) tutorial and have the Brand endpoints running.
</Info>
...
Ransack predicates work on whitelisted attributes and associations. The [API tutorial](/developer/tutorial/api) shows how to register `brand_id` and `brand` via `Spree.ransack` — once that's done, you can filter:
...
- [API Tutorial](/developer/tutorial/api) - Creating the Brand API endpoints
...
- [Agentic Development](/developer/agentic/overview) - Agent skills and the docs MCP server cover the SDK patterns used here
