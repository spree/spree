...
The card itself receives the product from the slot's context:

```tsx apps/dashboard/src/brands/product-card.tsx
import { Card, CardContent, CardHeader, CardTitle } from '@spree/dashboard'
import { useTranslation } from 'react-i18next'

export function BrandCard({ product }: { product: { id: string; brand_id?: string | null } }) {
  const { t } = useTranslation()
    ...
    <Card>
      ...
      <CardContent>
        <p className="text-sm text-muted-foreground">
          {product.brand_id ?? t('admin.brands.product_card.none')}
        </p>
      </CardContent>
    </Card>
  )
