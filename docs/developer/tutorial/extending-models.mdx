...
For the API to save the brand association, we need to permit the `brand_id` parameter.

Declare it on the model you extended. The product create and update endpoints
pick the attribute up automatically — no controller decorator, and no
initializer:

```ruby app/models/spree/product_decorator.rb {5-7}
module Spree
  module ProductDecorator
    def self.prepended(base)
      base.belongs_to :brand, class_name: 'Brand', optional: true

      def base.additional_permitted_attributes
        [:brand_id]
      end
    end
  end
...
Return the full list your extension contributes. For collections and nested
structures, use the same shapes `params.permit` accepts — `[{ region_ids: [] }]`.

<Note>
  For merchant-managed data, prefer [Custom Fields](/developer/core-concepts/custom-fields),
  which need no code at all and are filterable and sortable out of the box.
  `additional_permitted_attributes` is for extensions that add real database columns.
</Note>
...
**Events approach** (recommended for side effects): subscribe to `product.updated` and react in a `Spree::Subscriber` — no decorator, no callback coupling. The [Events & Webhooks tutorial](/developer/tutorial/events) builds this out fully, including external-system sync (OMS, warehouse) and outbound webhooks.
...
### Permitting the Brand Parameter

```ruby app/models/spree/product_decorator.rb
# Permit brand_id on every API v3 endpoint that writes a product
def base.additional_permitted_attributes
  [:brand_id]
end
```
