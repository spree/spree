...
Add it to the model's permitted attributes in your Spree initializer. The
product create and update endpoints pick it up automatically — no controller
decorator needed:
...
Use `+=` rather than `=`, so you append to the list instead of replacing what
another extension already added. For collections and nested structures, use the
same shapes `params.permit` accepts — `[{ region_ids: [] }]`.
...
```ruby config/initializers/spree.rb
# Permit brand_id on the API v3 endpoints that write a product
Spree::Product.additional_permitted_attributes += [:brand_id]
```
