...
<Note>
  A generated resource is addressed by its prefixed ID. Looking one up by slug needs a `find_resource` override on the controller — the products endpoint does exactly that, so `/products/{slug}` works. Until you add one for brands, route by ID and treat `slug` as display data.
</Note>
...
export default async function BrandPage({ params }: { params: { id: string } }) {
  const brand = await client.request<Brand>('GET', `/brands/${params.id}`)
