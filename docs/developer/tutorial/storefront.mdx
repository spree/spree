...
<Info>
  This guide assumes you've completed the [Model](/developer/tutorial/model) and [Extending Core Models](/developer/tutorial/extending-models) tutorials. You should have a working `Spree::Brand` model with products associated to brands.
</Info>
...
module Spree
  class BrandsController < StoreController
    ...
    def index
      @brands = current_store.brands.active.order(:name)
    end
    ...
    def load_brand
      @brand = current_store.brands.find(params[:id])
    end
  ...
  <% if @products.any? %>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      <% @products.each do |product| %>
        <%= render 'spree/products/product_card', product: product %>
      <% end %>
    </div>
...
You now have working brand pages using standard Rails MVC patterns. In the next guide, [Page Builder Integration](/developer/tutorial/page-builder), we'll enhance these pages with:

- Admin-manageable page layouts
- Drag-and-drop sections
- Customizable design settings

This allows your team to modify brand pages without code changes.
...
- [SEO](/developer/tutorial/seo) - Add friendly URLs, meta tags, and Open Graph data
- [Page Builder Integration](/developer/tutorial/page-builder) - Next tutorial
