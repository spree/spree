...
Failed deliveries are retried with exponential backoff, and the dashboard shows the delivery history per endpoint, which is where to look when a third party says they never received something.
