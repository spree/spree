...
- Use [FriendlyId slugs](/developer/core-concepts/slugs) library for nice URLs
...
- [Ransack search & filtering](/developer/core-concepts/search-filtering) to allow for searching, filtering and sorting the Brands. `created_at` and `updated_at` columns are always included by default so you don't need to specify them in the `whitelisted_ransackable_attributes` array.
