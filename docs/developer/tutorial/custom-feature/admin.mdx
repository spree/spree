...
This file is responsible for displaying the list of brands in the Admin Dashboard. It will handle sorting, filtering, pagination, and the empty state.
...
- `content_for :page_title` - Set the page title
- `content_for :page_actions` - Add a new button to the page actions, uses `can?` to run a check if the current user has permission to create a new brand
- `link_to_with_icon 'plus', Spree.t(:new_brand), new_object_url, class: 'btn btn-primary'` - Add a new button to the page actions to create a new brand, `new_object_url` is a helper that generates the URL to create a new brand, it's the same as calling `spree.new_admin_brand_path`
- `render 'spree/admin/shared/index_table'` - Render the index table partial which handled sorting, filtering, pagination, and the empty state.
...
```erb app/views/spree/admin/brands/new.html.erb
...
Scaffold also created a `app/views/spree/admin/brands/edit.html.erb` template with the following content:

```erb app/views/spree/admin/brands/edit.html.erb
...
As with the new view, you can just leave it as is.
