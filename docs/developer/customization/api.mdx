...
controller, append to the model's list from an initializer:
...
declaration covers the model's create and update endpoints. Two endpoints
deliberately ignore the hook because their parameters are authorization data
rather than resource data: API keys and invitations.
