...
Before you start customizing Spree API endpoints, make sure you reviewed all existing API endpoints in the [Spree API docs](/api-reference/admin-api/endpoints).
...
To add a writable attribute to a **core** Spree resource without decorating its
controller, declare it on the model instead:
...
declaration covers the model's create and update endpoints. A handful of
endpoints build their parameters by hand for provider-specific reasons (payment
method credentials, for one) and do not consult the hook — if an attribute you
declared is being ignored, check whether that controller overrides
`permitted_params` instead of `resource_permitted_attributes`.

Prefer [Custom Fields](/developer/core-concepts/custom-fields) for
merchant-managed data — this hook is for extensions that add real database
columns.
