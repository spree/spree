...
<Warning>
  **Register inside `to_prepare`, not at the top level of an initializer.** The
  registry is reloadable code holding class-level state, so anything registered once
  at boot is discarded the first time your app reloads in development, and your
  requirement quietly stops applying. `to_prepare` runs again after every reload.
</Warning>
...
The step now appears in the cart's `checkout_steps`, its requirements show up while
unsatisfied, and completion refuses the cart until they are met.
