---
title: Configuration
description: Every global Spree setting that is actually read by Spree — grouped by area, with defaults and what each one changes.
---
...
This page lists only settings Spree actually reads. Settings that exist for backwards compatibility are listed under [Deprecated settings](#deprecated-settings) rather than mixed in with the rest.
...
The global versions of these settings still exist but are no longer read, so setting one in the initializer changes nothing. If you're upgrading from an installation that used them, run `bin/rake spree:store_settings:backfill_from_config` to copy your values onto every store.
...
If you're upgrading from an installation that used the older `auto_capture` and `auto_capture_on_dispatch` booleans, two tasks carry it over: `spree:store_settings:backfill_from_config` maps the old globals onto each store's `capture_method`, and `spree:migrate_capture_methods` converts the per-payment-method `auto_capture` column. A method left without a value inherits its store.
...
These still exist so upgrades don't break, but Spree either ignores them or reads them only from subsystems it has replaced. Don't use them in new code.
...
| `restock_inventory` | Restocking is decided per return line item by its resellable flag — see [Returns](/developer/core-concepts/returns-exchanges-claims#processing-a-return) |
| `expedited_exchanges` | [Exchanges](/developer/core-concepts/returns-exchanges-claims) |
| `expedited_exchanges_days_window` | [Exchanges](/developer/core-concepts/returns-exchanges-claims) |
...
| `checkout_zone` | Use [delivery zones](/developer/core-concepts/fulfillments#delivery-methods-and-zones). |
