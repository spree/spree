...
### Changing when a rule applies

Most conditional rules on an address are gated on a predicate — `require_name?`,
`require_street?`, `require_postal_code?`, `require_phone?` and
`require_company?`. Overriding one changes *when* the rule applies without
removing it, which is safer than dropping the validation outright.

This is the one case here that needs Ruby. See
[decorators](/developer/customization/decorators) for how to override a model
method.
