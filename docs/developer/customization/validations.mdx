...
The block is needed because validator classes cannot be loaded while initializer files are still running. A rule unregistered there stays off even though Spree adds its defaults later in boot.
