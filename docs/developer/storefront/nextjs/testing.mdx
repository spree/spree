...
```bash
SPREE_IMAGE=ghcr.io/your-org/your-spree:latest pnpm e2e:up
pnpm test:e2e
```

### A `create-spree-app` project's backend

A project scaffolded with [`create-spree-app`](/developer/create-spree-app/quickstart) has its customizable Spree in `server/` (built from `server/Dockerfile`) and this storefront in `apps/storefront/`. Build that backend into a local image and point the suite at it — this is what tests the storefront against the **exact Spree you deploy**, extensions and all:
...
cd apps/storefront
SPREE_IMAGE=project-spree:e2e pnpm e2e:up
pnpm test:e2e
pnpm e2e:down
```

<Note>
  New `create-spree-app` projects wire this into the generated storefront CI workflow automatically — the workflow builds `server/Dockerfile` and runs the storefront E2E against that image rather than stock Spree.
</Note>

### Interactive / manual

To click through the storefront against a running backend (no E2E harness), point it there directly: set `SPREE_API_URL` and `SPREE_PUBLISHABLE_KEY` in `.env.local` (see [Environment Variables](/developer/storefront/nextjs/environment-variables)) and run `pnpm dev`.
