---
title: Custom JavaScript in the Rails storefront
sidebarTitle: "Custom JavaScript"
description: "Extend the Spree Rails storefront with custom JavaScript using Stimulus controllers, importmap entries, and Turbo-compatible behaviors."
---

## Extending Storefront with JavaScript

Spree Storefront can be easily extended with custom JavaScript. Most of the JavaScript in the storefront is powered by a framework called [Stimulus.js](https://stimulus.hotwired.dev/). It's a very simple and minimalistic framework only enhancing our server-side rendered HTML with a bit of interactivity.
...
2. Creating a new partial file `app/views/spree/shared/_my_tracking_code.html.erb` (note the leading underscore — it's rendered as a partial), where you can add your tracking code, eg.

    ```erb app/views/spree/shared/_my_tracking_code.html.erb
    <script>
...
- [Custom CSS](/developer/storefront/rails/custom-css) - Styling your storefront
- [Sections](/developer/storefront/rails/sections) - Creating custom sections
- [Blocks](/developer/storefront/rails/blocks) - Creating custom blocks
