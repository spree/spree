...
<ResponseField name="try_spree_current_user" type="Spree.customer_class" description="The current user object.">
  <SpreeUser />
</ResponseField>

Returns the current user object (class depends on the `Spree.customer_class` configuration). If the user is not signed in, it will return `nil`.
