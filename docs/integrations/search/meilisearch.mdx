...
[Meilisearch](https://www.meilisearch.com/) is a fast, open-source search engine that provides typo tolerance, relevance ranking, faceted filtering, and sub-50ms search responses. The `spree_meilisearch` gem provides a Meilisearch search provider that replaces the default SQL-based search.

<Note>
In Spree 5.x this provider shipped inside `spree_core` as `Spree::SearchProvider::Meilisearch`. From Spree 6.0 it lives in its own `spree_meilisearch` gem as `SpreeMeilisearch::SearchProvider`. The old class names keep working with a deprecation warning for one release. No reindex is needed when upgrading — the documents are unchanged.
</Note>
