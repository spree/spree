...
sidebarTitle: Stripe
description: "Set up Spree's native Stripe integration to accept cards and alternative payment methods, and pay marketplace sellers through Stripe Connect."
---
...
<Tip>Running a marketplace? The same Stripe account pays your sellers through [Stripe Connect](/integrations/payments/stripe-connect).</Tip>
...
<Note>Before using Stripe Tax, you need to configure your tax nexus (the regions where you're registered to collect tax) in your Stripe dashboard.</Note>
...
See [<u>Settings → Tax Rates</u>](/user/settings/tax-rates) for more on configuring tax rates and categories, and [<u>Create a Product</u>](/user/products/creating-products) for details on assigning tax categories.


## Stripe Connect for Marketplaces

Stripe Connect is the payout provider that ships with this integration: each seller onboards to an Express account through Stripe's hosted pages, their share of every order is transferred as it ships, and settlements are confirmed by webhook. Setup and the money flow are on their own page — [Stripe Connect for marketplaces](/integrations/payments/stripe-connect).
