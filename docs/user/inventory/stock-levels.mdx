...
<Warning>A correction is for what you have counted. Stock that arrives from a supplier belongs on a purchase order, and stock moving between your own warehouses belongs on a [stock transfer](/user/inventory/stock-transfers), so that the delivery is recorded and the **Incoming** figure stays right.</Warning>
