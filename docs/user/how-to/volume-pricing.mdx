...
<Note>A single price list can also carry per-variant **quantity breaks**: a lower unit price from a given quantity up, set in the price edit form or imported from a CSV. Use separate price lists, as below, when whole assortments or rules differ between tiers; use quantity breaks when the same list should simply charge less per unit at volume. See [Price Lists](/user/pricing/price-lists#quantity-breaks).</Note>
...
Familiarity with how Price Lists work is helpful. See [<u>Price Lists</u>](https://spreecommerce.org/docs/user/pricing/price-lists) for a full reference.
...
- [<u>Price Lists</u>](https://spreecommerce.org/docs/user/pricing/price-lists) — full reference for rules, product selection, and price editing
