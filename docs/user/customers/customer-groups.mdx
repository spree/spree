...
**Pricing tiers** — By assigning a [<u>Catalog</u>](https://spreecommerce.org/docs/user/catalogs/catalogs) to a customer group, every member of that group automatically sees their correct prices the moment they log in. The catalog can price your entire range, or restrict the group to specific products at their own prices. You can have as many groups and catalogs as you need — one per customer tier, one per named account — and Spree applies the right one without any manual work. When you add a new customer to a group, they inherit the pricing immediately. A buyer whose company has a catalog of its own gets that instead: company assignments win over group ones.
...
Each group is assigned its own catalog. When a buyer logs in, Spree checks their group membership and applies the matching catalog automatically.
...
- [<u>Price Lists</u>](https://spreecommerce.org/docs/user/pricing/price-lists) — create pricing rules that target specific customer groups
