...
Render sets `DATABASE_URL` and `SECRET_KEY_BASE` automatically from the Blueprint. For additional configuration (SMTP, file storage, Sentry, etc.), see [Environment Variables](/v5/developer/deployment/environment_variables).

Generated URLs (images and attachments in API responses, email links) automatically use Render's `RENDER_EXTERNAL_HOSTNAME` (`<your-app-name>.onrender.com`). When you attach a custom domain, set [`RAILS_HOST`](/v5/developer/deployment/environment_variables#urls-and-hosts) to it — it takes precedence.

## Scaling Out

The Blueprint's commented **worker** block switches to [split mode](/v5/developer/deployment/quickstart#web-and-worker): background jobs move into a dedicated service — same image, no rebuild. Uncomment it and set `SOLID_QUEUE_IN_PUMA=false` on the web service so it stops running jobs.
...
High-traffic installs can additionally swap [caching](/v5/developer/deployment/caching) to a managed Redis/Valkey — the default needs no extra service.

## Next Steps

Render provides ephemeral storage — uploaded files (product images, etc.) won't persist across deploys. Set up cloud storage:

- [Asset Storage](/v5/developer/deployment/assets) (Amazon S3 / Cloudflare R2)

Before going to production:

- [Set environment variables](/v5/developer/deployment/environment_variables) (SMTP, SSL, etc.)
- [Configure CDN](/v5/developer/deployment/cdn)
- [Configure caching](/v5/developer/deployment/caching)

Deploying a storefront? The Next.js storefront ships separately — typically to Vercel. See [Storefront Deployment](/v5/developer/storefront/nextjs/deployment).
