...
canonical: "https://spreecommerce.org/docs/developer/deployment/emails"
---
...
3. **The storefront handles everything else** — signature verification, event routing, email rendering, and delivery are built in. See the [Next.js storefront email docs](/v5/developer/storefront/nextjs/customization#transactional-emails) for template customization.
...
See [Webhooks documentation](/v5/developer/core-concepts/webhooks) for the full payload format and verification details.
...
| `RAILS_HOST` | `example.com` | Public host used in email links and other [generated URLs](/v5/developer/deployment/environment_variables#urls-and-hosts) — image/attachment URLs use `CDN_HOST` instead when set |
