...
canonical: "https://spreecommerce.org/docs/developer/deployment/database"
---
...
You can also configure the database in `config/database.yml`. The connection pool is sized to cover web request threads plus [background job](/v5/developer/deployment/background_jobs) threads, since jobs run inside the web container by default:
