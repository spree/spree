...
canonical: "https://spreecommerce.org/docs/developer/deployment/aws"
---

The simplest way to run Spree on AWS is the [single-node topology](/v5/developer/deployment/quickstart#single-node-vs-distributed): **one EC2 instance running the Docker image, one RDS database**. No clusters, no load balancers, no task definitions — a complete production store for roughly the cost of a t3.small and a small RDS instance.

Need auto-scaling, zero-downtime deploys, or CI/CD-driven infrastructure? That's the [ECS Fargate guide](/v5/developer/deployment/aws_ecs) — it runs the same Docker image, so you can start here and graduate later without rework.
...
| **S3 bucket** (recommended) | Uploaded files (product images) — they shouldn't live on the instance disk. See [Asset Storage](/v5/developer/deployment/assets) | — |
...
Spree also runs on RDS MySQL/MariaDB and both Aurora flavors — see [Database Configuration](/v5/developer/deployment/database). RDS takes automated daily backups by default, so the database needs no extra care.
...
Build [your project's image](/v5/developer/deployment/docker) and push it to a registry the instance can pull from (ECR or any other) — or use the stock `ghcr.io/spree/spree:latest` image to start. Then create a `docker-compose.yml` on the instance:
...
The database is migrated automatically on boot. Once DNS resolves, your store is live at `https://store.example.com` — admin at `/admin`, background jobs running inside the web container ([combined mode](/v5/developer/deployment/quickstart#web-and-worker)).
...
This restarts the container with a few seconds of downtime — the point at which that matters is a good signal to consider [ECS Fargate](/v5/developer/deployment/aws_ecs) and its rolling deploys.
...
- **Split the worker** — add a second service to the compose file with `command: bin/jobs`, and set `SOLID_QUEUE_IN_PUMA: "false"` on `web` ([split mode](/v5/developer/deployment/quickstart#web-and-worker))
- **Scale the database** — RDS resizes independently, and read replicas are a console click

## Next Steps

- [Asset Storage](/v5/developer/deployment/assets) — store uploads in S3
- [Set environment variables](/v5/developer/deployment/environment_variables) — SMTP, Sentry, etc.
- [Configure CDN](/v5/developer/deployment/cdn) — CloudFront in front of your assets
...
- [ECS Fargate](/v5/developer/deployment/aws_ecs) — when you outgrow one box
