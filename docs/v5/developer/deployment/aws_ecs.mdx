...
canonical: "https://spreecommerce.org/docs/developer/deployment/aws_ecs"
---

This is the advanced AWS path: containers managed by ECS Fargate with auto-scaling, zero-downtime rolling deploys, CI/CD via GitHub Actions, and secrets in AWS Secrets Manager. If you're getting started or running a single store, the [EC2 + RDS guide](/v5/developer/deployment/aws) gets you to production with a fraction of the moving parts — on the same [Docker image](/v5/developer/deployment/docker), so you can graduate to ECS later without rework.
...
| [RDS](https://aws.amazon.com/rds/) | The database — data, background jobs, and cache all live here. PostgreSQL, MySQL/MariaDB, and both Aurora flavors work — see [Database Configuration](/v5/developer/deployment/database) |
| [ECR](https://aws.amazon.com/ecr/) | Registry for your Docker image |
| [S3](https://aws.amazon.com/s3/) | Uploaded files (product images) — Fargate containers have no persistent disk. See [Asset Storage](/v5/developer/deployment/assets#aws-s3) |
...
| Service | When |
|---------|------|
| [CloudFront](https://aws.amazon.com/cloudfront/) | [CDN](/v5/developer/deployment/cdn) in front of assets and images |
| [ElastiCache](https://aws.amazon.com/elasticache/) | Valkey/Redis for [high-traffic caching](/v5/developer/deployment/caching) — the default cache lives in the database |
| [Route 53](https://aws.amazon.com/route53/) | DNS — convenient on AWS, but any DNS provider works |

## Docker Image

Build [your project's image](/v5/developer/deployment/docker) with `spree build --production`, or use the stock `ghcr.io/spree/spree` image for deployments without customizations.
...
For a full list of available variables, see [Environment Variables](/v5/developer/deployment/environment_variables).
...
Switches to [split mode](/v5/developer/deployment/quickstart#web-and-worker): background jobs move into a dedicated service — same image, no rebuild. When you deploy it, set `SOLID_QUEUE_IN_PUMA=false` in the web task's `environment` so the web container stops running jobs. Save as `.aws/worker-task-definition.json`:
...
The database also stores background jobs and cache — the sizing above accounts for it. Scaling the web service to multiple instances is safe with in-process jobs: instances coordinate through the database. Add ElastiCache (Valkey or Redis) only for [high-traffic caching](/v5/developer/deployment/caching).
...
- [Asset Storage](/v5/developer/deployment/assets) — configure S3 for product images and uploads
- [Configure CDN](/v5/developer/deployment/cdn) — set up CloudFront for asset delivery
- [Configure caching](/v5/developer/deployment/caching) — works out of the box; add ElastiCache for high-traffic installs
- [Set environment variables](/v5/developer/deployment/environment_variables) — SMTP, Sentry, SSL, etc.
- [Deploy the storefront](/v5/developer/storefront/nextjs/deployment) — the Next.js storefront ships separately, typically to Vercel
