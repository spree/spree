...
canonical: "https://spreecommerce.org/docs/developer/deployment/docker"
---

## Building Your Project's Image

Every [create-spree-app](/v5/developer/create-spree-app/quickstart) project builds into a single production image: the web server and background jobs run in one container, and the only dependency is a PostgreSQL database (the [single-node topology](/v5/developer/deployment/quickstart#single-node-vs-distributed)).
...
See [Environment Variables](/v5/developer/deployment/environment_variables) for the full list.
