...
canonical: "https://spreecommerce.org/docs/v5/developer/upgrades/5.5-to-5.6"
---

<Info>
  Before proceeding to upgrade, please ensure you're at [Spree 5.5](/v5/developer/upgrades/5.4-to-5.5).
</Info>
...
<Note>
**Already on 5.6.0 or 5.6.1?** This step was added to the manifest in 5.6.2, so a `spree:upgrade` you ran earlier did not include it. Re-run `spree:upgrade` after bumping (every step is idempotent, so the earlier steps are no-ops), or run just this one:
...
<Warning>
`iframe` is stripped by default. If your descriptions embed videos or other iframes, permit the tag **before** running the task (and before saving products under 5.6.2):
