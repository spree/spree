...
canonical: "https://spreecommerce.org/docs/developer/customization/decorators"
---
...
<Warning>
  **Decorators should be a last resort.** They tightly couple your code to Spree internals and can break during upgrades. Before using decorators, consider these modern alternatives that are safer and easier to maintain:
  ...
  - **[Events & Subscribers](/v5/developer/core-concepts/events)** - For reacting to model changes (after save, create, update, delete)
  - **[Webhooks](/v5/developer/core-concepts/webhooks)** - For notifying external services when events occur
  - **[Dependencies](/v5/developer/customization/dependencies)** - For swapping out services, serializers, and abilities
  - **[Admin Navigation](/v5/developer/admin/navigation)** - For adding menu items without controller decorators
  - **[Admin Partials](/v5/developer/admin/extending-ui)** - For extending admin UI without view decorators
  - **[Admin Tables](/v5/developer/admin/tables)** - For customizing admin list views
  - **[Ransack Configuration](/v5/developer/core-concepts/search-filtering#extending-ransackable-configuration)** - For adding searchable/sortable fields without model decorators
</Warning>
...
|----------|---------------------|----------|
| After-save hooks (sync to external service) | Model decorator with `after_save` | [Events subscriber](/v5/developer/core-concepts/events) |
| Notify external service on changes | Model decorator with callbacks | [Webhooks](/v5/developer/core-concepts/webhooks) |
| Custom add-to-cart logic | Service decorator | [Dependencies injection](/v5/developer/customization/dependencies) |
| Custom API responses | Serializer decorator | [Dependencies injection](/v5/developer/customization/dependencies) |
| Add admin menu item | Controller decorator | [Admin Navigation API](/v5/developer/admin/navigation) |
| Add section to admin form | View decorator/override | [Admin Partials injection](/v5/developer/admin/extending-ui) |
| Add searchable/filterable field | Model decorator with `ransackable_attributes` | [Ransack configuration](/v5/developer/core-concepts/search-filtering#extending-ransackable-configuration) |
...
<Warning>
  **High risk of breaking during upgrades.** When you override an existing Spree action, your code depends on Spree's internal implementation details. If Spree changes the action's behavior, instance variables, or method signatures in a future version, your decorator may silently break or cause unexpected bugs. Use [Events](/v5/developer/core-concepts/events) for post-action side effects instead.
</Warning>
...
<Info>
  **Better alternative:** For post-action side effects like notifications, use [Events subscribers](/v5/developer/core-concepts/events) instead. Subscribe to `product.created` to be notified when products are created, without coupling to controller internals.
</Info>
...
- [Events](/v5/developer/core-concepts/events) - Learn about Spree's event system
- [Webhooks](/v5/developer/core-concepts/webhooks) - HTTP callbacks for external integrations
- [Dependencies](/v5/developer/customization/dependencies) - Swap core services with your own
- [Admin Navigation](/v5/developer/admin/navigation) - Extend admin menu without decorators
- [Admin Partials](/v5/developer/admin/extending-ui) - Extend admin UI without view decorators
- [Extending Core Models Tutorial](/v5/developer/tutorial/extending-models) - Step-by-step guide to connecting custom models with Spree core
- [Customization Overview](/v5/developer/customization/quickstart) - General customization patterns
