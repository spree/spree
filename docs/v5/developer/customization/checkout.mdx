...
version: v5
canonical: "https://spreecommerce.org/docs/v5/developer/customization/checkout"
---
...
The Spree checkout process consists of the following steps. With the exception of the Registration step, each of these steps corresponds to a state of the [Order object](/v5/developer/core-concepts/orders):
...
For more information about payments, please see the [Payments guide](/v5/developer/core-concepts/payments).
...
2. conditionally for specific orders by overriding the `confirmation_required?` method in `Spree::Order` with a [decorator](/v5/developer/customization/decorators), eg.
...
For actions that should happen **after** a checkout step completes (syncing with external services, sending notifications, logging, etc.), use [Events subscribers](/v5/developer/core-concepts/events). This is the recommended approach because it keeps your code decoupled from Spree internals.
...
<Info>
  Events are fired automatically when orders change state. See [Events documentation](/v5/developer/core-concepts/events#order-events) for all available order events.
</Info>
...
<Warning>
  Avoid using state machine callbacks for side effects like API calls or notifications. These can slow down checkout and may fail silently. Use [Events subscribers](/v5/developer/core-concepts/events) instead.
</Warning>
