...
canonical: "https://spreecommerce.org/docs/developer/storefront/nextjs/architecture"
---
...
The `(wholesale)` route group is an opt-in B2B portal, off unless a wholesale channel is configured. Its gating, surfaces, and pricing modes are covered in the [Wholesale Portal](/v5/developer/storefront/nextjs/wholesale) guide.
...
The storefront serves multiple countries, currencies, and languages from one deployment via `/{country}/{locale}` URL segments and an edge middleware that detects and persists the visitor's region. See the [Multi-Region](/v5/developer/storefront/nextjs/multi-region) guide.
