...
canonical: "https://spreecommerce.org/docs/developer/storefront/nextjs/emails"
---

The storefront can own its customer-facing transactional emails instead of the Spree backend. Emails are rendered with [react-email](https://react.email) and sent via [Resend](https://resend.com); the Spree backend delivers order, shipment, and account events to the storefront over [webhooks](/v5/developer/core-concepts/webhooks).
...
For the backend perspective on delivering these events, see [Sending out Emails](/v5/developer/deployment/emails).
