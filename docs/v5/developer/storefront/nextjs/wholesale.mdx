...
canonical: "https://spreecommerce.org/docs/developer/storefront/nextjs/wholesale"
---
...
A **surface** is a distinct sales context backed by its own Spree [channel](/v5/developer/core-concepts/channels). The storefront models two out of the box — `dtc` (the public storefront) and `wholesale` (the trade portal) — but the mechanism is general: any channel on your backend can back its own surface.

The surface selects which SDK client a request goes through. A channel-bound client sends the channel code on every request via the `X-Spree-Channel` header, and the Store API [resolves the request against that channel](/v5/developer/core-concepts/channels#resolution-at-request-time) — its catalog, pricing, and access rules. In the SDK this is the `channel` client option:
...
**Trade pricing** is unlocked by volume. The seeded model grants a trade price once a line reaches a minimum quantity of the same item (10 or more per item in the sample data). The applicable volume rule lives on a backend [Price List](/v5/developer/core-concepts/pricing#price-lists); the storefront surfaces the trade price the API returns for a qualifying quantity.
...
Switching a wholesale channel between the two supported modes is a single change to [`storefront_access`](/v5/developer/core-concepts/channels#storefront-access-gating) on the backend — set on the channel, or inherited from the [store's default](/v5/developer/core-concepts/stores#storefront-access-defaults). Nothing in the storefront needs redeploying — the portal reads the channel's posture at request time and adapts. Login and signup always run through the same approval flow regardless of mode.

## Related Documentation

- [Channels](/v5/developer/core-concepts/channels) — The sales channel that backs the wholesale surface, and the general [Storefront Access Gating](/v5/developer/core-concepts/channels#storefront-access-gating) mechanism the portal builds on
- [Stores](/v5/developer/core-concepts/stores) — Store-level [defaults](/v5/developer/core-concepts/stores#storefront-access-defaults) that channels inherit
- [Pricing](/v5/developer/core-concepts/pricing) — [Price Lists](/v5/developer/core-concepts/pricing#price-lists) and the [Customer Group Rule](/v5/developer/core-concepts/pricing#customer-group-rule) behind approval and trade pricing
- [Store SDK: Configuration](/v5/developer/sdk/configuration) — `setChannel` and the channel client option used to bind a surface
