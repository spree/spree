...
canonical: "https://spreecommerce.org/docs/developer/agentic/overview"
---
...
- **Docs designed for machine consumption** — the full documentation is available as [llms.txt, per-page Markdown, and a local npm package](/v5/developer/agentic/llm-docs), so agents read authoritative content instead of guessing from training data.
- **A guided project workflow** — [create-spree-app](/v5/developer/create-spree-app/quickstart) scaffolds projects with a generated `AGENTS.md` and `CLAUDE.md`, local docs, and the [Spree CLI](/v5/developer/cli/quickstart), so an agent can boot, migrate, generate code, and run tests without bespoke setup.
