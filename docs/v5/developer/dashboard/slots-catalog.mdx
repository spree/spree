...
canonical: "https://spreecommerce.org/docs/developer/dashboard/slots-catalog"
---
...
**Host form:** on pages marked *host form: yes*, the slot renders inside the page's own `<form>` and the form context is exposed — widgets can bind inputs via [`useHostForm()`](/v5/developer/dashboard/recipes/custom-form-field) that hydrate, dirty-track, and save with the page's Save button. On pages without one, widgets own their persistence (use `useOptionalHostForm()` to adapt).
...
- [Slots customization page](/v5/developer/dashboard/customization/slots) — how to register against a slot
