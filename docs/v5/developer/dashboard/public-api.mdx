...
canonical: "https://spreecommerce.org/docs/developer/dashboard/public-api"
---
...
Single declarative entry-point for every registry. See [Customization Quickstart](/v5/developer/dashboard/customization/quickstart) for the full shape.
...
Each registry exposes `add`/`remove`/`update`-style mutators. See the per-feature pages for examples — form fields and custom field components are covered in the [custom form field recipe](/v5/developer/dashboard/recipes/custom-form-field).
...
Vite plugin that resolves third-party dashboard plugins and injects Tailwind `@source` directives for their CSS. See [Distributing](/v5/developer/dashboard/plugins/distributing) for the host wiring.
