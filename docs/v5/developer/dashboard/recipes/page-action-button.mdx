...
canonical: "https://spreecommerce.org/docs/developer/dashboard/recipes/page-action-button"
---
...
This recipe shows both — and how to gate them by permission + resource state. (Strings are hardcoded for brevity; real buttons should go through i18n — see [Translations](/v5/developer/dashboard/customization/translations).)
...
(The slot entry's `if` predicate receives only the slot's own context — `{ resource }` here — so use it for resource-state conditions, and keep permission checks in the component. See [Slots](/v5/developer/dashboard/customization/slots).)
...
The `resource` prop is whatever the host page passed to `<PageHeader resource={…} />`. On the order detail page it's an `Order`; on a product page it's a `Product`. The slot doesn't statically know which — see [Slots](/v5/developer/dashboard/customization/slots) for the typing escape hatches.
...
- [`page.actions` / `page.actions_dropdown`](/v5/developer/dashboard/slots-catalog#page-header-slots) in the slots catalog
...
- [Slots](/v5/developer/dashboard/customization/slots) — registry primer
