...
canonical: "https://spreecommerce.org/docs/developer/dashboard/recipes/attribute-end-to-end"
---
...
It's the full version of the [custom form field recipe](/v5/developer/dashboard/recipes/custom-form-field) — read that one first if you only need the form.

## First: should this be a custom field instead?

**Default to a [custom field definition](/v5/developer/dashboard/recipes/custom-form-field#path-a-custom-field-definition--no-code).** It needs zero code — declare the field, and the product form renders and saves it. That covers most plugin data and everything merchants define themselves.
...
- [Custom form field recipe](/v5/developer/dashboard/recipes/custom-form-field) — the form layer in isolation, and the custom-field-definition path
- [Tables](/v5/developer/dashboard/customization/tables) — the full `ColumnDef` and table registry API
- [Backend integration](/v5/developer/dashboard/customization/backend) — serializers, permitted params, error mapping
- [Decorators](/v5/developer/customization/decorators) — the `prepend` pattern used for the controller
