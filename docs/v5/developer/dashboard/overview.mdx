...
canonical: "https://spreecommerce.org/docs/developer/dashboard/overview"
---
...
→ Start at the [**customization quickstart**](/v5/developer/dashboard/customization/quickstart).
...
- **Peer-dependency rules** so your plugin's `@spree/dashboard-core` resolves to the host's instance (registries are module singletons — see [Publishing](/v5/developer/dashboard/plugins/publishing))
- **Auto-discovery** — the dashboard's build finds every installed dependency carrying the `spree.dashboard.plugin` marker, activates it, and wires up its styling. Installing a plugin is `pnpm add` + a dev-server restart; nothing to edit. See [Distributing](/v5/developer/dashboard/plugins/distributing) for how it works and the explicit-whitelist escape hatch.
- **File routes** — a plugin ships its pages as TanStack file routes that get compiled into the app's route tree, so links to plugin pages are fully type-checked. See [Routes](/v5/developer/dashboard/customization/routes). (In-app customizations use the simpler `routes:` registry instead, as above.)
...
→ Start at the [**plugin scaffolding guide**](/v5/developer/dashboard/plugins/scaffolding).
...
- [Customization quickstart](/v5/developer/dashboard/customization/quickstart)
- [Plugin scaffolding](/v5/developer/dashboard/plugins/scaffolding)
- [Slots catalog](/v5/developer/dashboard/slots-catalog) — every named slot the dashboard exposes
- [`@spree/dashboard-core` README](https://github.com/spree/spree/tree/main/packages/dashboard-core) — full extension API reference
- [Classic Admin extension model](/v5/developer/admin/extending-ui) — for Spree 5.x
