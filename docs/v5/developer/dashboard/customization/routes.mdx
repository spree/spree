...
canonical: "https://spreecommerce.org/docs/developer/dashboard/customization/routes"
---
...
Routes register at module-load time. The dispatcher reads from the registry on **every navigation**, so route registration is much more forgiving than nav: even a route registered after first render works as soon as the user navigates to it. The only ordering rule is the usual i18n one — register your translation bundle above any registration that calls `i18n.t(...)` (see [Translations](/v5/developer/dashboard/customization/translations)).
