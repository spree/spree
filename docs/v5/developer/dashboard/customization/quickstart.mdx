...
canonical: "https://spreecommerce.org/docs/developer/dashboard/customization/quickstart"
---
...
All customizations in this guide are edits to your dashboard app's own code. **You don't need to scaffold a plugin** — plugins are for [distributing features to other stores](/v5/developer/dashboard/plugins/overview).
...
Building a **distributable plugin** rather than customizing your own app? Ship pages as [file routes](/v5/developer/dashboard/customization/routes) instead — they compile into the app's route tree, so links to them are type-checked. The registry form shown here is for in-app customization.

## Next steps

- [Navigation](/v5/developer/dashboard/customization/navigation) — sidebar entries, nesting under built-in menus, settings sub-nav, permission gating
- [Routes](/v5/developer/dashboard/customization/routes) — path patterns, params, permission fallback
- [Slots](/v5/developer/dashboard/customization/slots) — inject widgets into built-in pages without forking them
- [Tables](/v5/developer/dashboard/customization/tables) — define your own list page or extend built-in tables
- [Translations](/v5/developer/dashboard/customization/translations) — add i18n keys without colliding with the framework
