...
canonical: "https://spreecommerce.org/docs/v5/developer/tutorial/events"
---

<Info>
  This guide assumes you've completed the [Model](/v5/developer/tutorial/model) and [API](/v5/developer/tutorial/api) tutorials — we'll reuse the Brand serializer created there.
</Info>
...
Now `brand.created`, `brand.updated`, and `brand.deleted` fire after the matching transactions commit — and because you created `Spree::Api::V3::BrandSerializer` in the [API step](/v5/developer/tutorial/api), the payloads automatically use it (the events system resolves the serializer by naming convention). Anyone — subscriber or webhook — can react to brand changes with the same JSON shape your API serves.
...
- [Events](/v5/developer/core-concepts/events) — the full event catalog, subscriber DSL, and delivery guarantees
- [Webhooks](/v5/developer/core-concepts/webhooks) — endpoint management, security, and the per-event payload schemas
- [Extending Core Models](/v5/developer/tutorial/extending-models) — when a decorator is the right tool instead
