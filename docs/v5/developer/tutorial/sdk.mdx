...
canonical: "https://spreecommerce.org/docs/v5/developer/tutorial/sdk"
---

In this tutorial, we'll use the `@spree/sdk` TypeScript SDK to consume the Brand API endpoints we created in the [API tutorial](/v5/developer/tutorial/api), and work with the extended Product data that now includes brand information.

<Info>
  This guide assumes you've completed the [API](/v5/developer/tutorial/api) tutorial and have the Brand endpoints running.
</Info>
...
Ransack predicates work on whitelisted attributes and associations. The [API tutorial](/v5/developer/tutorial/api) shows how to register `brand_id` and `brand` via `Spree.ransack` — once that's done, you can filter:
...
<Info>
  Ransack predicates like `_eq`, `_cont`, `_gt`, `_lt` work on whitelisted attributes and associations. See [Search & Filtering](/v5/developer/core-concepts/search-filtering) for the full list.
</Info>
...
- [API Tutorial](/v5/developer/tutorial/api) - Creating the Brand API endpoints
- [SDK Overview](/sdk) - Full SDK documentation
- [Search & Filtering](/v5/developer/core-concepts/search-filtering) - Ransack predicates and sorting
- [Agentic Development](/v5/developer/agentic/overview) - Agent skills and the docs MCP server cover the SDK patterns used here
