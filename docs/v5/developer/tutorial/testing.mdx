...
canonical: "https://spreecommerce.org/docs/v5/developer/tutorial/testing"
---
...
<Info>
  This guide assumes you've completed all previous tutorials through [API](/v5/developer/tutorial/api). You should have a complete `Spree::Brand` model with admin features and API endpoints.
</Info>

## Setup

**If your app came from [create-spree-app](/v5/developer/create-spree-app/quickstart) or [spree-starter](https://github.com/spree/spree-starter), the test environment is already set up** — RSpec, Factory Bot, Capybara, DatabaseCleaner, and the `spree_dev_tools` helpers (`stub_authorization!`, the `'API v3 Store'` shared context, Spree factories) ship preconfigured in `spec/support/`. Skip to creating the fixtures file below.

<Accordion title="Setting up an app that doesn't have RSpec yet">
...
If you used `spree generate api_resource` in the [API step](/v5/developer/tutorial/api), you already have controller specs (`spec/controllers/spree/api/v3/store/brands_controller_spec.rb` and `…/admin/brands_controller_spec.rb`) and a factory (`spec/factories/spree/brand_factory.rb`). The sections below build the same things by hand — compare as you go, and keep whichever you prefer.
...
When you extend core Spree models with decorators (see [Extending Core Models](/v5/developer/tutorial/extending-models)), test the added functionality:
...
Test the API endpoints we created in the [API tutorial](/v5/developer/tutorial/api). Store API tests use the `'API v3 Store'` shared context which sets up a store, publishable API key, and JWT tokens — it ships in the `spree_api` gem, so require `spree/api/testing_support/v3/base` at the top of the spec.
...
* [Extending Core Models](/v5/developer/tutorial/extending-models) - Connecting Brands to Products
* [API Tutorial](/v5/developer/tutorial/api) - Creating Brand API endpoints
* [Spree CLI](/v5/developer/cli/quickstart) - `spree rspec` and the other dev workflow commands
