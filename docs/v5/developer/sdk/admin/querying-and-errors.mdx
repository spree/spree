...
canonical: "https://spreecommerce.org/docs/developer/sdk/admin/querying-and-errors"
---
...
When a request fails because the secret API key lacks the required [scope](/v5/developer/sdk/admin/authentication), the error has `code: 'access_denied'` and `details.required_scope` names the missing scope:
...
For cookie-authenticated apps, register an `onUnauthorized` handler to transparently refresh and retry on `401` — see [Authentication](/v5/developer/sdk/admin/authentication#jwt--cookie-authentication-browser-apps).
