...
canonical: "https://spreecommerce.org/docs/developer/sdk/admin/authentication"
---
...
Each key carries the list of scopes granted at creation time. A request to an endpoint the key isn't scoped for fails with `code: 'access_denied'` — the error's `details.required_scope` names the missing scope (see [Querying & Errors](/v5/developer/sdk/admin/querying-and-errors#scope-errors)).
...
`auth.login()` also accepts third-party identity-provider payloads when the server has a [matching strategy registered](/v5/developer/how-to/custom-api-authentication):
