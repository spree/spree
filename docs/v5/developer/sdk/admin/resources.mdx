...
canonical: "https://spreecommerce.org/docs/developer/sdk/admin/resources"
---
...
| `client.customFieldDefinitions` | CRUD on [custom field](/v5/developer/core-concepts/metafields) definitions. |
...
| Client | Endpoints |
|---|---|
| `client.auth` | Login (email/password or identity provider), cookie-driven refresh, logout, invitation lookup/acceptance — see [Authentication](/v5/developer/sdk/admin/authentication). |
...
Resources that support [custom fields](/v5/developer/core-concepts/metafields) (products, variants, orders, customers, categories, option types) expose a `customFields` accessor taking the parent ID first:
...
The resource map grows with your store. The [`spree generate api_resource`](/v5/developer/tutorial/api) generator scaffolds new Admin API endpoints (model, serializers, controllers, routes, specs) in one command — then call them from the SDK with a [custom fetch or raw request](/v5/developer/sdk/extending), or generate a typed client for them.
