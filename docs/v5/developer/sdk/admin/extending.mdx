...
canonical: "https://spreecommerce.org/docs/developer/sdk/admin/extending"
---
...
- The [Store SDK extending guide](/v5/developer/sdk/extending) covers the same pattern for storefront endpoints — the API is identical, only the base path (`/api/v3/store`) and auth header differ.
- The [API](/v5/developer/tutorial/api) and [SDK](/v5/developer/tutorial/sdk) tutorials walk through creating new Spree endpoints end-to-end (Rails controller, serializer, TypeScript types). Everything they show for the Store API applies equivalently for the Admin API.
- For building dashboard UI on top of your custom endpoint, see the [Dashboard](/v5/developer/dashboard/overview) section — start with [Customization Quickstart](/v5/developer/dashboard/customization/quickstart) for in-app changes or [Plugin Overview](/v5/developer/dashboard/plugins/overview) for redistributable packages.
