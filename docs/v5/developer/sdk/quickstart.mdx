...
canonical: "https://spreecommerce.org/docs/developer/sdk/quickstart"
---

`@spree/sdk` is the customer-facing SDK — products, carts, checkout, and account flows against the Store API. For back-office integrations (managing products, orders, customers, stock), use the [Admin SDK](/v5/developer/sdk/admin/quickstart).
...
Building your storefront with an AI coding agent? The [Spree agent skills](/v5/developer/agentic/agent-skills) include dedicated SDK and storefront skills (typed clients, Ransack filtering, error handling, custom endpoints), and the [docs MCP server](/v5/developer/agentic/mcp) gives your agent the full SDK reference on demand. See [Agentic Development](/v5/developer/agentic/overview).
