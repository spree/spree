...
canonical: "https://spreecommerce.org/docs/developer/core-concepts/addresses"
---
...
- [**Markets**](/v5/developer/core-concepts/markets) group countries into selling regions with their own currency and locale
...
During checkout, you can either reference a saved address by ID or pass a new address inline. See the [cart and checkout SDK guide](/v5/developer/sdk/store/cart-checkout) for the full flow of passing addresses during checkout:
...
Countries are the foundation of Spree's geographic system. They connect to [Markets](/v5/developer/core-concepts/markets), contain states, and belong to zones.
...
Only countries assigned to a [Market](/v5/developer/core-concepts/markets) are available during checkout. This lets you control exactly where you sell.
...
States are fetched via the country endpoint using [`?expand=states`](/api-reference/store-api/relations):
...
Zones group countries or states together for [tax](/v5/developer/core-concepts/taxes) and [shipping](/v5/developer/core-concepts/shipments) rules. A zone is either **country-based** or **state-based**.
...
1. Which **tax rates** apply (see [Taxes](/v5/developer/core-concepts/taxes))
2. Which **shipping methods** are available (see [Shipments](/v5/developer/core-concepts/shipments))
...
Each [Market](/v5/developer/core-concepts/markets) resolves a tax zone from its default country. This means product prices display the correct tax treatment (inclusive or exclusive) before the customer enters an address — just from knowing their market.
...
- [Markets](/v5/developer/core-concepts/markets) — Multi-region commerce with currency, locale, and country grouping
- [Taxes](/v5/developer/core-concepts/taxes) — How zones and addresses affect taxation
- [Shipments](/v5/developer/core-concepts/shipments) — How zones and addresses affect shipping availability
- [Orders](/v5/developer/core-concepts/orders) — Order billing and shipping addresses
- [Account SDK guide](/v5/developer/sdk/store/account) — Managing the logged-in customer's address book with `client.customer.addresses`
- [Store API authentication](/api-reference/store-api/authentication) — How to obtain and send the customer JWT (`Authorization: Bearer` header)
