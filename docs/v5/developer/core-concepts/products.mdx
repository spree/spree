...
canonical: "https://spreecommerce.org/docs/developer/core-concepts/products"
---
...
<Info>
Product names, descriptions, slugs, and SEO fields are [translatable](/v5/developer/core-concepts/translations#resource-translations).
</Info>
...
The examples above use the **Store API** (publishable key, read-only, customer-facing). To **create and manage** products, use the [Admin API](/api-reference/admin-api/introduction) — via the [Admin SDK](/v5/developer/sdk/admin/quickstart) or the [Spree CLI](/v5/developer/cli/admin-api).
...
Each variant can have multiple prices — one per currency, plus additional prices from [Price Lists](/v5/developer/core-concepts/pricing) that apply conditionally based on market, geography, customer segment, or quantity.
...
See the [Pricing](/v5/developer/core-concepts/pricing) guide for details on Price Lists, Price Rules, and market-specific pricing.
...
A product is visible on a [Channel](/v5/developer/core-concepts/channels) only when a `ProductPublication` record joins the two. Publications carry an optional time window so a product can be scheduled to go live and come down without code or manual toggles.
...
For Admin API filtering across channels (back-office reports, admin UI lists), use Ransack instead: `q[channels_id_in][]=ch_xxx`. See [Sales Channels](/v5/developer/core-concepts/channels) for the resolution rules.
...
See [Sales Channels](/v5/developer/core-concepts/channels) for the full channel lifecycle, including default-channel resolution and the `X-Spree-Channel` header.

## Related Documentation

- [Sales Channels](/v5/developer/core-concepts/channels) — Channels, publications, and order attribution
- [Pricing](/v5/developer/core-concepts/pricing) — Price Lists, Price Rules, and market-specific pricing
- [Inventory](/v5/developer/core-concepts/inventory) — Stock management and backorders
- [Media](/v5/developer/core-concepts/media) — Image management
- [Translations](/v5/developer/core-concepts/translations) — Translating product content
- [Search & Filtering](/v5/developer/core-concepts/search-filtering) — Full-text search and Ransack filtering
- [Store SDK Products](/v5/developer/sdk/store/products) — Listing, fetching, filtering, and categories via `client.products`
