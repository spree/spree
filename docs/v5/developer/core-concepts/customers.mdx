...
canonical: "https://spreecommerce.org/docs/developer/core-concepts/customers"
---
...
<Note>
The Store API flow above is **customer self-registration** — the customer sets a password and gets a JWT back. To [create a customer record from the back office](/v5/developer/sdk/admin/resources) (importers, CRM sync, phone orders), use the [Admin SDK](/v5/developer/sdk/admin/quickstart) instead. No password is set, so it's a managed record rather than a sign-up:
...
The response includes a JWT `token` and a `user` object. Pass the token in subsequent requests via the [`Authorization: Bearer <token>`](/api-reference/store-api/authentication) header.

## Token Refresh

Refresh an expiring token to keep the session alive. For how `client.auth.login` and `client.auth.refresh` fit into configuring the SDK for guest vs customer (JWT) auth, see the [SDK authentication guide](/v5/developer/sdk/authentication):
...
This fires a `customer.password_reset_requested` event with the reset token in the payload. If you're using the `spree_emails` package, the email is sent automatically. Otherwise, subscribe to this event to send the reset email yourself (see [Events](/v5/developer/core-concepts/events)).
...
|----------|-------------|
| [**Addresses**](/v5/developer/core-concepts/addresses#customer-address-book) | Billing and shipping addresses with default selection |
| [**Orders**](/v5/developer/core-concepts/orders#order-history) | Past order history |
...
Customers don't need to register to purchase. Guest checkout uses an order token (`X-Spree-Token`) to identify the cart. See [Orders — Cart](/v5/developer/core-concepts/orders#cart) for details.

## Related Documentation

- [Account (Store SDK)](/v5/developer/sdk/store/account) — Task-oriented walkthrough of registration, login, profile, addresses, and order history
- [Addresses](/v5/developer/core-concepts/addresses) — Customer address management
- [Orders](/v5/developer/core-concepts/orders) — Order history and checkout
- [Authentication](/v5/developer/customization/authentication) — Custom authentication setup
- [Staff & Roles](/v5/developer/core-concepts/staff-roles) — Admin users and permissions
