...
canonical: "https://spreecommerce.org/docs/developer/core-concepts/inventory"
---
...
Each [Variant](/v5/developer/core-concepts/products#variants) has a `StockItem` that tracks its inventory at a specific location. A variant can have multiple stock items if it's available at multiple stock locations.
...
- **Inventory Unit** → Represents individual units in [Orders](/v5/developer/core-concepts/orders) and [Shipments](/v5/developer/core-concepts/shipments)
...
Create and manage stock locations via the [Admin API](/api-reference/admin-api/introduction):
...
Create a transfer via the [Admin API](/api-reference/admin-api/introduction), listing the variants and quantities to move. Omit `source_location_id` to record an incoming receipt from a vendor:
...
- [Products](/v5/developer/core-concepts/products) - Product and variant management
- [Shipments](/v5/developer/core-concepts/shipments) - How inventory relates to shipments
- [Orders](/v5/developer/core-concepts/orders) - How inventory is allocated to orders
- [Admin SDK resources](/v5/developer/sdk/admin/resources) - `stockLocations`, `stockItems`, and `stockTransfers` methods used in the examples above
- [Admin API authentication](/api-reference/admin-api/authentication) - How to obtain and scope the secret key (`sk_xxx`) used by these calls
