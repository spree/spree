...
canonical: "https://spreecommerce.org/docs/developer/core-concepts/media"
---
...
| `video_provider` | string \| null | `youtube` or `vimeo`, derived from the link <Since version="6.0" /> |
| `video_embed_url` | string \| null | Embeddable player URL, derived from the link <Since version="6.0" /> |
| `video_url` | string \| null | Uploaded video file URL <Since version="6.0" /> |
| `poster_url` | string \| null | Still frame for a video <Since version="6.0" /> |
...
<Info>
For production deployments, use cloud storage (S3, GCS, Azure) instead of local disk storage. See [Asset Deployment](/v5/developer/deployment/assets) for configuration details.
</Info>
...
- [Products](/v5/developer/core-concepts/products) — Product catalog and media
- [Products & Categories](/v5/developer/sdk/store/products) — Store SDK guide for fetching products, `thumbnail_url`, and expanded media
- [Admin SDK](/v5/developer/sdk/admin/resources) — Admin SDK resource methods, including the nested `products.media` create/update calls
- [Deployment — Assets](/v5/developer/deployment/assets) — Storage and CDN configuration
