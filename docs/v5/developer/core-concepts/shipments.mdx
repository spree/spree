...
canonical: "https://spreecommerce.org/docs/v5/developer/core-concepts/shipments"
---
...
**Key relationships:**
- **Shipment** tracks delivery of items from a [Stock Location](/v5/developer/core-concepts/inventory#stock-locations)
- **Shipping Method** defines the carrier/service (UPS, FedEx, etc.)
- **Shipping Rate** represents the calculated cost for a method
- **[Zone](/v5/developer/core-concepts/addresses#zones)** defines geographic regions for shipping availability
...
During checkout, after the customer provides a shipping address, Spree calculates available shipping rates for each shipment. The customer must select a rate before proceeding. See the [Cart & Checkout SDK](/v5/developer/sdk/store/cart-checkout) guide for the full storefront cart and fulfillment flow.
...
- **[Zones](/v5/developer/core-concepts/addresses#zones)** — geographic regions where the method is available
- **Shipping Categories** — product groups the method handles
- **[Stores](/v5/developer/core-concepts/stores)** — which stores offer this method
...
Each shipping method uses a [Calculator](/v5/developer/core-concepts/calculators) to determine the cost. Spree includes these built-in calculators:
...
You can create custom calculators for more complex pricing. See the [Calculators guide](/v5/developer/core-concepts/calculators).

## Order Routing <Since version="5.5" />

When an order moves from cart to checkout, Spree decides which [Stock Location](/v5/developer/core-concepts/inventory#stock-locations) fulfills it. **Order Routing** is the system that makes that decision — driven by configurable rules so merchants can express preferences like "fulfill from the customer's preferred warehouse first," "minimize the number of split shipments," or "always pick the closest location."
...
See the [Build Custom Order Routing](/v5/developer/how-to/custom-order-routing) guide for both.
...
To add custom splitting logic — refrigerated SKUs, gift wrap separation, bulky-vs-small bin separation, anything that needs to fan one location's allocation into multiple shipments — write a new subclass of `Spree::Stock::Splitter::Base`. See the [Build Custom Stock Splitter](/v5/developer/how-to/custom-stock-splitter) guide.
...
- [Orders](/v5/developer/core-concepts/orders) — Checkout flow and shipping rate selection
- [Inventory](/v5/developer/core-concepts/inventory) — Stock locations and inventory management
- [Calculators](/v5/developer/core-concepts/calculators) — Shipping rate calculators
- [Addresses](/v5/developer/core-concepts/addresses) — Shipping address and zones
- [Build Custom Order Routing](/v5/developer/how-to/custom-order-routing) — Custom rules and strategies for choosing the fulfillment location
- [Events](/v5/developer/core-concepts/events) — Subscribe to shipment events (e.g., `shipment.shipped`)
- [Admin API](/api-reference/admin-api/introduction) — Inspect and manage shipments, shipping methods, and categories from the back office
