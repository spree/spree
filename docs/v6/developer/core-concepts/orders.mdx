...
An order is a placed purchase. It's created by completing a [Cart](/v6/developer/core-concepts/carts) and holds its own copy of everything the customer agreed to — items, prices, taxes, discounts and delivery.
...
| `number` | Order number shown to customers, e.g. `R123456789` |
...
| Value | Meaning |
|---|---|
| `pending` | Not ready to send yet |
| `ready` | Everything in stock and paid for |
...
const orders = await adminClient.orders.list({
  filter: { payment_status_eq: 'paid', fulfillment_status_eq: 'ready' },
})
...
Cancelling records who cancelled and why, puts stock back, and settles any payments.
...
Tax, discounts and fees are kept as separate records, so you can ask what tax was charged without picking through a mixed list. See [Taxes, Discounts & Fees](/v6/developer/core-concepts/taxes-discounts-fees).
...
- [Carts](/v6/developer/core-concepts/carts) — shopping and checkout
- [Fulfillments](/v6/developer/core-concepts/fulfillments) — getting items to the customer
- [Returns, Exchanges & Claims](/v6/developer/core-concepts/returns-exchanges-claims) — after the sale
