...
never un-ships itself — see [Fulfillments](/v6/developer/core-concepts/fulfillments#statuses).

Translate the carrier's vocabulary into Spree's in your gem, then hand it over:

```ruby
Spree.fulfillment_update_tracking_workflow.call(
  fulfillment: fulfillment,
  tracking_status: 'out_for_delivery',   # one of Spree::Fulfillment::TRACKING_STATUSES
  estimated_delivery_at: Time.parse(payload['eta']),
  details: payload.slice('status_detail', 'carrier')
)
```

Two things it does for you: reporting `delivered` also confirms receipt on the
fulfillment (setting `delivered_at`, which return windows count from), and an
update that only carries a scan leaves an earlier estimate alone.

Pass `delivered_at` from the carrier's delivering scan rather than letting it
default to now — a webhook usually lands well after the parcel arrives, and the
customer's return window starts at the doorstep.

Do the translation in your gem, never in core: `unknown` keeps an unrecognised
report visible instead of dropping it. A webhook endpoint should answer `200`
even for parcels it does not recognise, or the carrier will retry a payload
