...
Optionally declare `def self.service_catalog(integration)` returning `[{ carrier:, service:, label: }]` — the admin service picker renders it as a checkbox list. Without a catalog the merchant enters carrier/service identifiers free-form.
