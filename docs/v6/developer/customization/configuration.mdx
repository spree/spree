...
| `seller_panel_url` | `SPREE_SELLER_PANEL_URL` | Origin where the marketplace seller panel is hosted (e.g. `https://sellers.my-shop.com`). Used for links sent to sellers, today the invitation email. Leave it unset on a marketplace with no seller panel — those links then open the dashboard. |
