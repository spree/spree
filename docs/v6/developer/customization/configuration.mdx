...
| `on_dispatch` | The amount is reserved at checkout and taken when the goods are dispatched. |
| `manual` | The amount is reserved at checkout and staff take it when they choose. |
...
Before Spree 6 this was two separate booleans, `auto_capture` and `auto_capture_on_dispatch`, which could contradict each other. The upgrade task maps them onto the new setting.
