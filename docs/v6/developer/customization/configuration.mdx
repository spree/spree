...
|---|---|
| `send_core_emails` | Spree only sends an email when a mailer is called. Remove the mailer call, or override the mailer, to stop one being sent. |
...
| `layout`, `logo`, `mailer_logo`, `always_put_site_name_in_title`, `title_site_name_separator`, `max_level_in_taxons_menu`, `show_raw_product_description`, `shipping_instructions`, `show_only_complete_orders_by_default` | These configured the removed Rails storefront and admin. A storefront owns its own presentation. |
