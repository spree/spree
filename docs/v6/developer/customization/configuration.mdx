...
Settings that shape how a shop sells — when cards are charged, whether stock is counted, which products appear without a price — are properties of a store, not of the installation, so they live on `Spree::Store` and are edited in the dashboard under **Settings → Store**:

`auto_capture`, `auto_capture_on_dispatch`, `allow_checkout_on_gateway_error`, `track_inventory_levels`, `stock_reservations_enabled`, `track_price_history`, `show_products_without_price`, `disable_sku_validation`, `address_requires_phone`, `company_field_enabled`.
...
```ruby
Spree::Current.store.preferred_auto_capture           # => true
...
| `auto_capture` | The `auto_capture` setting on the store |
| `auto_capture_on_dispatch` | The `auto_capture_on_dispatch` setting on the store |
| `allow_checkout_on_gateway_error` | The `allow_checkout_on_gateway_error` setting on the store |
