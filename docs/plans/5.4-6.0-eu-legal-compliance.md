...
8. **Subject access requests are asynchronous and leave a record** (2026-09-02). `POST /store/customers/me/data_export` enqueues a job, answers `202`, and emails a signed link to a privately-stored file; a second request while one is pending returns the pending one instead of starting a second build. A new `Spree::DataRequest` model carries it, not a `Spree::Export` subclass — that pipeline is admin-owned, CSV-only and store-scoped, and a customer's JSON subject export is none of those. The pending record is the cooldown, so no separate rate limit is needed. This resolves Open Question 1.
