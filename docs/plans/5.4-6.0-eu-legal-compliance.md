...
**Status:** Implemented — Omnibus half shipped earlier (`Spree::PriceHistory` + recording callback + `?expand=prior_price`); GDPR subject requests, consent records and the withdrawal window implemented 2026-09-02, in review as spree/spree#14560
**Target:** Spree 5.4 (Omnibus — shipped), Spree 6.0 (GDPR + Consumer Rights)
...
8. **Subject access requests are asynchronous and leave a record** (2026-09-02). `POST /store/customers/me/data_requests` enqueues a job, answers `202`, and emails a signed link to a privately-stored file; a second request while one is pending returns the pending one instead of starting a second build. A new `Spree::DataRequest` model carries it, not a `Spree::Export` subclass — that pipeline is admin-owned, CSV-only and store-scoped, and a customer's JSON subject export is none of those. The pending record is the cooldown, so no separate rate limit is needed. This resolves Open Question 1.

9. **Both subject-request endpoints ship in open source, at both ends** (2026-09-02). `POST /admin/customers/:id/anonymize` and `GET /admin/customers/:id/export` are core, gated on `write_customers`. Erasure and access requests arrive as email to the merchant, from people who often cannot log in and from guests who never had an account — a self-service-only build serves the rare case and leaves the ordinary one to the Rails console. Enterprise keeps the request queue, the Art. 12(3) deadline clock, the audit trail, automated retention and per-market legal text. This resolves Open Question 2.

10. **The withdrawal period is its own market preference, anchored on delivery** (2026-09-02). `withdrawal_period_days` (default 14) sits beside `return_window_days` rather than reusing it: the statutory right runs from *receipt* of the goods, a return window is merchant policy running from purchase, and merchants routinely set the latter to 30 days as goodwill. The deadline computes from the order's latest `delivered_at`, falling back to `completed_at` while nothing has been delivered.

11. **No encryption of PII at rest in this plan.** Application-level PII encryption is a separate concern (and often handled at the infrastructure level with encrypted-at-rest databases). This plan focuses on data lifecycle management, not storage encryption.
...
- **Don't add PII to order metadata without considering anonymization.** If storing customer-identifiable data in `metadata` on orders, ensure the anonymizer clears it.
- **A new table carrying customer-identifiable data is covered by `Spree::Customers::Anonymize` in the same change that introduces it.** A schema guard spec fails when a PII-shaped column appears without the anonymizer reaching it. Adding the column and deferring the erasure path is how a store silently becomes non-compliant.
- **`scramble_email_and_names` is a deprecated shell.** It delegates to the anonymizer and is removed in 6.1; no new caller.
- **Never flip `accepts_email_marketing` with `update_column`/`update_all`.** The consent timestamp and source are written by the model callback, and a raw write leaves a consent state with no proof of when it was given.
- **The withdrawal deadline is read, never stored.** It derives from the order's market and its fulfillments' `delivered_at`; a persisted copy would go stale the moment a parcel is delivered. A partly-delivered order has **not** started its clock — the period runs from the last parcel, so anything reading a max over the delivered subset would expire the right while the buyer is still waiting.
- **Never record consent that was not given.** A `ConsentRecord` is evidence; writing one because an account was created, rather than because a box was ticked, manufactures proof of an agreement the buyer never made. Terms consent is recorded only when the caller passes `terms_of_service`.
- **The export download link is a credential.** It must not enter an event payload, a webhook body or the delivery log — `Spree::Api::V3::DataRequestEventSerializer` is what `data_request.*` publishes, and `download_url` is on the webhook redaction list as defence in depth.

## Open Questions

None. Questions 1 (export rate limiting) and 2 (admin anonymization in core vs enterprise) were resolved 2026-09-02 — see Key Decisions 8 and 9.
