...
**Status:** In Progress — implemented in 5.5 (PR #13978); the 6.0 refinements (Cart/Order split integration, `allocated_count` term) are still pending
...
- **Configurable trigger** — ~~`Spree::Config[:reserve_stock_on]` controls when reservations are created: `:checkout` (when entering checkout, default) or `:cart` (when adding to cart). Cart-level is more aggressive, checkout-level is the standard.~~ **Superseded (2026-08-04):** the Cart/Order split wired `Reserve` directly into the cart flows (`Carts::AddItem`, `Carts::SetQuantity`, `Carts::RemoveLineItem`, `Carts::Update`), gated only on `Spree::Config[:stock_reservations_enabled]`. A usage audit found the trigger switch has no read sites anywhere; the short TTL already bounds cart-level aggressiveness. `reserve_stock_on` is retired via `6.0-store-scoped-configuration.md` (deprecated shell in 6.0, deleted in 6.1).
- **Configurable TTL per store, with global default** — `store.preferred_stock_reservation_ttl_minutes` overrides `Spree::Config[:default_stock_reservation_ttl_minutes]` (default: 10). *(2026-08-04: the global fallback proved unreachable — the Store preference carries its own `default: 10` and a `greater_than: 0` validation, so it is never blank. `6.0-store-scoped-configuration.md` deprecates the global; `ttl_for` keeps its hard 10-minute floor for storeless contexts.)* Resolved by `Spree::StockReservation.ttl_for(order)`. **TTL deliberately lives on Store, not StockLocation** — it's a checkout-experience policy, not a warehouse property. A multi-location cart never has to merge conflicting TTLs from different warehouses. If future merchants need differentiation, the natural next move is splitting TTL by guest vs authenticated customer on the Store, not by location. BOPIS lead time belongs on `StockLocation#pickup_ready_in_minutes` (see `6.0-fulfillment-and-delivery.md`), which is a different concept.
...
Scheduled via Solid Queue's `config/recurring.yml` — runs every 1 minute. (Mechanism standardized by `6.0-solid-queue-single-container.md`; sidekiq-cron is not part of the default stack.)
...
```ruby
# 6.0: both move to Store preferences — see 6.0-store-scoped-configuration.md.
# `reserve_stock_on` is retired (never read); the TTL global is deprecated
# because the Store preference below always wins.
store.preferred_stock_reservations_enabled = true          # default: true
