...
- **Configurable TTL per store, with global default** — `store.preferred_stock_reservation_ttl_minutes` overrides `Spree::Config[:default_stock_reservation_ttl_minutes]` (default: 10). Resolved by `Spree::StockReservation.ttl_for(order)`. **TTL deliberately lives on Store, not StockLocation** — it's a checkout-experience policy, not a warehouse property. Matches Shopify (tenant-wide), Medusa (global), Saleor (per-channel), Vendure (per-shop). A multi-location cart never has to merge conflicting TTLs. If future merchants need differentiation, the next move is Saleor's split (guest vs authenticated TTL on Store), not a per-location knob. BOPIS lead time belongs on `StockLocation#pickup_ready_in_minutes` (see `6.0-fulfillment-and-delivery.md`), which is a different concept.
...
The same shape works for both 5.5 and 6.0. In 5.5 `Spree::StockItem#allocated_count` is a Ruby shim returning 0, so `available_stock` numerically equals `SUM(count_on_hand)`. When 6.0 Typed Stock Movements (`6.0-typed-stock-movements.md`) replaces the shim with a real column, the formulas continue to mean exactly what their names say.
...
class Spree::StockItem
  # 5.5 shim — 6.0 Typed Stock Movements replaces this with a real column.
  def allocated_count
    0
  end
