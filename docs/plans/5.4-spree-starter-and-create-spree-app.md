...
> **Partially superseded by `5.5-project-layout-and-dashboard.md`:**
> - `backend/` directory renamed to `api/` for new projects scaffolded on 5.5+. Existing 5.4 projects keep `backend/` and can opt in via `spree upgrade layout`.
> - `apps/admin/` reference for the future React admin (mentioned in this plan's Problem #5 and Design Details) is replaced by `apps/dashboard/`, matching the `@spree/dashboard` package name.
