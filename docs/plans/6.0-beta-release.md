...
3. **Every Developer-Preview package goes to `1.0.0-beta.1`: Spree 6 = 1.0.0
   for all of them.** `@spree/admin-sdk` (0.8.1), the dashboard trio (0.13.1)
   and the two seller packages (0.1.0) leave 0.x together and are promoted to
   stable `1.0.0` at 6.0 GA. One version line across the npm surface, matching
   the gems' own 6.0 line, instead of five unrelated 0.x counters nobody can
   map to a Spree release.

   The form is `1.0.0-beta.1`, **not** `1.0.0.pre` — npm requires a hyphen and
   rejects `1.0.0.pre` outright (that spelling is RubyGems'). They publish
   under the existing `next` tag, which the release jobs already select for any
   version containing `-`.

   This changes what the embedded starter templates pin. `publishedRange()` in
   `scripts/sync-starter.mjs` emits `>=x.y.z <2.0.0` only for `0.x`; at
   `1.0.0-beta.1` it emits `^1.0.0-beta.1`, which accepts the beta now and
   floats to stable `1.0.0` when it lands — the intended behaviour, no change
   needed. Verified with semver.

   `@spree/sdk` is the exception and stays on its own line (Decision 2): it has
   a published 1.x that means the 5.x Store API, so it goes to `2.0.0-beta.1`.

   Use `beta.1`, never `beta1`: semver compares dot-separated prerelease
   identifiers numerically but bare strings alphabetically, so `1.0.0-beta9`
   sorts ABOVE `1.0.0-beta10` and a release would ship backwards. This is the
   one place the npm and RubyGems spellings must differ — `6.0.0.beta1` is
   correct for the gems, where the trailing digits parse as their own numeric
   segment. `@spree/cli` already shipped `2.0.0-beta` → `-beta.7`, so the
   dotted form is this repo's existing precedent, not a new convention.

   **Accepted break:** the dashboard trio currently publishes to `next` at
   `0.13.1`. At `1.0.0-beta.1` the release jobs route it to `beta` instead
   (any version containing `-`), so anyone installing `@spree/dashboard@next`
   stops receiving updates and must move to `@beta`. Call this out in the
   release notes — the Developer Preview audience is small and reachable.
...
4. **Refresh the Developer-Preview notes in the package READMEs.**
   `@spree/dashboard` and `@spree/dashboard-ui` both say APIs "may change
   between 0.x releases", which stops being true at `1.0.0-beta.1`; the seller
   packages' notes already name the `beta` tag. Reword them to the beta line
   (published under `beta`, promoted to `latest` at GA) in the same change as
   the version bump, so npm never shows install instructions that contradict
   the published tag.

5. **Version and publish the npm packages.** Clear the changeset backlog with
   `pnpm changeset version` (plain — *not* `version:preview`, which carries
   `--ignore @spree/sdk`), then set the versions by hand: `1.0.0-beta.1` for
   `@spree/admin-sdk`, the dashboard trio, `@spree/seller-sdk` and
   `@spree/seller-dashboard`; `2.0.0-beta.1` for `@spree/sdk`. Changesets has
   no first-class prerelease-major path that plays well with these exceptions,
   so the edit is deliberate and reviewed. Merge to main and let the release
   chain publish — `admin-sdk` and `seller` run in parallel, both gating
   `release-cli`, which gates `create-spree-app`.

6. **Merge `6-0-dev` into `main`** on `spree/spree-starter` and
...
7. **Seller panel distribution** (done): `sync-dashboard-starter.mjs` is
   generalized to `sync-starter.mjs <target> [starter-package]` and the CLI
   build renders both templates; `spree add` gained a `seller-dashboard`
   component (`addDashboard` → app-parameterized `addApp`, with the unit
   battery now running against both apps); `create-spree-app` scaffolds
   `apps/dashboard` **and** `apps/seller-dashboard`; the starter's Dockerfile
   gained a mirrored `seller-dashboard` build stage (custom
   `apps/seller-dashboard/` when present, stock template otherwise) that builds
   with `VITE_BASE_PATH=/sellers/` and sets `SPREE_SELLER_PANEL_DIST_PATH`; and
   `packages.yml` gained a `release-seller` job publishing `@spree/seller-sdk`
   then `@spree/seller-dashboard`, with `release-cli` gated on it. The
   Dockerfile change lives in the starter repo and rides the same merge as
   step 2.

8. **Smoke test the real path** — `npx create-spree-app@<candidate>` into a
...
9. **Announce as beta**, naming the install commands explicitly: gems need
...
- `scripts/sync-starter.mjs` — embedded template version flooring
