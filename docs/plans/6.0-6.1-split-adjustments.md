...
**Status:** 6.0 implementation shipped (Phases 0–4 + data migration on main, verified 2026-08-05); remaining: v6 developer docs (pre-release), then Phase 5 cleanup + stacking in 6.1
...
- **v6 developer docs** (before 6.0 release): typed-model coverage lives under `docs/v6/developer/` (`core-concepts/taxes-discounts-fees.mdx` exists; promotions core-concepts + custom-promotion how-to variants needed). The v5 docs under `docs/developer/` stay untouched until release — they document the stable 5.x behavior (decision 2026-08-05).
...
- Factories + specs (~65 files). Docs: typed-model coverage goes in `docs/v6/developer/` (`core-concepts/taxes-discounts-fees.mdx`, promotions core-concepts + custom-promotion how-to variants); the v5 `docs/developer/` pages stay untouched until the 6.0 release
