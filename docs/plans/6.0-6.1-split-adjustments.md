...
**Status:** Implemented for 6.0 (Phases 0–4 + data migration + v6 developer docs, verified 2026-08-05); Phase 5 cleanup + stacking are 6.1
...
**2026-08-05 additions (from the `6.0-tax-provider.md` contract refinement, driven by the spree/spree#14056 review):** `taxability_reason` (string, nullable — machine-readable cause of the treatment: `standard_rated`, `zero_rated`, `reverse_charge`, `intra_community_supply`, `export`, `customer_exempt`, …; extensible vocabulary + reason→UNTDID 5305/VATEX lookup maps live on the tax-provider plan) and the jurisdiction snapshot `country_code` / `state_code` (the taxing jurisdiction of the line — OSS returns group per member state of consumption, and origin-taxed cases make it non-derivable from ship-to). EN 16931 category/exemption codes are deliberately NOT columns — derivable at invoicing time from the reason plus order facts.
...
**CreateLineItems** writes a line-level `Discount` covering the gifted units of the line it adds (`amount = -price * gifted_quantity`), so the gift costs nothing without a second action paying for it. It is the one action whose discount the promotion's rules do not narrow: the gift is rarely the product the rules name, so the rules decide whether the promotion applies and the action decides which line it pays for, through `PromotionAction#applies_to_line_item?`.
...
5. **Order-level fees are legal** (both adjustable FKs nil). At seller split they prorate across child orders by item totals.
