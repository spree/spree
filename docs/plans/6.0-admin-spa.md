...
| `<PageBreadcrumbs>` | Real breadcrumbs derived from route matches; replaces the static placeholder in `$storeId.tsx`. | `shared/_breadcrumbs.html.erb` |
...
- ⏳ Tier 1 components (`<PageHeader>`, `<PageTabs>`, `<PageBreadcrumbs>`, `<StickyFormFooter>`, `<ResourceLayout>`, `<EmptyState>`, `<RouteErrorBoundary>`)
