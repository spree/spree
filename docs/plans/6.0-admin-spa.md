...
2. **`useTranslation()` + `admin.fields.<resource>.<attribute>.{label,placeholder,help}` keys** in `packages/admin/src/locales/en.json` keep labels translatable.
...
```jsonc
// packages/admin/src/locales/en.json
{
  "admin": {
    "fields": {
      "name":        { "label": "Name" },
...
```ts
// packages/admin/src/lib/form-errors.ts
...
import { registerSlot } from '@spree/admin/lib/slot-registry'
import type { ProductFormSlotContext } from '@spree/admin/components/spree/products/slots'
...
import { Card, CardContent, CardHeader, CardTitle } from '@spree/admin/components/ui/card'
import { Field, FieldError, FieldLabel } from '@spree/admin/components/ui/field'
...
- Reference plugin example in `packages/admin/examples/` (a no-op "extra field" plugin so plugin authors have something to copy)
