...
**Sign-in:**
- `no_store_access` — replaces the screen shown to a signed-in admin who holds a role on no store. Context: `{ user, signOut }`. Name and context type exported from `@spree/dashboard-core` (`NO_STORE_ACCESS_SLOT`, `NoStoreAccessSlotContext`).
