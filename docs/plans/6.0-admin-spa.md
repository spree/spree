...
**Status:** In Progress — the 2026-07-08 header understated reality; re-audited 2026-08-27 against the code. Phases 1 and 2 are done (page chrome, slot registry, nav registry, `useResourceMutation`, per-resource hooks normalized into `src/hooks/`), Phase 4 is largely done (75 route files across products, orders, customers, promotions, stock, categories, collections, settings, roles), and Phase 6's first step is done — the `spree/admin` engine no longer exists in the monorepo. **Remaining:** `<StickyFormFooter>` and `<EmptyState>` (Phase 2); the `<CustomFieldsDrawer>`, `<TranslationsDrawer>` and `<AuditLogDrawer>` (Phase 3 — `<JsonPreviewDrawer>` and the internal-note card shipped); most of Phase 5 (`<SavedViews>`, `<FilterChips>`, `<NotificationCenter>`, `<KeyboardShortcuts>`, export/import surfaces — the command palette, bulk action bar and plugin loader shipped); Phase 4's reports and exports pages, which have no route at all (their Admin API endpoints are pending too — see `6.0-admin-api.md`); and Phase 6's tail — `create-spree-app` installing `@spree/dashboard` by default, plus the docs sweep
**Target:** Spree 6.0
...
The dashboard is split into three workspace packages with a strict one-way dependency direction. The split lets plugin authors compose new pages from the same primitives core uses, and lets app developers build bespoke admin variants (seller panels, white-labels) without forking the SPA.
...
We are not extracting per-feature packages (`@spree/dashboard-orders`, …). The competitor evidence is unanimous that this is the wrong cut, and the seller-panel use case is better solved by forking the app shell with reuse of `dashboard-ui`/`dashboard-core` than by feature imports. Revisit only if a real consumer hits the boundary.
...
Custom admin apps (seller panels, etc.) compose the packages directly: import `@spree/dashboard-ui` for the components, mount `@spree/dashboard-core`'s providers in their own app shell, and ship only the routes they want. No SDK abstraction to fight.
...
registerSlot('product_form_sidebar', {
  id: 'seller-info',
  component: SellerInfoCard,
  position: 100,
  if: ({ permissions }) => permissions.can('read', 'Spree::Seller'),
})
...
**Sign-in:**
- `no_store_access` — replaces the screen shown to a signed-in admin who holds a role on no store (a hosted platform's "create your store" CTA or redirect). Context: `{ user, signOut }`. Name and context type exported from `@spree/dashboard-core` (`NO_STORE_ACCESS_SLOT`, `NoStoreAccessSlotContext`).
...
### Phase 2 — page chrome + slot API (✅ done except two components)

Highest-leverage work. Every detail page reuses the same chrome and gains the legacy "more actions" drawer toolkit.

- ⏳ Tier 1 components — `<PageHeader>`, `<PageTabs>`, `<ResourceLayout>` and `<RouteErrorBoundary>` ✅; `<StickyFormFooter>` and `<EmptyState>` still missing
- ✅ Slot registry (`registerSlot`, `<Slot>`, `removeSlot`, `updateSlot`) with the built-in slot names from the Slot API section
- ✅ Navigation registry (`nav.add` / `remove` / `update` / `insertBefore` / `insertAfter`)
- ✅ Refactor `orders/$orderId.tsx` and `products/$productId.tsx` onto the new chrome — proves the design
- ✅ Normalize per-resource hooks into `src/hooks/`; `useResourceMutation` lives in `@spree/dashboard-core`

### Phase 3 — resource meta tooling (Tier 2, partially done)
...
- ⏳ `<AuditLogDrawer>`
- ✅ `<InternalNoteCard>` extracted from inline order code (shipped as `customer-internal-note-card.tsx`)

### Phase 4 — resource parity with Rails admin (largely done — 75 route files)

Build remaining resource pages on top of the chrome and Tier 3 form primitives (added as needed, not speculatively).

- ✅ Products list + detail/edit (sub-card extraction into the `spree/` layer is still partial)
- ✅ Orders list + detail/edit (plus drafts)
- ✅ Customers (plus groups)
- ✅ Promotions (plus gift cards)
- ✅ Stock locations, categories, collections, store settings, staff, roles
- ⏳ Imports ✅ and the dashboard home ✅; **reports and exports have no route yet**

Beyond the original list, these shipped too: claims, returns, exchanges, sellers,
companies, catalogs, price lists, product types, markets, delivery profiles,
tax categories/rates, payment methods, channels, integrations, webhooks, API keys,
emails, reasons, custom field definitions, commission rates and seller requirements.

### Phase 5 — bulk + power-user (Tiers 4 + 5, partially done)

- ⏳ `<BulkActionsBar>` ✅ (as `bulk-action-bar.tsx`); `<FilterChips>`, `<SavedViews>`, `<ExportDropdown>`, `<ImportDialog>` still missing (Admin API batch operation support is a prereq)
- ⏳ `<CommandPalette>` ✅ (`use-command-palette.tsx`); `<KeyboardShortcuts>` and `<NotificationCenter>` still missing
- ✅ Plugin loader / discovery mechanism (`defineDashboardPlugin` in `@spree/dashboard-core/plugin`; see `5.6-dashboard-typed-plugin-routes.md`)

### Phase 6 — drop Rails admin (started)

- ✅ Remove `spree/admin` engine entirely — gone from the monorepo
