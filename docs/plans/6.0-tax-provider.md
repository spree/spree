...
> **Amended 2026-08-07** (`6.0-drop-country-state-models.md`): the Country/State AR models are dropped in 6.0, so TaxRate's geography columns are **born as ISO strings** (`country_iso`, `state_abbr`), never FKs. The matching semantics below are unchanged; only the column type is.
...
class Spree::TaxRate < Spree.base_class
  ...
  # country_iso: 'DE' (nil = all countries)
  # state_abbr:  'CA' (nil = entire country; only meaningful with country_iso)
...
scope :for_address, ->(address) {
  where(country_iso: [address.country_iso, nil])  # country-specific or global
    .where(state_abbr: [address.state_abbr, nil]) # state-specific or country-wide
}
```

State-level rates override country-level (more specific wins). A rate with `country_iso=US, state_abbr=CA` takes precedence over `country_iso=US, state_abbr=nil` for California addresses.
...
class Spree::TaxExemptionCertificate < Spree.base_class
  ...
  # country_iso: jurisdiction (nil = all countries)
  # state_abbr:  state-level (nil = entire country) — ISO strings per 6.0-drop-country-state-models.md
  ...
  scope :for_address, ->(address) {
    where(country_iso: [address.country_iso, nil])
      .where(state_abbr: [address.state_abbr, nil])
  }
...
#   Extract country/state from ZoneMembers → set TaxRate.country_iso / TaxRate.state_abbr directly
#   (Zone with multiple countries = one TaxRate per country)
```

- Add `country_iso` and `state_abbr` string columns to `spree_tax_rates` (**amended 2026-08-07** — ISO strings, not FKs, per `6.0-drop-country-state-models.md`; the columns had not shipped, so this is a spec change, not a migration)
