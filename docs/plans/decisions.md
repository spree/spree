## 2026-09-24: A split parcel keeps its own delivery charge, and staff can override any parcel's cost afterwards

**Context:** Splitting a fulfillment on a paid order quotes the new parcel its own delivery rate. Reproduced on `main`: order R1001, goods $359.98 plus Standard delivery $5.00, paid in full at $364.98; one split moving a single unit left delivery $10.00, total $369.98, `partially_paid` with $5.00 due (V-3655). PR #14622 had fixed a neighbouring path — the leftover parcel of a partial fulfil now carries nothing — but the operator's Split goes through `Spree::FulfillmentChanger`, which re-quotes. The product call, made on the issue, is that a second parcel is a second shipment and keeps its charge; what was missing is a way for the merchant to adjust it afterwards. There was none. `PATCH .../fulfillments/:id` permitted no `cost` and answered 200 while dropping it. Switching the selected rate only reaches rates already quoted, and a hidden free method is not quoted on a split parcel at all until the parcel is moved to another warehouse, because the split quotes with the storefront audience. And any cost that did get set was restated from the rate by `Fulfillment#update_amounts` at the next re-quote: `Fulfillments::Create` protects an explicit `cost` for exactly one call, and a second create on the same order was seen re-quoting it.

**Decision:** The fulfillment's cost gets an owner, on the pattern `spree_line_items.price_source` already set: a nullable `spree_fulfillments.cost_source`, `'manual'` when staff set the amount. One guard in `update_amounts` refuses to restate an overridden cost, and every re-quote reaches the cost through that method — `refresh_rates` itself never writes it — so a single line protects all of them while quoting keeps working. The same guard freezes a parcel that has left (`fulfilled_or_delivered?`), which closes the one live path that restated a shipped parcel's charge, a rate correction through `Fulfillments::Update`. `Fulfillments::Update` takes a tri-state `cost` keyword, as `Orders::UpdateItem` takes `price`: absent leaves the cost alone, an amount overrides it, an explicit `nil` hands the parcel back to its selected rate — restating what the cost was before the override, quoting first when there is no rate, and clearing only the marker when nothing can quote it. It runs after the rate change so a revert sent with a new rate restates the new rate, and it recalculates and re-derives statuses explicitly because the model's own recalculation skips placed orders. A rate change or a move does not clear an override; only `null` does, since otherwise a merchant who zeroed a split parcel and then corrected its carrier would re-bill the customer. The Admin API forwards `cost` only when the request carried the key, through an `update_workflow_options` seam beside `fulfill_workflow_options`; the seller branch adds nothing, because what the customer is charged is the operator's call. `cost_source` is on the Admin serializer only, like `price_source`, and `Fulfillment#unpriced?` answers false once a freight parcel has been priced by hand. The strict cost parser moves into `Spree::Fulfillments::CostParsing`, a module Create and Update both include, and now also refuses an amount too large for the column rather than failing the write.

Rejected: making the split carry the charge instead — the product call went the other way, and apportioning a cost without the tax lines, discounts and fees that hang off the parcel leaves per-parcel money that `money_frozen?` never repairs on a placed order. A `Fee`, which cannot be negative. A manual `Discount`, which would record a concession the store never made on the invoice and in promotion reports. Accepting `cost` on Split and Create as well: kept to "adjust later", and Create's one-call behaviour is left as it was.

Why post-placement, when line price overrides are pre-placement only (2026-08-30): a unit price before placement is a negotiation, and a placed order's price edits stay fees and discounts. A delivery charge the system added after payment is a correction to a charge nobody agreed to, not a concession.

**Consequences:** The split is unchanged, so a paid order still reads `partially_paid` after one until someone acts; the user guide now says how. The override reaches freight parcels too, which narrows the wholesale-shipping plan's "freight charges are Fees" rule to charges nobody quoted. `update_amounts` no longer restates a shipped or delivered parcel, a behaviour change for a rate correction on a parcel that has left. Update refuses a blank cost where Create reads blank as omitted — two readings of one input, documented rather than unified. The two `#transfer_to_location` specs that asserted split costs never ran the split; they do now, and pin the second parcel's own charge. The OpenAPI files were not regenerated in this change: adding examples shifts FactoryBot sequences across unrelated endpoints, so the integration spec is committed and the regeneration is left to a docs pass. Left alone, all pre-existing: Create's explicit `cost` is still protected for one call, and its docs are wrong about that (`Fulfillments::Create`'s docstring, the admin controller's create comment, `FulfillmentCreateParams.cost`); the split quotes with the storefront audience, so backoffice-only methods are not offered on a split parcel; draining a source in a split destroys its tax lines, discounts and fees, which a placed order never regenerates; creating a parcel re-quotes every pending sibling that is not overridden; and `Carts::SplitBySeller#apply_divided_cost` protects its divided cost against `update_amounts` but not against a later `refresh_rates`.

**Plans amended:** `6.0-fulfillment-and-delivery.md` (the `delivery_amount` wording, a new Key Decision, Constraints on Current Work), `6.0-b2b-wholesale-shipping.md` (the freight-Fee Key Decision and its constraint).
