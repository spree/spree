...
**Decision:** Attribution moves to `Spree::Orders::AttributeToSeller`, called by both entry points. It partitions by seller and either stamps one seller onto the order or divides it into a group, apportioning the payment. `Orders::Complete` runs it after payment and before placement, then places the siblings the division produced; `Carts::Complete` keeps calling it from FINALIZE, where its replay guards live. `Carts::SplitBySeller` takes `cart:` as optional — `build_group` was its only cart-coupled method and the order carries every fact it read. The admin completion endpoint answers with the group when one was made, following the Store API, and the dashboard redirects to the orders list filtered to that group.
