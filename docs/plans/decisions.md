## 2026-08-22: BusinessAddress is real STI, and it is universal

**Context:** `Spree::BusinessAddress` began the day as a table-sharing subclass of `Spree::Address` — no `type` column, the class chosen by whichever association loaded it — serving only the seller's billing address. But company locations carry the same kind of address, and B2B checkout will put one on a cart, where the loading associations (`bill_address`/`ship_address`) are base-typed and cannot choose the class.

**Decision:** `spree_addresses` gains a nullable `type` column and the subclass becomes real STI. The rules now travel with the row: an address copied from a company location onto a cart loads back as a `BusinessAddress` and validates as one — company required, no personal name — without checkout knowing anything about it. `CompanyLocation`'s two addresses are typed `BusinessAddress`; the seller's billing address already was. Plain rows keep a NULL type and load as `Spree::Address`. Three consequences inside `Address` itself: `empty?` excludes `type` (a subclass row always carries it), while value comparison and `check`'s dedup keep it — the same postal facts under different validation rules are different addresses, and a business address must never merge into a personal twin another reference depends on. `CompanyLocation` stamps its own company's name onto an address payload lacking one, so the API contract does not start demanding data the branch already knows — deliberately not mirrored on `Seller`, whose billing address is the addressee of a commission invoice and must be stated rather than guessed from a trading name. The migration backfills `type` for rows already referenced by sellers and company locations — without that, a NULL-typed row behind a subclass-typed association reads back as nil, because the STI default scope filters on `type`.

**Rejected:** keeping table-sharing (the class cannot survive a copy onto a base-typed association, which is exactly the B2B checkout path); a storefront "buying as a company" flag on cart address writes (a Store API design question for when B2B checkout actually lands, not schema work).

**Consequences:** callers create the right class — the `becomes` coercion on `Seller#billing_address=` is gone and assigning a plain `Address` record to a typed association raises; the `:business_address` factory is the spec-side answer. Nested attribute writes were already correct: a typed `belongs_to` builds through its reflection, so Hash payloads need nothing. `StockLocation#address` stays a plain `Address` — it is a transient warehouse address for rate quotes and labels, never validated as a business one. No serializer exposes `type`.
