...
2. **Keeping `type` significant for dedup breaks address-book reuse.** `find_existing_address` matches on all attributes. With a `type` column, a business address and a personal address with identical postal details never dedupe, so a B2B buyer re-creates a row on every order.
