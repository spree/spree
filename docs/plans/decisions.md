...
3. **No cross-record validation tying a custom field's value to its definition's store.** Resolution is store-scoped at every path that reaches a definition, so a mismatched pair is unreachable through normal code. A validation would also have to skip the resource types that have no store of their own (customers, addresses), making it a partial guarantee that reads like a full one.
