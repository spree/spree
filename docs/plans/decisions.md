...
**Decision:** `spree_fulfillment_items` gains `replacement`, a boolean defaulting to false, set on every unit an exchange or claim sends. `OrderInventory#verify` leaves flagged units out of the count and out of the units it removes when the quantity falls, so an edit only ever adds or removes goods the customer paid for. A line being destroyed still counts and drains its replacements as it did before, so the stock promise on a same-variant replacement is released rather than left with nothing to cancel. Both workflows build their replacements through one `Spree::Fulfillments::Replacements` module, which builds the units by id rather than through the association writers: with `has_many_inversing`, each writer adds the unit to its owner's collection, and that owner's next save persists it. `Fulfillments::Create` and `FulfillmentChanger`, the two paths that rebuild units when a fulfillment is split or moved, carry the flag across. The changer takes it from the first unit it moves, the same way it already takes the line.

Rejected: counting only units of the line's own variant. It needs no migration and fixes exchanges, where a replacement is always a different variant, but a claim often sends the same variant again, and that replacement would still count. Rejected: fixing only the stray unit, which stops the deletion but leaves the raise adding nothing.

**Consequences:** Raising the line from 1 to 2 now keeps the original fulfillment and its $5.00, adds one Stainless unit and one Stainless allocation, and brings the total to $164.98. No data repair ships: replacement units already fulfilled on a beta install stay unflagged, and their stray duplicates stay behind, so an edit of those lines still misbehaves. The order still reads `paid` after the edit raises its total; that is V-3638. Left alone, all pre-existing: `OrderInventory#add_to_shipment` rebuilds every fulfillment on the order when no open fulfillment can take the unit, which on a placed order is what `Orders::BuildFulfillments` refuses to do. `#determine_target_fulfillment` can put a raised unit into a pending replacement fulfillment, which carries no delivery charge. Removing a line drains only units of the line's own variant, so an exchange replacement's stock promise outlives the line. `FulfillmentChanger` merges the units it moves by variant alone, so a fulfillment mixing bought and replacement units of one variant can come out with the wrong flag, as it can already come out with the wrong line. `Stock::Packer#default_package` keeps one unit per variant, so two replacements of the same variant for different lines fail as out of stock. An exchange accepts units that never shipped, then restocks them on receipt while the original fulfillment still holds them.

**Plans amended:** `6.0-returns-exchanges-claims.md` (the Exchange section's replacement-fulfillment note, the Phase 3 note on the removed exchanged-unit API).
