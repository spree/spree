## 2026-09-24: Store credit is applied rather than chosen, and a gift card follows the total it pays

**Context:** A buyer with a gift card that did not cover the cart was offered Store Credit as the only payment method, and Place Order did nothing (V-3668). Reproduced on `main` against a fresh install, which seeds exactly one payment method, Store Credit: $44.99 cart, card with $10.01 left, amount due $34.98, `payment_methods` `[Store Credit]`, and `POST /carts/:id/payments` answered 422 "Source can't be blank". The gift card was only what put Store Credit in the list: a buyer holding store credit and no gift card failed the same way, because that endpoint builds a payment with no source and a store credit payment needs one. The docs already describe store credit as a balance applied through `POST /carts/:id/store_credits` before choosing how to pay. Testing the combinations turned up four more defects. A gift card's payment never moved with the total: the resize existed as `Order#recalculate_store_credit_payment`, ran from `before_transition to: :payment`, and lost its only caller when the state machine went (6ff55b19f8), so a buyer paid $5.00 by card where $2.99 was owed while the gift card still held $2.01 — and switching to a cheaper delivery left the card paying the old, higher amount. Any change to a store credit payment's amount made the order impossible to place ("Could not find store credit"): `PaymentMethod::StoreCredit#purchase` takes a payment against the eligibility event recorded for its amount, and only creation recorded one. Applying credit, adding an address and applying credit again was enough. The same resize also counted the payment twice against its own credit, so a $30 credit ended up behind a $44.99 payment, the cart showed nothing due, and Place Order failed. `StoreCredits::Apply` spent a gift card's credit as if it were the buyer's own, so the card's balance paid for another cart while its own cart could neither be placed nor have the card removed. And mixing the two was refused in one direction only: store credit applied after a gift card inflated the card's payment, $10.01 of balance ending up behind $24.99 of payments.

**Decision:** Store credit is never one of a cart's `payment_methods` (`Purchase::PaymentProcessing`), and the Store API payments endpoint refuses store credit, which passes its active-and-storefront-visible lookup but could only ever fail there. The deprecated `Order#collect_payment_methods` delegates to it rather than keeping its own copy of the rule. The `order.gift_card.present?` clause in `PaymentMethod::StoreCredit#available_for_order?` stays, because `Payment`'s own validation needs it for the gift card's payment. Resizing a store credit payment goes through one method, `Payment#update_store_credit_amount!`, which records a new eligibility event; `StoreCredits::Apply` and the gift card resize both use it. A resized payment grows only into its own credit's remaining balance, since a payment not yet taken reserves nothing, and any shortfall is drawn from the buyer's other credits, never the same one again. A gift card's payment follows the total in `Carts::RecalculateTotals` (orders inherit it), the seam every totals change passes through, including P3's in-lock recalculation. When the total reaches zero the card is released rather than resized, since a store credit cannot hold zero and a stale amount would charge the card for a free order. The resize draws only what the card can still give (`GiftCard#spendable_amount`): nothing more once it has expired, been redeemed or been cancelled, and never less than it already pays when an admin lowers the card's amount below what is in use. `GiftCards::Apply` sizes a new payment the same way. Item changes still unapply the card in `Carts::Recalculate`, as before. A gift card and store credit are never combined: `StoreCredits::Apply` refuses once a gift card is on the order, mirroring `GiftCards::Apply`, and draws only on the credits the buyer's balance counts — their own, in the order's store and currency.

Rejected: seeding Store Credit with `storefront_visible: false`, which every existing install would miss and an admin could switch back on. Also rejected: dropping the gift card clause from `available_for_order?`, which breaks gift card application outright — tested. Also rejected: resizing only at completion, which the old plan text described. The amount due shown during checkout would stay wrong, and a remainder payment sized from it would overpay once the card grew.

**Consequences:** Store Credit leaves the storefront's payment list. The storefront has no control that applies store credit, so its buyers cannot use store credit until it calls the endpoint; that is a storefront change, deliberately out of scope here. `POST /payments` now answers 422 `payment_method_unavailable` for store credit instead of "Source can't be blank". A cart that a gift card covers in full still completes without a payment, since the storefront already skips payment when nothing is due. Plain store credit is still not resized when the total changes, and the plans now say so instead of claiming it was. That leaves a pre-existing defect standing, reproduced here: a buyer who applies credit and then picks a cheaper delivery is charged the old amount — order R1028 took $44.99 of credit on a $39.99 total and reads `overcharged`. Shrinking the credit to fit the total would fix it; whether credit should also grow with the total is a product decision, so both are deferred. Gift card specs that faked totals on orders with no items now build real ones — they had asserted a $30 payment on an order whose recalculated total was $0, which held only because nothing resized it. Left alone, all pre-existing: one ordinary store credit can sit on two open carts at once, and the second fails at Place Order with no money lost — deferred. On an order authorised at checkout and charged later, `amount_due` subtracts captured store credit twice — skipped. The storefront shows nothing when Place Order fails. The Bogus test gateway never settles a payment session. A payment session for store credit answers 500. Applying a gift card to an empty cart reports "Amount must be greater than 0". The older refusal, a gift card after store credit, returns its untranslated key. `StoreCredits::Apply` takes the store credit payment method without scoping it to the order's store. The deprecated `Order#available_payment_methods` passes an argument that `collect_payment_methods` does not take, so it raises.

**Plans amended:** `6.0-cart-order-split.md` (the offline payments bullet and the partial store credit edge case), `6.0-core-rewrite-tasks.md` (the PAYMENT line), `6.0-6.1-loyalty-gift-cards-store-credits.md` (three new Constraints on Current Work bullets).

## 2026-09-25: Editing a placed order's money re-derives its payment status

**Context:** Adding a charge to a paid order left it reading `paid` with money owed (V-3638). Reproduced on `main`: order R1001, total $205.00, paid $205.00; a $50.00 fee took the total to $255.00 and `amount_due` to $50.00 while `payment_status` stayed `paid`, and `update_statuses!` answered `partially_paid` at once. The stale status then stuck through every later edit: deleting the fee left a fully paid order reading `partially_paid`, and cutting a quantity until the customer had paid $100.00 over the total still read `partially_paid` rather than `overcharged`. The cause is a gap in the trigger list. `UpdateStatuses` runs from payment, refund, store credit, fulfillment and return events, and the post-placement edit services (fees, manual discounts, items) re-sum the totals and stop, because no event announces a total moving. Changing a fulfillment's delivery rate was the one edit that worked: `Fulfillments::Update` calls `update_statuses!` itself.

**Decision:** Each service that edits an order's money calls `order.update_statuses!` after it re-sums the totals: `Orders::Fees::{Create,Update,Destroy}`, `Orders::Discounts::{Create,Update,Destroy}`, `Orders::Update`, `Orders::UpdateItem` and `Orders::AddItem`. The fee and discount services make the call inside their own order lock. The other three rely on the lock their API controllers take, and `AddItem` makes it after its transaction has committed. The call is unconditional, as the event subscriber's is, so a draft order's statuses stay derived as well. Edits that save a payment or fulfillment row (gift cards, store credit, fulfillment create and move) are already covered, since those events fire after commit, once the new total is in place.

Rejected: deriving the statuses inside the totals workflow, which would cover every caller at once but give up that workflow's money-only contract; and publishing a new "totals changed" event for the subscriber, which adds a public event name to answer something the edit services already know.

**Consequences:** A new service that edits a placed order's money has to make the same call, because nothing else will. `Orders::RemoveLineItem` does not, and is left alone deliberately. It routes through `Orders::UpsertItems`, which never recalculates totals, so `DELETE /api/v3/admin/orders/:id/items/:id` currently removes the line and leaves the total where it was. Deriving a status from that stale total would change nothing, and the missing recalculation is a defect of its own. `Exchanges::Fulfill` and `Claims::Resolve` are not covered either: they recalculate totals as their last step, after their refund and fulfillment events have already derived the statuses. Whether that final recalculation moves the total, for example by charging delivery on a replacement, was not established here. `amount_due` never goes below zero, so an overpaid order shows an outstanding balance of $0.00 and only the `overcharged` status says a refund is owed.

**Plans amended:** `6.0-cart-order-split.md` (the derive-then-persist triggers).
...
## 2026-09-16: A split checkout confirms once from its OrderGroup, and counts deliveries from parcels

**Context:** A checkout spanning several sellers splits into one order per seller and deliberately sent a single confirmation email — but it sent the *first child order's* own email, carrying that order's number, items, delivery and total. On a mixed basket the first-party child sorts first, so a $374.97 purchase of three items confirmed $50.67 and one item, with nothing to say the rest existed (V-3642). The workflow already published `order_group.completed` and nothing had ever subscribed to it.

**Decision:** The confirmation moves onto that event. Every child of a split places with `notify_customer: false`, `Spree::OrderGroupEmailSubscriber` answers the group, and `Spree::OrderGroupMailer` renders the whole purchase under the number the customer saw at checkout. The email is organised **by parcel**, and a parcel is a set of fulfillments sharing `[stock_location_id, delivery_method_id]` — collapsed by `Spree::FulfillmentGroup`, because when one warehouse serves several sellers the packer builds one package and the split divides its single quote across the children, so three orders can ship in one box. The store-owner notification moves to the group with it, and an admin resend on a grouped child sends the group's email. Two delivered flags on `spree_order_groups` mirror the order's, since replay re-publishes the event.
...
**Decision:** Money stays per-currency and is never converted. `Query#scope_currency` returns the currency only when some aggregated component of the requested metrics is money-formatted, and the bases skip the filter when it is nil — so counts, quantities and money-free ratios span every currency, while any query containing money narrows as before. A ratio counts as money when either side is (average order value), and not when neither is (sell-through). The dashboard home screen carries a currency switcher, shown only where the store sells in more than one.

**Consequences:** No base-currency column on orders and no conversion anywhere in reporting. The market splits on this: Magento and Shopify store a second, converted amount per order and report on it, while Saleor's Pulse analytics, Medusa and Vendure keep money scoped per currency. The converted approach buys one aggregate number and pays for it in reconciliation — Shopify converts at each money movement rather than freezing the sale's rate, so a full refund need not net to zero in shop currency and their own documentation calls the reporting conversion "approximate and may differ slightly from actual captured amounts"; Magento's own Advanced Reporting only supports installations that kept a single base currency throughout. For a platform whose figures merchants may file, exactness beats a single headline number. If a cross-currency total is ever wanted it must be a separate, clearly labelled figure carrying its rate and date — never the same metric as the exact one. A base that registers its own relation must guard the currency filter against nil.
