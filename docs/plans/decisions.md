## 2026-09-24: A rebuilt delivery proposal keeps the customer's choice and its price, silently, with no completion guard and no cleanup

**Context:** Adding an item after the address was set dropped delivery to $0.00 and the order completed that way (V-3696). Reproduced on `main`: a cart read $20.00 + $5.00 Standard = $25.00 after the address; one more unit made it $40.00 + $0.00 = $40.00, and order R1001 was placed and paid at $40.00. Every cart recalculation rebuilds the delivery proposals through `Cart#ensure_updated_fulfillments` -> `rebuild_fulfillments!`, and nothing copied the selected rate's price onto the new rows, so their stored `cost` stayed 0 and `RecalculateTotals` summed that into `delivery_total`. Only the address-change path priced its rebuild; `Checkout::Advance`'s rebuild for quick checkout had the same gap. Completion's drift guard could not see it, because its in-lock recalculation re-sums the same stored zero. Review then found two more faces of the same rebuild. It re-ran the estimator's default pick, so a shopper who had chosen free Store Pickup was switched back to Standard on the next item change - hidden while the cost came out $0.00, a $5.00 charge once it did - against the rule in `6.0-delivery-rate-provider.md` that nothing may silently reset a chosen rate. And draft orders ride the same recalculation, where `Order#ensure_updated_fulfillments` deleted the fulfillments outright and rebuilt nothing: a $25.00 draft became $40.00 with no delivery at all when an admin added an item.

**Decision:** A rebuild carries the choice over and prices what it builds. `Fulfillment#carry_over_selection` selects the rate standing for the one chosen before - the same carrier service when still quoted, then the same method, then the estimator's pick, the rule `refresh_rates` already applied, now shared through one private method - and sets the proposal's cost from it before it is saved. `Cart#rebuild_fulfillments!` and `Order#rebuild_fulfillments!` both call it, so every caller is covered: the recalculation after any item change, quick checkout, the draft-order build. The earlier choices are grouped by stock location, and each new proposal takes the first one whose method it is quoted for, because one location can ship several proposals - a digital item beside a physical one, stock on hand beside a backorder - each with its own choice. A rebuild for a changed destination keeps nothing (`keep_selection: false`, decided by `Carts::Update` and `Orders::Update` and passed down through `Cart#recalculate_for_address_change!` and `Orders::BuildFulfillments`): a choice made for one address is not a choice for another, and before an address is known only pickup is offered, so keeping that default once shipping is available would pin every cart to pickup. A destination change is a different address row, a pickup counter picked or cleared, or an edit to the fields a quote reads (`Spree::Address::DESTINATION_FIELDS`: street, city, state, postal code, country). The address id alone cannot say, because a guest's edit updates the row in place; and a corrected phone number or recipient name, or a market change, keeps the choice and quotes it again. `Orders::Update` now rebuilds on an in-place edit to those fields, where before it did not rebuild at all. `Order#ensure_updated_fulfillments` rebuilds instead of deleting. When the new contents re-quote the kept rate at a different price, the cart takes the new price without telling the shopper: it is the current quote for what they are buying, and the total already shows it.

Rejected: a warning when delivery changes on rebuild - the new figure is correct, not a surprise to flag. Deferred: a completion guard refusing an order whose stored delivery cost disagrees with its selected rate. It would catch this whole class of defect, but fixing the one path that produces the disagreement is enough for now. Rejected: a cleanup task or upgrade note for orders already placed with $0.00 delivery - they exist only on 6.0 beta installs. Rejected: setting the cost in `Stock::Package#to_fulfillment`, which would reach every builder at once - including `OrderInventory`'s rebuild of a placed order, whose delivery was charged at placement and is re-priced only through the post-placement edit path. For the same reason `Order#rebuild_fulfillments!` does not carry a choice over or set a cost on a placed order; it still deletes and replaces that order's fulfillments, as it did before.

**Consequences:** Adding, changing or removing items keeps the delivery the customer or admin chose, at its price for the new contents, on carts and draft orders alike; a new address still starts from the default. Beta orders already placed at $0.00 delivery stay as they are. Two proposals from one location offering the same methods (stock on hand beside a backorder) that had different choices are matched in order, so the pairing can swap; nothing identifies a proposal across a rebuild. Left alone, all pre-existing: `Order#ensure_updated_fulfillments` publishes `order.updated` before the recalculation has summed the new delivery costs, so the event carries the previous figures; a rebuild still drops the parcel locker frozen into `pickup_point_data`, so a kept pickup-point method needs its point chosen again; `recalculate_for_address_change!` quotes delivery before `item_total` is refreshed, so a rate that reads the order value quotes on the old figure; and the currency-change callback rebuilds before the line items are converted. Nothing yet stops a future path from storing a cost that disagrees with the selected rate; that is the deferred guard.

**Plans amended:** `6.0-delivery-rate-provider.md` (Selected-rate preservation).
