...
- **First server-side enforcement.** Today guest checkout is unenforced in the API — the legacy `Spree::Config.allow_guest_checkout` was *"only used in the rails frontend, not implemented in API"* and `Store#prefer_guest_checkout` only drove the legacy admin form. In 5.6 the v3 Store checkout (order-create / `complete` transition) requires an authenticated customer when the order's `channel.resolved_guest_checkout` is false.
