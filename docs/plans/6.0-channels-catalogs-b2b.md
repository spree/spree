...
- Resolution via a `HasStorefrontAccessPreference` concern mirroring `OrderRouting::HasStrategyPreference`: `channel.resolved_storefront_access` returns the channel value, or the store value when the channel preference is `nil`.
...
- **Enforcement (v3 Store API):**
  - `prices_hidden` → the base `Store::ResourceController` flags the serializer context so price fields (`price`, `compare_at_price`, `prior_price`, `display_*`) render `null` for guest requests. Logged-in customers (incl. B2B) serialize normally.
  - `login_required` → a `before_action` in the Store `ResourceController` returns `401` on catalog reads (products, variants, categories) for guest requests. Auth endpoints stay open.
...
- `Spree::Channels::HasStorefrontAccessPreference` resolution concern (mirrors `OrderRouting::HasStrategyPreference`); `channel.resolved_storefront_access` / `channel.resolved_guest_checkout`.
- v3 Store API enforcement: serializer price-nulling for `prices_hidden`, `before_action` 401 guard for `login_required`, guest-checkout guard on order-create / `complete`.
...
- Storefront access concern (planned): `spree/core/app/models/concerns/spree/channels/has_storefront_access_preference.rb`
- Pattern reference: `spree/core/app/models/concerns/spree/order_routing/has_strategy_preference.rb` (channel preference → store fallback)
