...
class Spree::CompanyContact < Spree.base_class
  ...
  belongs_to :user, class_name: Spree.customer_class.to_s
...
**The approval flow rides an Enterprise-owned approval record**
(`5.5-6.0-order-cancellation-and-approval.md` was superseded 2026-09-02 —
OSS ships no approval substrate):
...
- `guest_checkout` is channel-wide; fine for B2B since company buyers are always real users (`order.customer.present?` holds).
