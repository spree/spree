...
**Status:** Superseded in part — Phase 1 implemented in 5.5 (Channel + new `spree_product_publications` table + single-store Product + channel-aware scopes + Admin API action endpoints + Store API back-compat + legacy admin Channels CRUD + Publishing card + dashboard SPA + Meilisearch channel filter); gated storefront access **shipped in 5.6** (`Spree::Channel::Gating` + v3 Store API enforcement — see `5.6-store-channel-context-and-key-binding.md` for the follow-on channel-context endpoint + key binding); the Catalog + Company phase is **superseded — see below**
**Target:** Spree 5.5 (Phase 1 ✓) → 5.6 (Gated Access ✓) → 6.0 (Catalog + Company, via the superseding plan)

> **Superseded (2026-08-25):** the Phase 2 deliverable (Catalog + Company tree + pricing integration) is pulled back into **6.0** and redesigned by **`6.0-b2b-companies-and-catalogs.md`** — B2B is a 6.0 headline and the merged-but-unreleased Company → CompanyLocation → CompanyContact tree is replaced by a multi-level `Spree::Company` organization tree (typed nodes, address book, subtree-scoped memberships, subtree catalog inheritance). The Company/Catalog model sketches and the Phase 2 migration below are the **historical record only** — several details were superseded even before the redesign (`tax_exempt` booleans are not built per decisions.md 2026-08-07; contacts shipped against `customer_id`, not `user_id`) and none of them should be implemented from this file. Phase 1 (5.5) and the 5.6 Gated Access phase remain this plan's authoritative shipped record, including the Gating × Catalog composition rules, which the superseding plan inherits unchanged.
...
**Price resolution (extends the Internal pricing provider's resolution — `Spree::PricingProvider::Internal::Resolution`, formerly `Pricing::Resolver`, moved 2026-08-19):**
...
- **Tax obligations from `6.0-tax-provider.md` (2026-08-05):** Company becomes an owner on `spree_tax_identifiers` (`company_id` FK — the durable VAT/tax registration for the entity, joining the resolution chain above the customer's own row), and `CompanyLocation` is the eventual home for per-branch registrations (Shopify's `CompanyLocation.taxRegistrationId` precedent). `Spree::TaxExemptionCertificate` hangs off Company (`has_many :tax_exemption_certificates`) but is **specified and built by the tax-provider plan's last phase**, which is gated on Company shipping — see that plan for the schema and the model. Nothing is owed here beyond the association.
...
class Spree::Company < Spree.base_class
  ...
  # Identity in an ERP/CRM is Spree::ExternalReference, not a column — a
  # company is known to several systems under different keys
  # (`6.0-third-party-pricing-inventory.md`, shipped 2026-08-18).
...
### Company roles and approvals — storefront authorization (Enterprise on OSS seams)

Contact roles enforce through the storefront seams, never by decorating Store
API controllers (decisions.md 2026-08-08). The full architecture, settled
2026-08-08 while designing the seams:

**Company roles are data, not code.** The string `role` attribute above is a
placeholder; the real model is an Enterprise `Spree::CompanyRole` —
`capabilities` JSON array over a fixed company capability vocabulary
(`place_orders`, `approve_orders`, `view_location_purchases`,
`view_company_purchases`, `manage_company_contacts`,
`manage_company_addresses`, …) with `CompanyContact` referencing a role. Data
because the company admin manages contacts and roles themselves, through
Enterprise storefront endpoints (`/api/v3/store/company/*`) gated by
`manage_company_contacts`, with anti-escalation mirroring the staff
`RoleGrantGuard` pattern. This is the commercetools associate-roles shape
(data roles over a fixed capability vocabulary) and the reason storefront
permission sets were not revived: code-defined sets cannot be edited by a
company admin in a UI. The vocabulary is Enterprise-owned and **never shared
with the staff catalog** (`6.0-admin-rbac.md`: `write_orders` means "manage
the store's orders" to staff, "place my company's orders" to a buyer).

**Visibility is a policy subclass.** Enterprise `CompanyAccessPolicy <
Spree::Storefront::AccessPolicy`, registered via
`Spree::Dependencies.storefront_access_policy_class`. `scope` / `readable?` /
`writable?` widen carts, orders and shared wishlists per the contact's
capabilities — an approver lists their location's pending carts through the
same core endpoints every customer uses.

**The approval flow rides the cart-side approval substrate**
(`5.5-6.0-order-cancellation-and-approval.md` defers buyer-organization
approvals to land here; the approver is already polymorphic with
`Spree::CompanyContact` anticipated):

1. An employee checks out → an Enterprise `carts.complete.validate` hook
   rejects when the contact lacks `place_orders` or exceeds their spending
   limit (well-known `approval_required` error).
2. The client calls the Enterprise submit-for-approval endpoint → cart-side
   approval record created, event published.
3. The manager (`approve_orders`) sees the pending cart via the widened
   policy and approves → an Enterprise workflow completes the cart as the
   approver, recording the approval (append-only history per the approval
   plan).

No new core seams are required: the access policy, the checkout `validate`
hooks, and events cover it. A standardized `approval_required` error code on
cart completion is a nice-to-have for this release. Contact roles never touch
`Spree::Ability` or the staff permission catalog.
...
- Drop `Spree::Product.available_on`, `Spree::Product.discontinue_on`, `Spree::Product.make_active_at` columns and the deprecated setters in `Spree::Product::Channels`.
  - **Blocked on `6.0-replace-taxons-with-categories.md` Phase 5.** `Spree::CollectionRules::AvailableOn` reads `spree_products.available_on`; it must be reimplemented against `published_at` first, or dropping the column breaks it. That plan's Phase 5 owns the reimplementation and is sequenced before this drop.
