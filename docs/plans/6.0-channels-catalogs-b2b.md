...
**Status:** Phase 1 shipped in 5.5 (Channel + new `spree_product_publications` table + single-store Product + channel-aware scopes + Admin API action endpoints + Store API back-compat + legacy admin Channels CRUD + Publishing card + dashboard SPA + Meilisearch channel filter); Catalog, Company tree, pricing integration, and gated storefront access land in 6.0
**Target:** Spree 5.5 (Phase 1 ✓) → 6.0 (Catalog + Company + Gated Access)
...
- **This closes the previously-unspecified "deny anonymous" gap.** Product visibility resolution (above) falls back to *"show all channel listings (default catalog)"* for users with no catalog — which always exposed the default catalog to anonymous visitors. `login_required` now fronts that resolver: a guest request `401`s before `Products::ForContext` runs. `prices_hidden` composes *with* Catalogs — a guest browses the default catalog with no prices, while a logged-in B2B buyer sees their company-location catalog at its PriceList price.
...
- **First server-side enforcement.** Today guest checkout is unenforced in the API — the legacy `Spree::Config.allow_guest_checkout` was *"only used in the rails frontend, not implemented in API"* and `Store#prefer_guest_checkout` only drove the legacy admin form. In 6.0 the v3 Store checkout (order-create / `complete` transition) requires an authenticated customer when the order's `channel.resolved_guest_checkout` is false.
