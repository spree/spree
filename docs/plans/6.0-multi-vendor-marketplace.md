...
**Per-currency amounts (RESOLVED 2026-08-17, shipped).** A flat fee's amounts live in `spree_commission_rate_values`, one row per currency, rather than a single `currency` column on the rate — which had meant a dollar-priced rate silently not applying to a euro sale, so a seller's rate depended on how the buyer paid. A rate is skipped for a currency it states no amount in and the sale falls to the next rate: converting one currency's figure into another would invent a fee nobody set. `currency` survives on the rate for one purpose only — naming the currency a *percentage* rate's floor or cap is stated in.
