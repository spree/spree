...
**One tax or several? (OPEN, raised 2026-08-17.)** We model the fee's tax as a single `tax_amount` + `rate` pair on the line. The enterprise platform surveyed ships exactly that shape as **deprecated**: its scalar `commission_vat`/`commission_rate_vat` are superseded by a `commission_taxes[]` array of `{code, rate, amount}`, because the scalars can only return "the first tax applied" where several apply at once. Its `total_commission` is then defined as fee + the summed tax. Its taxability is also a per-seller property (`applicable_taxes[]` bound to fee types), which matches our resolution against the vendor's billing address.

A scalar is right for a single-VAT supply, which is the EU case this plan is built for, and it is what the legacy module did. It is wrong wherever a fee attracts two levies at once. Deciding now is cheap — nothing has shipped — and expensive later, since widening a scalar to a collection changes the ledger's shape and every reader of it. Flagged rather than resolved: the fee-VAT itself is the headline feature and works today; this is about whether the *container* is a number or a list.
