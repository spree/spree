...
Open-source the core of Spree's multi-vendor marketplace into Spree 6.0, per [spree/spree#13323](https://github.com/spree/spree/issues/13323). Today multi-vendor lives in the paid Enterprise `spree_multi_vendor` gem as a **decorator-heavy monkey-patch** of core (`Spree::Order.prepend(SpreeMultiVendor::OrderDecorator)`, ~30 `vendor_order_*` associations, a `state == 'splitted'` hack on the order state machine). 6.0 brings the **Vendor identity, per-seller order splitting, the commission engine, and the payout ledger** into core as first-class models — built natively on the 6.0 Cart/Order split rather than retrofitted onto the legacy `Order`-only model.

What stays paid (Enterprise — boundary amended 2026-07-15, see `decisions.md`): refund **clawbacks** (automatic Stripe transfer reversals + negative-balance netting), ledger⇄provider **reconciliation**, KYC/account-health operations, payout reports (incl. DAC7), marketplace-facilitator taxes, Shopify/WooCommerce sales-channel apps, the category mapper. **Basic Stripe Connect execution — Express onboarding + on-fulfillment transfers — ships open-source** in the OSS Stripe extension. **Core runs the happy path end-to-end (onboard, sell, get paid); Enterprise operates the unhappy paths and compliance at scale.**

This plan covers the four subsystems: **(A) vendor identity & teams**, **(B) order splitting**, **(C) the commission engine**, and **(D) the payout/fund-transfer ledger**. It is informed by a source-level reading of the legacy `spree_multi_vendor` gem (`/Users/damian/github/spree_multi_vendor`), the current Enterprise behavior documented in the user docs, and a survey of modern open-source marketplace implementations — see References and the Competitive Benchmark (competitor names deliberately omitted; this file ships in a public repo).
...
| **Basic Stripe Connect payouts** (Express onboarding link + on-fulfillment transfers, via the OSS `spree_stripe` extension) | Ledger⇄Stripe **reconciliation** |
...
- At `cart.complete!` (the `Spree::Carts::Complete` service from the cart/order-split plan), partition cart line items **by `line_item.variant.product.vendor`** and fan out into **N child `Spree::Order`s, one per vendor**, all owned by **one `Spree::OrderGroup`** that holds customer/payment/address context.
...
This is the single most important integration point with the rest of 6.0: **the split is a consumer of the Cart/Order split, implemented inside the completion service**, not a state-machine `after_transition` callback bolted onto a monolithic `Order` (which is how the legacy gem did it — `order_decorator.rb:51`, `after_transition to: :complete, do: :split_order`). The 6.0 cart/order plan removes the order-level state machine entirely, so the legacy trigger cannot survive — splitting must be invoked explicitly from `Carts::Complete`.

### 2. Commission is a frozen snapshot line attached to the order line item, computed once at placement

Mirror the core insight from both the legacy gem and the industry references: commission is **denormalized and frozen at sale time**, never recomputed on read. This matches Spree's existing EU Omnibus "frozen at sale" precedent (`5.4-6.0-eu-legal-compliance.md` PriceHistory).

- New model **`Spree::CommissionLine`** (NOT the legacy `Spree::MarketplaceCommission`/`spree_platform_fees` table, NOT a polymorphic `Adjustment`). One row per commissioned line item — plus one per fulfillment when the rate commissions delivery revenue (`include_shipping`, Decision 4) — storing a **snapshot** of `rate`, `amount`, and the resolved rule's identity.
...
This is the one place where the **legacy gem is ahead of the surveyed industry implementations** and the reason #13323 calls out "commission taxation which is essential for European marketplaces." The common industry pattern uses an `include_tax` toggle that only decides whether the *seller's item tax* inflates the commission base; it charges **no tax on the marketplace fee itself**. The legacy gem does (`calculate_item_fee.rb:18`):

```ruby
fee_additional_tax_total = (fee_amount * fee_tax_rate / 100).round(2, BigDecimal::ROUND_HALF_UP)
fee_total                = fee_amount + fee_additional_tax_total
```

The marketplace's commission is itself a **taxable B2B service** the platform renders to the seller. 6.0 must model:
...
**6.0 adopts the explicit per-seller payment record** (pattern b) rather than proportion-on-read, because a marketplace needs a durable per-seller audit trail for partial captures, partial refunds, and (Enterprise) payouts. This is also what the legacy gem effectively did by creating real `Spree::Payment` rows on each vendor sub-order (`create_vendor_order.rb:15`, `run :create_payments`) — but 6.0 does **not** clone `Payment` rows: the gateway object stays singular.
...
- No ambiguity / no empty parent orders. A parent `Order` with `parent_id: nil` and zero line items (the legacy shape) breaks every "an order has line items" assumption and forces `all_line_items`/`all_shipments`/`splitted?` workarounds (see `order_decorator.rb:177-239`). `OrderGroup` simply *has many* orders.
...
**This requires an Enterprise-side data backfill** (not core — existing OSS stores have no vendors). The legacy `spree_multi_vendor` gem must ship a one-time migration that, for every legacy `splitted` parent order, creates an `OrderGroup`, attaches its `parent_id`-children as the group's orders, and migrates `spree_platform_fees` → `spree_commission_lines`. The backfill lives in Enterprise because only Enterprise customers have legacy marketplace data; core ships clean. Tracked as a hard dependency for the Enterprise 6.0 upgrade. See Migration Path.
...
**Transfer and payout are two distinct events** — the legacy gem conflated them, and an earlier draft of this plan repeated the mistake. They must be separate models:

- **`Spree::VendorTransfer`** — the **per-order** fund movement, triggered **on-fulfillment**. When a vendor's order is fully fulfilled (shipped; digital fulfills instantly after payment), the vendor *earns* `sale − commission`; that credits their balance. One transfer per fulfilled vendor order. This is what the legacy `SpreeStripe::Transfer` actually modeled (it fires on shipment — `create_stripe_transfer_for_order.rb` runs when `shipments.all?(&:transferable?)`).
...
The legacy gem only models the transfer and **outsources payouts to Stripe** (`update_payouts_schedule.rb` just configures Stripe's native connected-account payout schedule — there is no `Payout` record; the `vendors_payout` files are *reports* aggregating transfers, not a ledger). That's fine for Stripe-only Enterprise but **wrong for OSS core**: a `system`-provider marketplace has no Stripe to schedule settlements, so it needs its own payout records.
...
class Spree::Vendor < Spree.base_class
  ...
  # Ports the legacy machine (vendor.rb:133 — status column renamed from `state` per
  ...
  state_machine :status, initial: :pending do
    event(:invite)              { transition pending: :invited }            # operator sends the Invitation
...
- **Trigger:** explicit call inside `Spree::Carts::Complete`, after payment authorization, gated on `cart.line_items.any?(&:vendor)`. Not a state-machine callback — and not the legacy shape either: the legacy gem splits **30 seconds after checkout** in a background job (`SplitByVendorJob`), leaving a window where the parent order exists un-split. 6.0 splits synchronously inside completion, so the checkout response already returns the finished `OrderGroup` and no `splitted`-pending state ever exists.
- **Partition key:** `line_item.vendor_id`, denormalized from `variant.product.vendor` when the item is added (the legacy gem does the same, with a `with_vendor` scope). Denormalization is load-bearing, not an optimization: the split, vendor ability scoping, and vendor order lists all key on it.
...
class Spree::VendorTransfer < Spree.base_class
  ...
  # original minus already-reversed, floored at zero (port of transfer.rb:98)
...
```ruby
# Pluggable execution strategy — CORE base + system no-op; Enterprise adds StripeConnect.
...
# OSS — ships in the spree_stripe extension (not core): BASIC Stripe Connect.
...
#   (ports SpreeStripe::CreateTransfer); pay! → Stripe's native connected-account payout
#   schedule (ports update_payouts_schedule.rb) + VendorPayout rows from payout webhooks;
#   reverse! → ledger-only (records the reversal, moves no money back).
#
# ENTERPRISE — extends the OSS provider with the money OPERATIONS (not in this repo):
#   reverse! → prorated Stripe transfer reversals (ports transfers/reverse.rb) +
...
**Settlement sweep (level 2):** the scheduled job (per vendor `payouts_schedule_interval`) collects `vendor.vendor_transfers.unsettled`, creates one `VendorPayout` with `amount = Σ`, stamps each transfer's `payout_id`, and invokes `provider.pay!`. `system` leaves it `pending` for manual completion; `stripe_connect` settles and a webhook completes it.
...
The legacy gem creates **reverse commission lines** when items are refunded (`create_reverse_item_fees.rb`, plus `create_reverse_order_fee.rb` — an *order-scoped* negative `MarketplaceCommission` that prorates the order's total item commission by `refunded_amount / (total − ship_total)` for whole-order refunds) **and** reverse transfers (`transfers/reverse.rb`). 6.0 keeps both, symmetric — but reversals are always **per line** (a whole-order refund reverses each line's commission exactly; the legacy order-level proration and its rounding fuzz are not carried over):
...
- **SKU uniqueness relaxes to per-vendor scope.** Two vendors legitimately sell the same manufacturer SKU (the legacy gem disables SKU validation outright); 6.0 scopes the uniqueness check by `vendor_id` instead of dropping it.
...
`spree/emails` is deprecated in 6.0, so vendor notifications (new order, payout completed) are **event-driven**: every marketplace model publishes lifecycle events (below) consumable by subscribers and webhooks. The hosted/Enterprise stack ships the actual vendor emails; OSS operators wire webhooks (or a subscriber) to their own mailer/n8n/Slack. This also gives OSS payout automation a seam: `vendor_payout.due` → webhook → operator's banking flow → `POST /vendor_payouts/:id/complete`.
...
- Checkout completion of a mixed cart returns the **order group**: `{ order_group: { id, number, orders: [...] } }`; single-vendor carts keep returning the bare order. Customer account history lists groups with nested per-vendor orders + per-order fulfillment tracking (today's "separate tracking per vendor" UX).

### Admin API surface

Endpoint list lives in `6.0-admin-api.md` → "Marketplace (new in 6.0)": vendors CRUD + `invite`/`approve`/`reject`/`suspend` (approve also un-suspends, matching the legacy machine), commission rates CRUD (nested `rules: []`, PriceList-style), read-only `commission_lines` + `vendor_transfers`, `vendor_payouts` + `POST :id/complete` (manual settlement), read-only `order_groups`. API key scopes: `read_vendors`/`write_vendors`, `read_commissions`/`write_commissions`, `read_payouts`/`write_payouts`. Vendor *users* hit the same admin API with JWT + vendor-scoped abilities — there are no separate "vendor API" routes to maintain (the legacy gem's separate vendor endpoints collapse into ability scoping).
...
1. **From the Enterprise gem to core**, for existing marketplace customers. A one-time backfill rake task (shipped in Enterprise — only Enterprise has legacy data) must:
   - For each legacy `splitted` parent order, create a `Spree::OrderGroup` and attach its `parent_id` children as the group's orders.
   - Migrate `spree_platform_fees` (`MarketplaceCommission`) → `spree_commission_lines`. **Sign convention flips**: legacy stores `order.platform_fee_total` negative (load-bearing in its payout math); `CommissionLine` amounts are positive, reversals are their own negative/`reversed` rows.
   - Decrypt the legacy encrypted columns (`vendors.name`, `contact_person_email` — ActiveRecord encryption, a multi-tenant SaaS choice) into core's plain columns.
   - Migrate `spree_stripe_transfers` (`SpreeStripe::Transfer`) → `spree_vendor_transfers` (the per-order, on-fulfillment level) with `provider: 'stripe_connect'` and the Stripe transfer id in `reference`. The legacy gem has no payout *records* (it delegated scheduling to Stripe), so there is nothing to backfill into `spree_vendor_payouts` — historical Stripe payouts can optionally be reconstructed from Stripe's API, or simply start fresh from the cutover.
   The Enterprise gem then becomes a thin layer on top of core's new models, deleting its splitting + commission + transfer decorators. The basic `Spree::PayoutProvider::StripeConnect` moves to the OSS `spree_stripe` extension (2026-07-15 amendment); Enterprise extends it (clawbacks, netting, KYC ops, reconciliation) and keeps Shopify/WooCommerce.
2. **Build order** (each phase shippable):
   - **Phase 1 — Vendor identity.** `Spree::Vendor` model (see Vendor identity & teams: lifecycle, branding, ToS acceptance, holiday mode), onboarding task registry + core tasks, vendor teams via `RoleUser`/`Invitation` (`resource: vendor`), self-serve application endpoint, product ownership (`products.vendor_id` + approval preference), Admin API Marketplace/vendors endpoints + Store API vendor profiles, vendor CSV import/export on the `Spree::Import` pipeline. No splitting yet.
   ...
   - **Phase 3b — OSS Stripe provider** (in the `spree_stripe` extension, its own release train, after Phase 3): Express onboarding link in the vendor dashboard, `account.updated` status webhook, `transfer!` on fulfillment with `source_transaction`, payout-schedule mapping + payout webhooks. Ledger-only `reverse!`.
...
- **Fund ledger → two levels: `Spree::VendorTransfer` (per-order, on-fulfillment) + `Spree::VendorPayout` (per-interval settlement), both generic in core + pluggable `PayoutProvider` (system no-op in core, StripeConnect in Enterprise)** (RESOLVED 2026-06-25). See Decision 9. Transfer credits the vendor balance on fulfillment; payout sweeps unsettled transfers into a scheduled bank settlement (transfers `belong_to` their payout — auditable). `vendor.balance(currency) = Σ completed transfers − Σ completed payouts` per currency. Splits the legacy `SpreeStripe::Transfer` (which conflated the two and outsourced settlement to Stripe) into core ledger + provider execution along the OSS/Enterprise seam.
...
1. **Store credit / gift card split across vendors.** The references and the legacy gem prorate store credit across child orders. Confirm this survives the 6.0 store-credit model and the `Carts::Complete` flow. (Distinct from the resolved transfer-basis decision — this is about *which child order* a store-credit payment is attributed to during the split, not about the transfer amount, which is now payment-source-agnostic.)
...
## Competitive Benchmark (verified 2026-07-14)

Benchmarked against the closest open-source competitor — an actively developed, MIT-licensed marketplace layer built on a headless JS commerce framework, positioning itself against the enterprise SaaS marketplace suites — from its public docs and repository. **Competitor names are deliberately omitted** (this file ships in a public repo); the maintainers hold the named source notes.

**Different catalog model, deliberately.** The competitor is **offer-based**: a shared *master products* catalog the operator governs, with sellers listing *Offers* (own SKU, price, inventory) against master products — the Amazon-style commodity model, with Buy Box + EAN dedup in its paid tier. Spree 6.0 is **vendor-owned catalog**: each vendor owns their products — the branded-marketplace model our production references run (Maisonette, Goop, Artemest). This is positioning, not a gap; an offers/buy-box layer is explicitly out of scope for 6.0 (could compose later: Offer as an alternate `LineItem` owner-side source).

| Capability | OSS competitor | Spree 6.0 (this plan) |
|---|---|---|
| Seller identity, self-serve + invite, teams w/ roles | ✓ (members, store switching; roles are admin/non-admin only) | ✓ (`RoleUser`/`Invitation` rails; one user can hold roles at N vendors; granular permission sets) |
| Seller lifecycle | approve/suspend/terminate | invited → onboarding → ready_for_review → approved / rejected / suspended (+ soft delete) |
| Product approval | staged `ProductChange` pipeline (also for *edits* to live products) | status-based (draft → operator/pref-gated activation); edit-staging **not** in v1 — acceptable simplification, revisit on demand |
| Order container | `OrderGroup` | `OrderGroup` (domain-neutral, reused for location/availability/B2B splits) |
| Per-seller delivery choice at checkout | ✓ | ✓ (routing packages + vendor-scoped `DeliveryMethod`) |
| Commission engine | fixed (per-currency values) / percentage; rules on 5 dimensions (product, type, collection, category, seller); AND-across/OR-within; specificity precedence; `include_tax`; `include_shipping` (global rate only) | same rule semantics; 3 subject dimensions v1 (Product/Taxon/Vendor — ProductType + Collection are trivial additions once those 6.0 models land); explicit operator priority (more predictable than specificity); min/max caps (they lack); `include_shipping` per-rate (theirs: global only) |
| **Tax ON the commission** | ✗ (no field at all) | **✓ `tax_amount` on every `CommissionLine` — the EU differentiator** |
| Payout execution (OSS) | ✓ Stripe Connect provider incl. onboarding, per-order payout on capture, webhooks | ✓ basic Stripe Connect provider (Express onboarding, on-fulfillment transfers) in the OSS Stripe extension — 2026-07-15 amendment — plus the `system` no-gateway mode and the public `PayoutProvider` interface |
| Payout bookkeeping | per-order payout rows only (capture at a configurable fulfillment status, default `fulfilled`, then a daily payout job); **no balance ledger; refunds do NOT auto-reverse seller transfers** (documented footgun — clawback is custom code); single currency per seller | two-level `VendorTransfer`/`VendorPayout` ledger, **per-currency balances**, itemized settlements, automatic reversal chain on refund |
| Seller promotions | ✓ (cart workflows) | Open Question 5 (post-6.0) |
| Reviews / messaging / return escalations | ✓ reviews, 3rd-party seller↔customer chat, return-escalation flows | reviews + messaging: extension territory; returns via `6.0-returns-exchanges-claims.md` (vendor-scoped through abilities) |
| API surfaces | Admin + Store + separate Vendor API | Admin + Store; vendor users = admin API + ability scoping (one less surface to version — deliberate) |
| B2B | explicitly "in development" — no shipped company accounts/quotes/net terms | 6.1 release theme (Catalog/Company plans exist; PriceLists/customer groups already in core) |
| External-platform vendor onboarding | ✗ in OSS; their paid tier's connector suite is *operator-level* Shopify/Magento/ERP/PIM sync, not vendor self-onboarding | Enterprise **white-label** Shopify/WooCommerce apps — vendors connect *their own* shops, dropship sync (the moat) |

**Monetization symmetry.** The competitor is open-core too: its paid tier sells EAN matching/dedup, a Buy Box engine, master-data governance, multi-channel stock sync, an operator-level connector suite (Shopify/Magento/ERP/PIM), **automated split payouts, vendor KYC** — pricing unpublished, sales-led; the company behind it also monetizes services and managed deployment. So gating the compliance/operations layer is industry-normal. As of the 2026-07-15 boundary amendment the checklist comparison is parity-plus: our OSS also ships a baseline Stripe Connect provider (Express onboarding, on-fulfillment transfers tied to the charge via `source_transaction`) **and** the full ledger, which their OSS lacks entirely. Their documented footgun — refunds never claw back seller transfers — is, in our OSS, a *ledgered* limitation with a paid fix: the books always record the reversal; Enterprise executes the pullback (transfer reversals + netting). Paid-tier differentiation rests on clawbacks, netting, reconciliation, KYC/account-health operations, and DAC7/payout reports — plus EU commission VAT, which they don't model at all, and the white-label external-platform vendor onboarding.
...
- Legacy gem (source-read, v3.8.1 — actively maintained, `spree >= 5.4`): `/Users/damian/github/spree_multi_vendor` — `app/services/spree_multi_vendor/fees/calculate_item_fee.rb` (commission + commission-tax formula), `app/services/spree/orders/create_vendor_order.rb` + `split_by_vendor.rb` (split flow; runs as a 30s-delayed job), `app/models/spree/marketplace_commission.rb` (legacy `spree_platform_fees` — note the class↔table name mismatch), `app/models/spree_stripe/transfer.rb` + `app/services/spree/shipments/create_stripe_transfer_for_order.rb` + `app/services/spree_stripe/transfers/reverse.rb` (legacy fund-transfer/payout, to split into core ledger + Enterprise provider; Stripe Connect is hardwired — no provider abstraction), `app/models/spree_multi_vendor/order_decorator.rb` (legacy parent/child via `parent_id` + `splitted` state; `allowed_vendor_transfer_amount` payout basis at :333), `app/models/spree/vendor.rb` (Vendor model to port; state machine at :133, `stripe_payouts_schedule` at :222), `app/models/concerns/spree/vendor_concern.rb` (the `vendor_id` scoping concern), `app/models/spree/permission_sets/vendor_user.rb` (vendor-user ability baseline), `app/models/spree/reports/vendors_payout.rb` (settlement statement → OSS ledger export).
- Surveyed modern open-source marketplace implementations (primary-source benchmark 2026-07-14 — see Competitive Benchmark; names kept out of this public file): source of the commission rule-engine shape (dimension-grouped matching, `percentage | fixed`, `include_tax`/`include_shipping`), the `OrderGroup` container pattern, and the payout-provider abstraction. **Framework-specific machinery NOT to copy:** big-number jsonb sidecar columns, remote-link join-table indirection (use Rails FKs + `has_many`), and external workflow/step engines (use Spree services + `publish_event`). The leading implementation's payout pipeline creates per-order payouts after capture and keeps no balance ledger and no reversal chain — Spree's two-level ledger is designed from scratch.
