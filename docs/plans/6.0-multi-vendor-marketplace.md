...
**Percentage only (RESOLVED 2026-08-17, shipped).** A flat fee cannot charge delivery: the fee is charged once per sale, so applying the same amount to a parcel bills one sale twice — a £5 rate on a three-item order was charging £20. The flag is refused on a `fixed` rate and the toggle is hidden for one, matching the surveyed OSS engine, which reads its shipping flag off the percentage default only. A marketplace wanting a flat charge on delivery states it as its own rate.
