...
class RenameTaxonsToCategories < ActiveRecord::Migration[7.2]
  def change
    ...
    # Translation tables (missed by the original draft — 2026-07-27):
    # spree_taxon_translations carries the description text column that
    # 6.0-rich-text-descriptions.md Phase 3 populates before this rename
    ...
    add_index :spree_collection_translations, [:collection_id, :locale], unique: true
...
#      re-point rows whose taxon moved to spree_collections or was a deleted
