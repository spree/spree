...
> **Vocabulary note (2026-07-27):** the new `spree_stock_movements` columns are born with **`fulfillment_id`** (never `shipment_id`) per `6.0-fulfillment-and-delivery.md` — same born-correct rule as the split-adjustments tables. Every `shipment_id` below reads as `fulfillment_id`; the backfill still matches legacy rows on `originator_type IN ('Spree::Shipment', 'Spree::Fulfillment')`.
...
- Related plan: `6.0-6.1-split-adjustments.md` (same pattern: replace polymorphic with concrete FKs)
