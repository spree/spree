...
**Depends on:** `6.0-channels-catalogs-b2b.md` (Channel, shipped 5.5), `5.6-6.0-single-store-promotions-payment-methods.md` (single-store PaymentMethod, 5.6), Markets (existing), `6.0-channel-markets.md` (composes, not required)
...
- **Rules evaluate the order's persisted context** (`order.channel_id`, `order.market_id`, `order.total`, `order.customer`), never `Spree::Current` — webhook/job/admin evaluation must not depend on request state.
...
module Spree
  class PaymentMethodRule < Spree.base_class
    ...
    # Declares where PreferenceSchema finds the registered rule kinds
    # (drives `find_by_api_type` + the admin type picker). Superseded the
    # per-class `registered_subclasses` override on 2026-08-06 — a class
    # that declares no registry now raises instead of silently reporting
    # zero subclasses.
    registers_subclasses_via { Array(Spree.payment_method_rules) }
  end
end
```
...
`MarketRule` is identical over `market_ids` / `order.market_id`. `CustomerGroupRule` matches when `order.customer` belongs to any preferred group (guest orders fail a configured group rule — an invoice method restricted to "Wholesale" must never surface to guests). `OrderTotalRule`:
...
- Sibling plan: `docs/plans/6.0-channel-markets.md` (Channel→Markets allowlist; `MarketRule` + a restricted channel compose into "this channel's markets pay this way")
