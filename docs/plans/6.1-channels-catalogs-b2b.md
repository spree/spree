...
API controllers (decisions.md 2026-08-08): access widening (an approver sees
every purchase of their company location) subclasses
`Spree::Storefront::AccessPolicy` via
`Spree::Dependencies.storefront_access_policy_class`; approvals and spending
limits are checkout workflow `validate` hooks; per-location catalogs are data
scoping. Enterprise ships the policy subclass and hook handlers; the seams are
open source. Contact roles never touch `Spree::Ability` or the staff
permission catalog (`6.0-admin-rbac.md`).
