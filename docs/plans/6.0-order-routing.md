...
3. **`Spree::OrderRoutingRule` STI base** + **subclass per rule kind** — rules are *signals* the rules engine considers. Plugins extend by defining a new STI subclass (`class AcmeFresh::Routing::RefrigeratedRule < Spree::OrderRoutingRule`). The persisted `type` column drives polymorphism; no separate registry needed. Mirrors `Spree::PromotionRule`, `Spree::PromotionAction`, `Spree::Calculator`, `Spree::PaymentMethod`.
...
Plugins extend by defining a new subclass — no registry, no boot-time wiring:
...
Once defined and autoloaded, the subclass behaves like any other rule kind: persists via STI, appears in the admin UI, feeds the reducer.
...
This pattern mirrors `Spree::PromotionRule`, `Spree::PromotionAction`, `Spree::Calculator`, and `Spree::PaymentMethod` — established Spree STI patterns. Avoids the boot-order foot guns of a class registry (autoload timing, plugin ordering).
...
### Admin API (Phase 2 — defer to channels)
...
- Admin API + SPA settings page for channels and per-channel routing rules.
...
- **New rule kinds** subclass `Spree::OrderRoutingRule` directly (STI) **and register the class** via `Rails.application.config.spree.order_routing_rules << MyRule`. STI still does the runtime *dispatch* via the `type` column — the registry is the curated allowlist of *available/valid* kinds (it backs admin pickers and the `type` inclusion validation), not a dispatch table. Don't add `if rule.is_a?(Foo)` branches to `Rules` strategy or the reducer; rule classes own their own `#rank` logic.
...
3. **Custom rules in admin UI.** Plugin-registered kinds need to render in the admin SPA. Likely a `config_schema` method on rule classes that returns a JSON schema for the rule's `config` field. Defer to Phase 2.
