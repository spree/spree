...
**Status:** In Progress — Phase 1 implemented in 5.5 (Channel + OrderRoutingRule + strategy base/Rules/Reducer/Legacy); rules admin (Admin API v3 + React dashboard editor, no legacy Rails admin — see decisions.md 2026-07-21) shipped in 5.6; rest of Phase 2+ pending in 6.0
...
**Amended 2026-09-23 — three methods, lifecycle hooks are notifications.** `6.0-typed-stock-movements.md` shipped the stock lifecycle in the workflows themselves (`Orders::Complete` allocates, `Fulfillments::Fulfill` ships, `Fulfillments::Cancel` releases, `Returns::Receive` restocks), so the Phase 3 idea of strategies writing typed movements is superseded and the hooks had no caller at all. They are now wired as post-commit notifications for outside systems (OMS/WMS): `for_sale(fulfillment:)` runs after `Fulfillments::Fulfill` commits, `for_release` after `Orders::Cancel` commits; a raising strategy is reported through `Rails.error` and never undoes the committed change. `for_cancellation` ("a shipped order is canceled") is removed: 6.0 cannot cancel an order once anything ships, and post-shipment reversal is item-level through Returns, which have their own events. Still no defaults — `Base` raises for all three.
  ...
  module OrderRouting
    module Strategy
      class Base
        ...
        # Notify outside systems after Fulfillments::Fulfill commits.
        # The workflow has already written the stock movements; do not change stock here.
...
- ~~Strategy fires in `Spree::Carts::Complete` (the cart→order conversion site post-split).~~ Shipped differently (2026-09-23): the strategy fires wherever fulfillments are proposed — `Cart#rebuild_fulfillments!` during checkout and `Order#rebuild_fulfillments!` for staff-built orders — and `Carts::Complete` copies the cart's routed fulfillments onto the order. `Order#order_routing_strategy`/`#inferred_preferred_stock_location_id` moved to `Spree::Purchase::OrderRouting`, shared by Cart and Order. `Strategy::Rules` now packs its ranking through `Spree::Stock::Coordinator#packages(ranked_locations)`, so delivery-profile coverage and the channel's served locations apply to routed allocations exactly as they do to direct Coordinator callers.
