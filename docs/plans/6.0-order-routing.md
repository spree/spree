...
- **New rule kinds** should subclass `Spree::OrderRoutingRule` directly (STI). No registration, no kind strings — Rails STI does the dispatch via the `type` column. Don't add `if rule.is_a?(Foo)` branches to `Rules` strategy or the reducer; rule classes own their own `#rank` logic.
...
3. **Strategy registration.** Per-Store preference (Phase 1) → per-Channel preference falling back to Store (Phase 2). NOT `Spree::Dependencies` — that bakes "one strategy per app" into the API and would have to be undone for channels.
