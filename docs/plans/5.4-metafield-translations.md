...
- Accept HTML string, store it in the translation table's `value` column
- **Sanitization is not wired for custom fields yet.** The 5.6.2 `Spree::RichTextSanitizer` covers `Spree::Product#description` and its translations only; the base RichText value still goes through ActionText's Trix allowlist, which is a *different* ruleset. Applying `sanitizes_rich_text` to custom-field values (base + translations, one shared allowlist) lands with `6.0-rich-text-descriptions.md` Phase 5, which also moves the base value off ActionText. Until then, treat stored custom-field rich text as unsanitized by Spree's own sanitizer.

This asymmetry is temporary — `6.0-rich-text-descriptions.md` **Phase 5** (2026-07-27 revision) migrates `has_rich_text :value` to the existing `value` text column with sanitization, at which point RichText metafields are fully symmetric.
