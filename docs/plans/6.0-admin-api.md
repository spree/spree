...
10. **Returns/Exchanges/Claims, Product Types, Collections, Marketplace (Vendors/Commission/Payouts), Delivery Methods/Zones, Tax Rates/Markets-as-tax, Payment Methods ✅** — remaining 6.0 model work; Catalogs/Companies (B2B) deferred to **6.1** per `decisions.md` 2026-06-16
...
> **6.0:** `Spree::Supplier` is the procurement source for purchase orders (see `6.0-inventory-operations.md`). Not to be confused with marketplace **Vendors** below — `Spree::Vendor` is the marketplace seller (`6.0-multi-vendor-marketplace.md`, `decisions.md` 2026-07-14).
...
Per `6.0-multi-vendor-marketplace.md`. Vendor prefix is `ven_` (matches the legacy Enterprise gem).
...
- [ ] `POST /api/v3/admin/vendors/:id/invite` — (re)send the vendor invitation (`Spree::Invitation`, `resource: vendor`)
- [ ] `POST /api/v3/admin/vendors/:id/approve` / `POST /api/v3/admin/vendors/:id/reject` / `POST /api/v3/admin/vendors/:id/suspend` — lifecycle transitions (approve also un-suspends)
...
- [ ] `GET /api/v3/admin/commission_lines` — read-only snapshot ledger (filter by vendor/order)
- [ ] `GET /api/v3/admin/vendor_transfers` — read-only earnings ledger (filter by vendor/payout/status)
- [ ] `GET /api/v3/admin/vendor_payouts` / `GET /:id`
- [ ] `POST /api/v3/admin/vendor_payouts/:id/complete` — manual settlement (system provider)
- [ ] `GET /api/v3/admin/order_groups` / `GET /:id` — the multi-order transaction container

API key scopes: `read_vendors` / `write_vendors`, `read_commissions` / `write_commissions`, `read_payouts` / `write_payouts`.

> **Operator surface only (2026-08-15).** Vendor *users* never call the Admin API. They are `Spree::AdminUser` accounts signing in through a dedicated **`/api/v3/vendor`** branch with its own `vendor_api` JWT audience and login registry, where every endpoint scope-fetches through `current_vendor`; the vendor endpoint list and serializer rules live in `6.0-multi-vendor-marketplace.md` → "Vendor API surface" (Decision 10, `decisions.md` 2026-08-15).
