...
> **6.0:** `zone_id` FK is dropped. `TaxRate` gets direct `country_iso` + `state_abbr` string columns (amended 2026-08-07 — ISO codes, not FKs, per `6.0-drop-country-state-models.md`). Tax logic moves behind a `TaxProvider` interface. See `6.0-tax-provider.md`.
