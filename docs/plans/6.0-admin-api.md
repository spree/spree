...
>
> **Built by the tax-provider plan (2026-08-05), Phase 3.** These rows are the Internal provider's entire configuration, so that plan cannot verify its own tax calculation without them, and it specifies the serializer alongside the provider work. They ship against the `zone_id` shape and switch to `country_id`/`state_id` in its Phase 5 — tick these boxes from there rather than building separately.
