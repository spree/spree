...
| `/api/v3/admin/orders/:id/adjustments/*` | `/api/v3/admin/orders/:id/tax_lines`, `discount_lines`, `fees` (read-only) | Polymorphic `Adjustment` → typed `TaxLine`, `DiscountLine`, `Fee`. `discount_lines`, not `discounts` — that name stays with the applied-promotions array (`OrderPromotion`). See `6.0-split-adjustments.md` Resolved Question 10 |
...
### Discount Lines (nested under Orders, read-only; replaces Adjustments in 6.0)
- [ ] `GET /api/v3/admin/orders/:order_id/discount_lines`

### Fees (nested under Orders, read-only; replaces Adjustments in 6.0)
- [ ] `GET /api/v3/admin/orders/:order_id/fees`

> **6.0:** Polymorphic `Spree::Adjustment` is dropped. Tax, discount, and fee data is returned via typed, read-only endpoints. The typed discount endpoint is `discount_lines` — `discounts` remains the applied-promotions array on the order (backed by `OrderPromotion`). See `6.0-split-adjustments.md` Resolved Question 10.
