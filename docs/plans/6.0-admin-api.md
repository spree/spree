...
>
> **Rich-text write contract (2026-07-27, per `6.0-rich-text-descriptions.md`):** reads are `description` (plain) + `description_html` (HTML); **writes accept `description_html`** — sanitized by the model-owned bridge. HTML under the plain `description` param is rejected (422), preserving read/write symmetry. Same pattern for `internal_note`/`internal_note_html` on Orders/Customers.
>
> **Correction (2026-08-27, `6.0-store-policies.md`):** the `*_html`-on-write half of the paragraph above was never implemented. `Spree::SanitizableRichText` defines `field_html` as a **read-only** reader and sanitizes writes made to the plain attribute, which is what every shipped controller permits (`Admin::CategoriesController` takes `:description`, not `:description_html`). Policies follow the shipped behaviour: read `body` + `body_html`, **write `body`**. Treat the plain attribute as the write target until a bridge actually exists.
