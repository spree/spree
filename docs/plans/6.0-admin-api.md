...
- [ ] `GET /api/v3/admin/vendors` / `GET /:id` / `POST` / `PATCH /:id` / `DELETE /:id`
- [ ] `POST /api/v3/admin/vendors/:id/invite` — (re)send the vendor invitation (`Spree::Invitation`, `resource: vendor`)
- [ ] `POST /api/v3/admin/vendors/:id/approve` / `reject` / `suspend` — lifecycle transitions (approve also un-suspends)
- [ ] `GET /api/v3/admin/commission_rates` / CRUD + nested `rules: []` on the PATCH body (PriceList-style)
