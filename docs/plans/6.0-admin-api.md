...
> Custom-role CRUD ships with `6.0-admin-rbac.md`: DB-backed role permissions
> (catalog keys), a `GET /permissions` catalog endpoint, `/me.permission_keys`,
> and the `settings` → `staff` scope split for this whole staff group.
