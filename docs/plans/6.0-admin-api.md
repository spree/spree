...
>
> **Rich-text write contract (2026-07-27, per `6.0-rich-text-descriptions.md`):** reads are `description` (plain) + `description_html` (HTML); **writes accept `description_html`** — sanitized by the model-owned bridge. HTML under the plain `description` param is rejected (422), preserving read/write symmetry. Same pattern for `body`/`body_html` on Policies (whose Admin CRUD is built in that plan's Phase 4 — no policies controller exists today) and `internal_note`/`internal_note_html` on Orders/Customers.
