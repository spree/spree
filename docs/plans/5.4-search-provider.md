...
**Status:** In Progress — the 5.4 portion is in (`Spree::SearchProvider::Base/Database/Meilisearch`, PgSearch removed, `add_search_scope` removed); 6.0 metafield faceting pending. 2026-07-19: spree-starter / create-spree-app no longer provision Meilisearch by default — the Database provider is the out-of-the-box default everywhere; Meilisearch is a documented opt-in — installs: uncomment the compose blocks + set `MEILISEARCH_URL`; monorepo dev server: `SPREE_MEILISEARCH=1 pnpm server:dev` (chains `scripts/docker-compose.meilisearch.yml`); either way run `spree:search:reindex` once on first boot. See decisions.md. **2026-08-12:** the Meilisearch provider left core for its own `spree_meilisearch` gem (`spree/providers/meilisearch`), as this plan always intended — `Spree::SearchProvider::Meilisearch` → `SpreeMeilisearch::SearchProvider` and the document presenter moved with it (`Spree::Dependencies.search_product_presenter` now has NO core default; the gem registers it, and `#search_presentation` raises a directive error without one). Core keeps the interface (`Base`/`Database`/`SearchResult`/`FiltersResult`/`CustomFieldSchema`), the indexing jobs and the reindex task; the `meilisearch` gem is no longer a core dependency. Old constants resolve with a deprecation warning through `Spree::SearchProvider.const_missing` until 6.1.
...
- **Meilisearch is the recommended upgrade path.** For merchants who want typo tolerance, stemming, and relevance tuning, install `spree_meilisearch`. This gives them far more than PgSearch ever did.
...
- **Ship `spree_meilisearch` as an official Spree gem.** Uses `meilisearch-ruby` (raw client), NOT `meilisearch-rails` (no model-level overrides).
...
### Meilisearch Provider (`spree_meilisearch` gem)
    ...
    def serialize(product)
      {
        ...
        name: product.name,
        # plain text, not raw HTML — else tag names/attributes become searchable
        # tokens and pollute highlights (6.0-rich-text-descriptions.md Phase 5)
        description: Spree::RichTextHelper.to_plain_text(product.description),
        slug: product.slug,
...
### Phase 5: Meilisearch provider (official gem) ✓ shipped

- Create `spree_meilisearch` gem (official, maintained by Spree team) ✓
