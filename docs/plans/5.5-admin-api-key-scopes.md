...
> **Superseded in part (2026-08-07 — see `6.0-admin-rbac.md`):** at 6.0 the scope
> vocabulary becomes THE permission catalog — the single grant vocabulary for
> staff roles and API keys alike (permission sets are deleted with no
> compatibility bridge). `Spree::ApiKey::SCOPES` derives from that catalog instead of a frozen
> literal (extension-registered resources become mintable scopes), and the staff
> endpoints (`/admin_users`, `/invitations`, `/roles`) move off `settings` into a
> new `read_staff`/`write_staff` pair. The "don't add scope checks to
> JWT-authenticated paths" decision below is also superseded: the key gate
> generalizes to JWT staff (their role keys play the role scopes play for keys),
> becoming the primary admin enforcement for both principals. Still standing:
> resource granularity, `write_*` implies `read_*`, the aliases, per-controller
> `scoped_resource` declarations (now additionally the source of 403
> `required_permission` diagnostics), and scope immutability on secret keys.
...
- **Scopes apply to API keys only.** JWT admin users keep CanCanCan-based authorization (they have `Spree::Role`s; that surface is unchanged). *Superseded at 6.0: JWT staff pass the same key gate — see the notice above.*
...
| `read_settings` / `write_settings` | `/payment_methods`, `/markets`, `/countries`, `/tax_categories`, `/stores`, `/channels`, staff (`/admin_users`, `/invitations`, `/roles`), `/allowed_origins`, `/custom_field_definitions/*` (schema config, consolidated — no dedicated scope) |
...
- **Don't add scope checks to JWT-authenticated paths.** Scopes are an API-key concept only. *Superseded at 6.0: the generalized key gate covers JWT staff — see the notice above.*
