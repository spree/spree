...
As shipped for PaymentMethod (`ReplacePaymentMethodDisplayOnWithStorefrontVisible`):
...
**The conversion runs inside the migration, not in a rake task** — the usual "data transformations go in rake tasks" rule assumes the source column still exists when the task runs. Here the migration drops `display_on` in the same statement, so a follow-up task would have nothing left to read. Everything except `back_end` keeps the column default of `true`, which is why only one narrow `UPDATE` is needed. CustomFieldDefinition converted the same way; DeliveryMethod is the one exception, because its migration only added the column and left the backfill to `spree:migrate_shipping_to_delivery`, which converts and then clears `display_on` so re-running is safe.
...
2. Update Spree to 6.0 — run `bin/rails db:migrate`. Payment methods and custom field definitions convert atomically inside their own migrations, so there is no separate task to run and no window where the data is half-converted. Delivery methods convert in `spree:migrate_shipping_to_delivery`, already part of the 5.6 → 6.0 upgrade manifest.
