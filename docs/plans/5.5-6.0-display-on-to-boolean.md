...
**Status:** 5.5 bridge shipped (`storefront_visible` accessor + Ransacker on the `Spree::DisplayOn` concern). **DeliveryMethod completed the 6.0 flip on 2026-08-05** — real `storefront_visible` boolean column (`null: false, default: true`), off the concern, `display_on` kept as a deprecated reader/writer until 6.1; the tri-state `DISPLAY_ON_*` rate filter became `DeliveryMethod::STOREFRONT`/`BACKOFFICE`. **PaymentMethod and MetafieldDefinition are still on the concern** — their schema rename remains pending.
...
- **Custom Fields (`MetafieldDefinition`) — handled by `5.4-6.0-custom-fields-rename.md`**. Don't try to consolidate the two plans; the custom-fields one is further along and renames more than just the visibility field.
