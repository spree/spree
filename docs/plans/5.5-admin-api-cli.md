...
1. **Core 5.5.x patch** (`spree/core`): `SCOPES` env on `spree:cli:create_api_key` (fixes secret-key minting), `read_promotions`/`write_promotions` in `Spree::ApiKey::SCOPES`. Specs for both.
2. **`@spree/cli` minor**: add `@spree/admin-sdk` dependency; new `src/config.ts` (resolution chain + credential files); `src/commands/auth.ts` and `src/commands/api.ts` following the existing `registerXCommand` pattern; `--scopes` flag on `api-key create`; a build step copying `docs/api-reference/admin.yaml` into the package as the versioned snapshot; Changesets entry.
3. **Scaffolding**: `create-spree-app` (and the spree-starter template repo) add `.spree/` to `.gitignore`.
4. **agent-skills**: `spree-cli` skill PR to `spree/agent-skills`.
5. **Docs**: a page under `docs/developer/` covering the command group, credential layers, and agent usage (Claude Code snippet with the two env vars).
