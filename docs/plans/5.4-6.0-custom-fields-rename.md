...
| `Spree::Metafields::RichText` | `Spree::CustomFields::RichText` | STI subclass. **Value storage also migrates in the same 6.0 wave**: `has_rich_text :value` (ActionText) → the existing `value` text column, sanitized — owned by `6.0-rich-text-descriptions.md` Phase 5 (the dashboard already writes Tiptap HTML through ActionText's Trix allowlist today) |
