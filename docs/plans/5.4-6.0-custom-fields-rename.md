...
**Status:** Implemented — 5.4 serializer bridge ✓; 5.5 alias classes ✓; 6.0 model/table rename + visibility simplification ✓ (2026-08-09)
...
## 6.0 delivery notes (what actually shipped)

The rename landed in one wave. Deviations from the plan as written above:

- **Deprecation bridges everywhere, not a clean break.** Per the standing convention, every legacy name keeps working for one release with a warning: `Spree::Metafield`, `Spree::MetafieldDefinition`, `Spree::Metafields::*` (constant aliases), `include Spree::Metafields` (the module doubles as concern and STI namespace), `Spree.metafields`, the `metafields`/`public_metafields`/`private_metafields` associations, `set_metafield`/`get_metafield`/`has_metafield?`/`metafields_attributes=`, `.with_metafield_key*`, and the renamed definition columns (`name`, `metafield_type`, `display_on`). All are removed in 6.1.
- **`private_metafields` has no forward equivalent.** The plan dropped it; the deprecated association still resolves (backed by the new `admin_only` scope), but there is no `admin_only_custom_fields` association. Read `custom_fields` and filter, or query `Spree::CustomField.admin_only`.
- **`name` → `label` is a real column rename**, not the 5.5 `alias_attribute`. The definitions table now has `label`; `field_type` replaces `metafield_type` and holds the STI class name while the `field_type` *reader* returns the API token (`short_text`). `field_type_class_name` exposes the raw class name — internal callers that set a custom field's STI `type` must use it.
- **CSV headers are a clean break** (decision 2026-08-09): export and import both use `custom_field.<namespace>.<key>`; the legacy `metafield.` prefix is not accepted. Documented as BREAKING in the 5.6→6.0 upgrade guide.
- **No upgrade rake task.** All persisted data (STI type strings, both column renames, the `display_on` → `storefront_visible` conversion) is handled inside the migration, because a renamed STI class cannot load its own legacy rows — the substitution has to happen before the app boots on the new code. Nothing was left for the manifest.
- **The metadata cleanup was deferred** (decision 2026-08-09). Dropping `public_metadata` and renaming `private_metadata` → `metadata` spans nearly every table and is independent of this rename; it needs its own migration and review.
- **`Spree::DisplayOn` stays** for PaymentMethod and DeliveryMethod. Only `CustomFieldDefinition` left the concern, gaining a real `storefront_visible` boolean column.
- **Rich-text custom fields left ActionText** (the slice of `6.0-rich-text-descriptions.md` that overlaps this rename). `Spree::CustomFields::RichText` now stores sanitized HTML in the shared `value` column like every other type; the migration copies existing bodies across, leaving the Action Text rows as the rollback path. The other five ActionText users (Category, Collection, Policy, `Order#internal_note`, customer internal notes) and the gem dependency stay with that plan.
- **`Spree::Metadata` no longer includes the custom-fields concern.** Including one silently pulled in the other, which contradicts the two-system boundary this rename exists to draw — and the guard was inert anyway, testing the concern's own ancestors rather than the including model's. Models wanting both now include both.
...
| `Spree::Metafields::RichText` | `Spree::CustomFields::RichText` | STI subclass. Value storage migrated in the same wave ✓: `has_rich_text :value` (ActionText) → the existing `value` text column, sanitized via `sanitizes_rich_text` — the slice of `6.0-rich-text-descriptions.md` Phase 5 that this plan owns |
