...
**Internal method vocabulary follows the models (2026-08-05).** Core code now calls the delivery/fulfillment names everywhere; the shipping-era names survive one release as `Spree::Deprecation.warn` bridges. Renamed: `Stock::Estimator#shipping_rates` → `#delivery_rates` (and its internals — `calculate_`/`choose_default_`/`sort_`, plus the `shipping_methods` filter seam → `#delivery_methods`, which host apps override), `Stock::Package#shipping_rates`/`#to_shipment` → `#delivery_rates`/`#to_fulfillment`, `Stock::Coordinator#shipments` → `#fulfillments`, `Order#line_items_without_shipping_rates` → `#line_items_without_delivery_rates`, `Order#ensure_available_shipping_rates` → `#ensure_available_delivery_rates` (private — no bridge), and `LineItem#shipping_cost`/`#display_shipping_cost` → `#delivery_cost`/`#display_delivery_cost`. **`Spree::Cart` gets no deprecation bridges at all** — it is new in 6.0, so no legacy caller can exist; its equivalent is simply `#ensure_available_delivery_rates`.
