...
1. ~~Owner shape~~ — **resolved 2026-07-27** with the cart-order-split expansion: dual `cart_id`/`order_id` exactly-one is the unified pattern (LineItem included, no polymorphism); Phase 1b's `cart_id` is unblocked.
