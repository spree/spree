...
async function addDashboard(ctx: ProjectContext) {
  ...
  //    ctx.apiDir is 'api' or 'backend' — works for both pre-5.6 and 5.6+ projects.
...
- Pinned to `@spree/dashboard@^5.6.0-next` and `@spree/admin-sdk@^5.6.0-next` (Developer Preview dist-tag), rolling forward to `^6.0.0` at 6.0 GA.
