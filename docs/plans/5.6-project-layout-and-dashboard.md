...
- **Rails host** *(implemented)*: `Spree::Api::DashboardAppController` serves `Spree::Config.dashboard_dist_path` (env fallback `SPREE_DASHBOARD_DIST_PATH`) at `/dashboard` with SPA semantics — path fallback to `index.html` (no-cache), `assets/` immutable, traversal-guarded, 404 when unconfigured.
...
- **Ejected projects**: same stage shape, but `COPY apps/dashboard/` from the build context instead of extracting the template (requires widening the eject build context or compose `additional_contexts` — the wrapper's compose currently builds from `./backend`).
...
- **6.0**: when the Classic Admin retires, the SPA moves from `/dashboard` to `/admin` and this becomes the default admin delivery. The static/CDN topology (see the deployment docs) remains the alternative for customized dashboards on independent release cadences.
