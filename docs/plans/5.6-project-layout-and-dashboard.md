...
### Phase 3: Create `packages/dashboard-starter` *(implemented)* + sync job
...
4. Add the release sync job: copy to `spree/dashboard-starter`, rewrite `workspace:^` → published versions, strip monorepo-only devDeps, push. Mark the target as a GitHub template repository.
...
### Phase 5: Add dashboard scaffolding to create-spree-app

1. Implement `src/dashboard.ts` mirroring `src/storefront.ts`.
2. Add prompt + flag in `src/prompts.ts` and `src/index.ts`.
3. Update `src/scaffold.ts` to invoke dashboard phase after api, before storefront.
4. Update README template with Dashboard section.
5. Update tests.
...
5. **Build-time typed route composition (future).** Plugin routes currently go through the runtime route registry + catch-all dispatcher — simple, works for in-app customization, but plugin-route `<Link>`s need `as never` casts and get no route-level code splitting. Since the shell is source-only and the Vite plugin already knows the discovered plugin list at build time, a later phase could additionally compile statically-declared plugin routes into the host's route tree (typed links, per-route chunks) while keeping the runtime registry for dynamic cases. Requires route declarations to be importable without executing the full plugin module — likely a conventional `dashboard/routes` entry. Not preview-blocking; design when the first big plugin needs it.
