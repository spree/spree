...
**Status:** Superseded (2026-09-02) — the two tables are dropped in 6.0 and replaced by `cancel_reason` + `cancel_note` columns on the order; see "Outcome" below
...
`Spree::OrderCancellation` and `Spree::OrderApproval` shipped in 5.5.4 and
5.6 as write-only tables: the cancel and approve services inserted a row,
and nothing — no API endpoint, serializer, admin view, mail, export or
rake task — ever read one. The 5.5 backfill task was never written, the
cancel endpoint never accepted a reason, so every row carried `other`,
and the `restock_items` / `refund_payments` flags on the record described
decisions the workflow did not actually take (items are always restocked
through fulfillment cancellation; captured payments on an ordinary order
are always canceled). An audit row that misstates what happened is worse
than none.

The rest of the plan was reassessed against the platforms it cited and
the 6.0 decisions made since:

- Every major platform keeps the cancellation timestamp and reason **on
  the order** — no side table — which is also what the codebase already
  needs: Ransack filters, sorting, the CSV export and staff anonymization
  all read `canceled_at` as a column. Dropping the columns would have
  fought all of that for no reader.
- Multi-level merchant-side approval of a placed order has no industry
  precedent; the comparable feature elsewhere is a fulfillment hold, and
  `considered_risky` + `approved_at` already cover the fraud-review case.
- Buyer-side (company) approvals are Enterprise scope (decisions.md
  2026-08-30) and ride an Enterprise record of their own.

**What 6.0 ships instead** (`ReplaceOrderCancellationTablesWithCancelColumns`):
a fourth merchant-owned reason vocabulary,
**`Spree::OrderCancellationReason`** — `NamedType` like `ReturnReason` /
`ClaimReason` / `RefundReason`, store-scoped, `restrict_with_error` while
orders reference it, seeded per store and managed on the existing
Settings → Reasons page — bound to the order through
`spree_orders.cancel_reason_id`, alongside a free-text `cancel_note`.

The reason is deliberately **not** a fixed string list: nothing branches on
the value, and the 2026-08-07 `Claim#claim_type` ruling already rejected a
fixed vocabulary sitting beside a merchant-owned one. It is equally
deliberately **not** a `kind` column on a merged reason table — the four
vocabularies carry different associations and delete guards, `RefundReason`
has named lookups core itself depends on, and `NamedType` already factors
out everything they share (typed tables per grain, per the quantity-rules
doctrine).

`Spree::Orders::Cancel` takes `reason:` as a record and refuses one
belonging to another store; the admin cancel endpoint accepts
`cancel_reason_id` / `cancel_note`, and the admin order serializer exposes
`cancel_reason_id` + `cancel_reason_name` with the record behind
`?expand=cancel_reason`. Both old tables are dropped `if_exists`, the 5.5
migrations deleted so fresh installs never create them, models and specs
removed. `Orders::Approve` keeps `approved_at` / `approver_id` as the
risk-review clearance it always was; `level:` / `note:` (Approve) and
`restock_items:` (Cancel) warn and are ignored until 6.1. No data is
carried over: no row ever held anything but the default reason.

A reason stays **optional**, matching `Return#reason` and `Claim#reason`,
so a merchant who has not curated the vocabulary can still cancel.

**Constraints now:** no cancellation or approval history tables; a new
reason vocabulary is its own `NamedType` table, never a `kind` on a shared
one; a future customer self-cancel makes the actor on the order
polymorphic (a column change), never a side table; company approval flows
stay Enterprise.
