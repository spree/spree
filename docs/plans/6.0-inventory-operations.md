...
**Status:** Draft
**Target:** Spree 6.0 (transfers, purchase orders, history) / 6.1 (lot tracking — deferred 2026-08-30 scope review: purely additive, opt-in, nothing in 6.0 or Enterprise depends on it)
...
4. **Lot-level inventory** (added 2026-08-30, interactive review — full lot tracking chosen over provenance-only; **6.1** per the same-day scope trim). New `Spree::StockLot` decomposes a lot-tracked variant's on-hand quantity per location into lots (`lot_number`, optional `expires_at`); receives assign lots, dispatch picks them FEFO, and every stock movement on a lot-tracked variant names its lot — so a merchant can answer "which LOT/pallet went into this shipment" and "what expires next", the two questions wholesale and regulated-goods merchants actually ask.
...
### Lot tracking (added 2026-08-30 · targeted 6.1)

- **Lots are opt-in per variant** (`lot_tracked` boolean). Untracked variants — the vast majority — see zero behavior change; no lot rows, no lot columns on their movements.
- **A lot is a per-location quantity bucket, not a global batch record.** `Spree::StockLot` belongs to a variant + stock location and carries `lot_number`, `quantity`, `expires_at` (nullable), `received_at`, and optional provenance FKs (`purchase_order_id` / `stock_transfer_id`). Uniqueness on `(stock_location_id, variant_id, lot_number)`. The same physical batch split across two warehouses is two rows sharing a `lot_number` — receiving reconciles by number.
- **Lot quantities are a decomposition of `count_on_hand`, kept in sync transactionally.** For a lot-tracked variant, `SUM(stock_lots.quantity)` must equal the stock level's `count_on_hand` at that location — the same invariant discipline as the license-pool count sync in `6.0-digital-assets.md`. Every movement that changes `count_on_hand` on a lot-tracked variant adjusts a lot in the same transaction; drift is a bug, never reconciled lazily.
- **Movements name their lot.** `spree_stock_movements` gains a nullable `stock_lot_id`; a physical quantity change spanning several lots is several movement rows, one per lot. This keeps the typed-movements doctrine intact — `count_on_hand` still only ever changes through movements; lots add *which shelf section*, not a second write path.
- **Receiving assigns lots.** PO and transfer receive payloads accept per-item lot allocations (`[{ lot_number, expires_at, quantity }]`); receiving finds-or-creates the destination lot and writes the `received` movements against it. A lot-tracked item received without lot data is refused — the receive screen is the one moment the information exists.
- **Allocation stays lot-agnostic; dispatch picks lots.** Per `6.0-typed-stock-movements.md`, stock leaves the shelf at fulfillment: `allocated` movements (order placement) carry no lot — an allocation is a promise, not a physical pick. The `shipped` movements written at dispatch pick lots **FEFO** (earliest `expires_at`, then oldest `received_at`), with a per-fulfillment override so the warehouse can record what was actually grabbed. The fulfillment's movements are the record of which lots shipped, surfaced on the fulfillment page and in the variant history (`+200 received · PO-42 · LOT 2026-113`).
- **Returns and adjustments are lot-aware** for tracked variants: a return receive lands in the lot it shipped from when known (else a merchant-chosen lot), and manual adjustments name the lot they correct.
- **No storefront exposure in 6.0.** Lots are back-office data; nothing in the Store API reads them.

```ruby
class Spree::StockLot < Spree.base_class
  has_prefix_id :lot
  ...
  has_many :stock_movements, class_name: 'Spree::StockMovement', inverse_of: :stock_lot

  validates :lot_number, presence: true,
                         uniqueness: { scope: [:stock_location_id, :variant_id, *spree_base_uniqueness_scope] }
  validates :quantity, numericality: { greater_than_or_equal_to: 0, only_integer: true }

  scope :available, -> { where('quantity > 0') }
  scope :fefo, -> { order(Arel.sql('expires_at IS NULL, expires_at ASC, received_at ASC')) }
end
```

Admin surface: lots listed on the variant Inventory tab and the stock-location Activity view (with expiry badges); lot filter on the movement history; per-lot adjustment from the lot row. API: `GET /api/v3/admin/stock_lots` (Ransack-filterable by variant/location/expiry) + lot allocations embedded in the existing receive/adjustment payloads — no standalone lot mutation endpoints beyond adjustment, since lots change through receives and picks.
...
class AddInventoryOperations < ActiveRecord::Migration[7.2]
  def change
    ...
    create_table :spree_stock_lots do |t|
      t.references :variant,        null: false
    ...
    add_index :spree_stock_lots,
              [:stock_location_id, :variant_id, :lot_number],
              unique: true,
              name: 'idx_stock_lots_unique'

    add_column :spree_stock_movements, :stock_lot_id, :bigint
    add_index  :spree_stock_movements, :stock_lot_id
  end
end
```
...
- `StockTransfer` gains a state machine + line items; the old atomic `transfer`/`receive` class methods are removed.
...
- **Don't put lot numbers anywhere but `Spree::StockLot`.** No `lot_number` strings on line items, fulfillments, or movements-as-text — a movement references its lot by FK, and everything else reads through the movement. A lot quantity never changes outside the transaction that writes its movement.
...
5. **Expired lots and sellability.** Should a lot past `expires_at` be excluded from FEFO picking (forcing a manual override), and should expiring stock reduce the availability `Stock::Quantifier` reports? Leaning: warn-and-skip in FEFO, no availability change in 6.0 — availability semantics touch checkout and deserve their own decision. Unblocked by: implementing the receive/pick flows and seeing what merchants on lot-tracked catalogs report.
6. **Turning `lot_tracked` on for a variant with existing stock.** The existing `count_on_hand` has no lot decomposition; the flag flip likely requires an opening "untracked" lot holding the current quantity (adjusted into real lots by the merchant). Decide during implementation.
