...
9. **OptionValue is its own translatable resource**, not a nested special-case of OptionType. It already declares `TRANSLATABLE_FIELDS` and is in the registry; give it its own nested `…/option_values/:id/translations` route so the generic controller stays branch-free. (The legacy admin special-cases OptionValue inside OptionType — we deliberately do not port that.)
...
concern :translatable do
  resources :translations, only: [:index, :create],
            controller: '/spree/api/v3/admin/translations', param: :locale do
    collection { delete ':locale', action: :destroy, as: :locale }
  end
end
...
resources :option_types, concerns: [:custom_fieldable, :translatable] do
  resources :option_values, only: [:index, :show], concerns: [:translatable]
end
resources :taxonomies, concerns: [:translatable]
resources :policies,   concerns: [:translatable]
# Store translations: a singular nested route under the current store settings.
```
...
module Spree::Api::V3::Admin
  ...
  class TranslationsController < ResourceController
    ...
    # GET .../:parent/:parent_id/translations
    def index
      render json: serialize_translations(parent_resource)
    end

    # POST .../:parent/:parent_id/translations  (upsert; multi-locale, multi-field)
    def create
      authorize! :update, @parent
      @parent.upsert_translations(translation_params)
      render json: serialize_translations(@parent.reload)
    rescue ActiveRecord::RecordInvalid => e
      render_validation_error(e.record.errors)
    end

    # DELETE .../:parent/:parent_id/translations/:locale
    def destroy
      authorize! :update, @parent
      @parent.destroy_locale_translation(params[:locale])
      head :no_content
    end
    ...
    # { "fr" => { "name" => "…" } } — locale/field filtering + validation happen
    # in TranslatableResource#upsert_translations.
    def translation_params
      params.require(:translations).permit!.to_h
    end
  end
end
```
...
module Spree::Api::V3::Admin
  class ProductSerializer < V3::ProductSerializer
    attribute :translations, if: proc { expand?('translations') } do |product|
      Spree::Translations::Matrix.for(product) # { "de" => { "name" => …, "translated_field_count" => 5 }, … }
    end
  end
end
```
