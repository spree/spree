...
**Status:** Draft
**Target:** Spree 5.5 (Phase 1 API) → 6.0 (bulk + coverage + centralized page + metafields fold-in)
...
- **Reject staleness tracking entirely** — Shopify is the only one of the four that tracks whether a translation has drifted from its source, and its mechanism (a mandatory digest on every write) is a concurrency guard as much as a drift display. Three of four ship nothing. See Key Decision 8.
...
8. **No staleness tracking (decided 2026-09-02).** Earlier drafts planned a server-side `source_digest` per translation cell so the matrix could flag a translation whose source has since been edited. **Cut.** The business case is real but unproven — only Shopify ships anything comparable, and its digest is mandatory-on-write (a concurrency guard), not the advisory display flag we would have built; Vendure, Medusa and Saleor track nothing. It was also the most expensive item in Phase 2 (a migration across every translations table or a new polymorphic one, a write path on every cell, a read-time comparison, a verbose response mode, a backfill task) while being the only one nothing else depends on. It stays **purely additive**: no column, endpoint or payload shape here forecloses it, so it can land later on its own merits — ideally once merchants using the coverage page report source drift as an actual complaint.
...
13. **CSV stays product-only; generalizing across the registry is not built (decided 2026-09-02).** `Spree::Imports::ProductTranslations` and `Spree::Exports::ProductTranslations` already ship, are registered in the engine's import/export type lists, and carry a sample CSV — the Phase 2 item was always to *generalize* them, never to build them. On review that generalization has no demand behind it: bulk translation volume is a product problem, and the other registry members are edited a handful of records at a time through the per-resource dialog, which already exists. So this run **wires the existing product types into the dashboard's import-type registry** (they were never exposed there) and puts Import/Export on the centralized page. A registry-driven `Spree::Imports/Exports::ResourceTranslations` remains a clean follow-up: the row format is settled (long — see decision 14), so it is parameterization work, not design work.
14. **CSV row format is long, one row per record × locale (decided 2026-09-02, closing Open Question 6).** Identifier column, `locale` column, then one column per translatable field. This is exactly the shipped product format, so nothing merchants already have breaks, and a future generic importer inherits it. The wide alternative (one column per field × locale) was rejected: it breaks the existing format, needs a back-compat path, and its header set grows with fields × locales.
15. **Coverage counts every translatable field, not just the first (decided 2026-09-02).** A grid cell reports how much of a record is translated for a locale — a full check at 100%, the count otherwise (`2/5`), a dash at zero. The legacy Rails page counted a locale as translated when `name` alone was present, which showed a full check for a product whose description was untranslated. Counting all fields also keeps the grid consistent with `translated_field_count`, which the per-resource matrix already returns, so the two surfaces never disagree.
...
> **Note on rich text:** `Taxon#description` and `Policy#body` use the ActionText Mobility backend today (removed at 6.0 per `6.0-rich-text-descriptions.md`). Translated HTML must be sanitized on write per that plan. The `fields[].type == "html"` hint (below) tells both the serializer and the sanitizer how to treat the field. **Deliberate, documented divergence** (2026-07-27): this surface keys on the model attribute (`description` carrying HTML, both directions — symmetric *within* the matrix), while resource serializers use the `description` (plain) / `description_html` (HTML) pair with writes on `description_html`.
...
### Staleness — considered and cut

An earlier draft specified a `source_digest` per translation cell, a `stale`
flag computed at read time, and a `?include=staleness` verbose read shape. This
was **cut on 2026-09-02** (Key Decision 8): unproven demand, one-of-four
competitor support, and the largest build in Phase 2 for the only piece nothing
else depends on.

Nothing in this plan forecloses it. If it is revived, two things settled during
the cut are worth keeping: a **shared polymorphic digest table** beats a column
per `*_translations` table (per-field rather than per-row granularity, and every
future registry member is covered without another migration), and the flag stays
**advisory** — a digest is never required on write.
...
- Coverage grid powered by `GET /api/v3/admin/translations?resource_type=product` (one query).
...
**Phase 2 (6.0) — coverage + bulk + SPA centralized page:**
8. `GET /api/v3/admin/translations` coverage matrix; wire the shipped `Spree::Imports/Exports::ProductTranslations` into the dashboard's import-type registry — no JSON bulk endpoint, and no registry-wide CSV (Key Decision 13; a parameterized `ResourceTranslations` pair stays a follow-up).
9. Dashboard: per-resource translation tab (Surface 1) + centralized Translations page (Surface 2) consuming the above.

**Phase 3 (6.0) — fold in metafields:**
10. Per `5.4-metafield-translations.md`, register `MetafieldDefinition` (translatable `name`) and `Metafield` (translatable `value`, text types only) in `Spree.translatable_resources`. They become just-another-registered-resource — the `{locale:{field}}` write shape that plan already committed to is a subset of this contract, so no new endpoint is needed.
...
- **Do not build staleness tracking** without revisiting Key Decision 8. If it is revived it stays advisory — a digest is never required on write.

## Open Questions

1. **Store translations route** — Store is a singleton in the SPA's store-scoped context; does its translation editor live under store settings (`…/settings/general/translations`) rather than a nested resource route? Likely yes; confirm the route shape. Still open, and deliberately not forced by the centralized page: that grid lists only resource types the registry reports as `readable` (those carrying the `:translatable` route concern), so Store is absent from it until it gets a route of its own rather than appearing as a broken tab.
...
6. ~~**Generalized CSV schema**~~ — **resolved 2026-09-02** (Key Decisions 13 + 14): the format is long, and generalization across the registry is not built this run. When a generic importer is written, the remaining sub-question stands: confirm the join key per resource type (slug / permalink / name) is stable and unique per store. Products key on `slug` today.
