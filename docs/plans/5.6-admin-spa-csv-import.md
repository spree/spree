...
- **Completion email** → ~~out of scope~~ *revised 2026-07-11: in scope.* `Spree::ImportMailer#import_done` (subscriber on `import.completed`, so retry passes email again) mirrors `export_done`, with the failed-rows count called out. The link back to the wizard does **not** rely on legacy Rails route helpers: the dashboard passes a `results_url` at create (stored as an Import preference, validated against the store's allowed origins exactly like the password-reset `redirect_url` — silently dropped otherwise) and the mailer appends `?import=<id>`; the legacy `admin_import_url` helper remains only as a fallback when that engine is mounted.
