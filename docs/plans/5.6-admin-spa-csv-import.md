...
- CSV dropzone → `useDirectUpload`. **Force `content_type: 'text/csv'` in the presign for `.csv` files** — Windows browsers report `.csv` as `application/vnd.ms-excel` and the model validation rejects it. The picked file renders as an `Attachment` (name, size, uploading state, remove) — `Attachment` and `Progress` were ported into `dashboard-ui` from the shadcn **Base UI** registry for this feature (`cn-*` preset classes flattened to plain utilities, matching the package's pre-CSS-layer style); the wizard's processing bar is the `Progress` primitive with `value={null}` as the indeterminate "preparing" state. Migrating `ImageUploadField` to `Attachment` is a separate follow-up — it's a shipped shared component with its own controlled contract.
