...
**Status:** In Progress — open questions resolved and implemented in a single PR (2026-07-10)
**Target:** Spree 5.6
...
## Seller panel imports (2026-08-27)

A marketplace seller lists their catalog by CSV through the same pipeline, on
their own branch (`/api/v3/seller/imports`). The upload, the mapping step, the
row processors and the wizard are all the operator's — what differs is who
owns the result and how far a row can reach.

**Products only.** Customers belong to the marketplace and translations of
catalog copy are its merchandising, so the seller branch allowlists
`Spree::Imports::Products` and refuses the rest, matching the line the seller
products endpoint already draws around tags, categories and product types.

**Imports and exports now carry the same tenancy pair** (2026-08-27). An import
used to hold a polymorphic `owner` that was a Store *or* a Seller, never both,
so a seller's import had no store on it and the operator's store-scoped list
could not include it. An export already had `store_id` — plus a
`belongs_to :seller` whose column was never created, so filtering exports by
seller raised `PG::UndefinedColumn`.

Both tables now carry `store_id` (always present) and a nullable `seller_id`
(null = the operator's own), which fixes the export bug and gives operators
their sellers' imports for free: the base controller roots the admin index at
`model_class.for_store(current_store)`, and `for_store` now comes from
`SingleStoreResource`. `Import#owner` survives as a deprecated reader through
6.1, and the serializer emits `owner_type`/`owner_id` from the new pair so
existing clients see the same answer.

**Operators read seller rows; they do not act on them.** A retry re-runs writes
into the seller's own catalog and nothing on the import would record that the
operator caused them, so `retry_failed_rows` and `destroy` refuse a
seller-owned import with a 422 while every read spans the store.

**The import's ability is built against its owner.** `Import#current_ability`
resolved roles against the *store*, and a seller's staff hold theirs on the
`Spree::Seller` — so a seller's ability compiled zero rules and every
`accessible_by` through it came back empty. That is invisible on a first
import (nothing to find) and breaks the second one: the row cannot see the
product it already created, tries to create it again, and fails on slug
uniqueness. Variant rows, which only ever look the product up, failed
outright. Passing `resource: owner` fixes both.

**Two narrowings, both in the row processor.** They cannot live in a
controller: rows are processed in a background job, long after any request
scope is gone.

- *Ownership.* A product created by a seller's import is stamped with that
  seller. Without it the rows would belong to the store — invisible in the
  seller's own panel and unattributable at commission time. It has to happen
  before the variants save, because SKU uniqueness is scoped to the variant's
  resolved seller (which is what lets two sellers list the same part number).
- *Reach.* `product_scope` narrows to the seller's own products. The ability
  cannot answer this — it grants capability, never tenancy, so
  `write_products` reaches every product in the class. Without the narrowing a
  CSV naming another seller's slug would edit their listing.

**A seller's CSV cannot extend the marketplace's vocabulary.** The processor
creates records for values it does not recognise, which on a seller's import
would write into shared state: `product_type` (read-only for the seller
audience) is looked up but never created, `tags` are skipped entirely (they
are tenanted to the store, so a new one joins the marketplace's own
vocabulary), and `category1..3` are skipped (the `categories` resource has no
seller audience at all). All three match what the seller products endpoint
already refuses.

**Imported stock lands in the seller's own warehouse.** `set_stock` falls back
to `Variant#default_stock_location`, which answers the *store's* — so an
`inventory_count` column would deposit a seller's goods in the marketplace's
location. The processor passes the seller's default location instead, falling
back to the store's when they have none.

**The CSV's `status` column is ignored on a seller's import.** A seller
reaches `active` only through review, so an uploaded `status,active` would
otherwise publish to the marketplace catalog with nobody having looked at it —
the same hole `6.0-seller-product-submission.md` closed for the JSON endpoint.
Imported products land as `draft` and the seller submits each for review.

**`Spree::Import`/`Spree::ImportRow` joined the `products` catalog resource.**
An import is a bulk product write and nothing else, which is already why
`write_<resource>` is the key gating every import endpoint. Without the
subjects, a principal whose capability comes from catalog keys alone — every
seller — would pass the key gate on `write_products` and then be refused by
the `authorize!` immediately behind it. Operators are unaffected: the `admin`
role grants `can :manage, :all` and never consults the subject list.

**The wizard moved into `@spree/dashboard-core`** and reads its data through
the registered `PanelApiClient`, so both panels render one definition. Its one
route-bound piece — where "View records" goes — is an injected `onViewRecords`
prop, since the two panels file their catalogs under different route trees.

### Constraints

- **Never read the CSV `status` column on a seller's import**, and never add a
  path that lets an imported seller product reach `active` without
  `Products::Approve`.
- **A new row processor that resolves existing records must narrow by
  `import.seller`** when one is present. `accessible_by` is not a substitute —
  it answers capability, not tenancy.
- **A row processor that creates a record the CSV named must ask whether a
  seller may create it.** Product types, tags and categories are the
  marketplace's; a new find-or-create in this file needs the same check, or a
  CSV becomes a way around what the seller endpoint refuses.
- **A new import or export field goes on both models.** They are the same kind
  of record and drifted once already; a tenancy or attribution field added to
  one and not the other is how that happens again.
- **Never write `owner:` on an import.** It is a deprecated bridge kept for
  API clients — internal code assigns `store:` and `seller:`.
- **Test a seller import with a user who holds their role on the seller.** The
  `:product_import` factory defaults `user` to a store admin, whose blanket
  access hides every scoping bug in this file — which is exactly how the
  empty-ability bug above survived its first round of specs.
- **A new seller-reachable import type is added to the controller's
  allowlist deliberately**, never by widening it to `Spree::Import.available_types`.
- **New import UI goes into `@spree/dashboard-core`**, taking its data by prop
  or through the registered client — never `adminClient`, which a seller's
  panel has no credential for.
