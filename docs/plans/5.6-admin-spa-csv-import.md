...
- **Route-based wizard under the settings shell, not a modal.** Processing takes minutes; admins must be able to navigate away and come back. The flow lives at `/$storeId/settings/imports/$importId` (mapping → progress → results are states of one route, keyed off `import.status`), plus a `/settings/imports` history page registered in the settings nav under a new **`audit` group** (future home for exports history and audit logs; legacy groups imports with exports/audits too). Exports stayed toast-only because they end in a download; imports end in a report you review.
...
**`ImportButton` (`packages/dashboard-core/src/components/import-button.tsx`)** — sits beside `export-button.tsx`, same toolbar slot. Props: `{ type: ImportType; subject: SubjectName; onCreated: (imp: AdminImport) => void }` — dashboard-core must not know app routes, so navigation is the app's callback (products index passes `navigate({ to: '/$storeId/settings/imports/$importId', ... })`). The button wraps itself in `<Can I="create" a={subject}>` (per-context gating — see Key Decisions). The dialog contains:

- CSV dropzone → `useDirectUpload`. **Force `content_type: 'text/csv'` in the presign for `.csv` files** — Windows browsers report `.csv` as `application/vnd.ms-excel` and the model validation rejects it.
...
- `routes/_authenticated/$storeId/settings/imports/index.tsx` — history: `ResourceTable` over `client.imports.list` (number, type, status badge, `completed/failed/total` counts, user, created_at; row click → detail). Registered in `packages/dashboard/src/nav/settings.ts` under a **new `audit` settings-nav group** (`settingsNav.addGroup({ key: 'audit', ... })`, positioned after `team`), entry `{ key: 'settings.imports', path: '/imports', group: 'audit' }` — the group is the future home for exports history and audit logs.
- `routes/_authenticated/$storeId/settings/imports/$importId.tsx` — the wizard, one route switching on `status` per the poll contract:
...
- **Completion email** → out of scope (durable results page + history covers it; note the stale code comment in `import.rb` claiming an email exists).
