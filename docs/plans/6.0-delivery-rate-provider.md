...
`DeliveryRateProvider` backed by a multi-carrier aggregator** (Shippo or
EasyPost — see Open Questions), mirroring Stripe as the reference gateway.
...
4. **Aggregator pick for the reference provider.** Shippo vs EasyPost for
   the monorepo reference implementation (decisions.md 2026-07-29): EasyPost
   has the `spree_easypost` lineage; Shippo has the simpler API surface and
   pricing. Decide before Phase 2 implementation starts; the interface is
   aggregator-agnostic either way.

5. **Backfill heuristic for multi-store installs.** The zone-overlap signal in Phase 1 is imperfect where several stores share a zone. Is loud logging plus manual reassignment sufficient, or should the task refuse to guess and require an explicit mapping?
