...
**Phases 1–4 are implemented.** Phase 1 (`store_id`) shipped with the fulfillment rework; Phases 2–4 landed together:

- **Provider interface**: `DeliveryRateProvider::Base` (`estimate` required; `book`/`release` no-ops; `integration_class` + `available_for_store?` class hooks; `integration` resolved from the method's store) and `DeliveryRateProvider::Internal`, which delegates to the method's calculator so nothing changes by default.
- **`Estimate` value object** — `ActiveModel::Model` + `Attributes`, **not a `Struct`** as the design sketch below shows: CLAUDE.md forbids `Struct` for domain value objects. Fields as specified.
- **Registry**: `Spree.delivery_rate_providers` (engine-seeded, initializer-extensible) with inclusion validation on `DeliveryMethod#rate_provider` on change only, blank allowed — so rows survive a provider gem being uninstalled.
- **Model**: `rate_provider` string column, `DeliveryMethod::DEFAULT_RATE_PROVIDER`, `#rate_provider_instance` / `#rate_provider_class`.
- **Estimator**: the single dispatch point now calls `rate_provider_instance.estimate(package)`; all filtering, VAT gross-up, tax resolution and sorting are untouched.
- **Enriched `DeliveryRate`**: `carrier`, `service_level`, `estimated_delivery_date` and a single `metadata` JSON column (plain `serialize`, **not** the `Spree::Metadata` concern — that concern requires the legacy `public_metadata`/`private_metadata` pair, which the house rule forbids on new columns). Store serializer exposes carrier/service level/date; `metadata` is admin-only.
- **Admin surface**: `GET /admin/delivery_methods/rate_providers` (filtered by `available_for_store?`, returns the default), `rate_provider` permitted + serialized, `adminClient.deliveryMethods.rateProviders()`, and a dashboard provider select that renders only when more than one provider is available — so it stays hidden until a carrier gem is installed. i18n in all six locales.

**Acceptance criterion met:** the full core suite (7,463 examples) and API suite (2,661 examples) pass with zero failures and no spec modified to accommodate the change — the Internal provider's calculator delegation is behavior-identical.

**Two deviations from the design below:** (1) `Internal#estimate` does **not** compute `estimated_delivery_date`. The sketch calls `days.business_days.from_now`, but core has no business-day arithmetic (no `business_time` gem, and the transit columns are only ever rendered as a range string). Turning business days into a date needs a per-market holiday calendar; guessing one would show customers a wrong date — carrier providers return the real date. (2) `Estimate` has **no `estimated_transit_days`** field: no `spree_delivery_rates` column persists it, so a value there would be silently discarded — worse than not offering it. The method's own `estimated_transit_business_days_min`/`_max` remain readable from any rate via `delivery_method`; a transit-days field can return together with a column when a carrier gem needs it. Additionally, `available_for_store?` is derived from `integration_class` on the base class (declaring the integration is all a carrier provider needs), and `DeliveryMethod` validates provider availability on save — the admin picker filter is display-only over the same rule.
...
### Dynamic carrier rates: one method, many rates (2026-08-09)

A carrier-backed DeliveryMethod is the carrier connection, not one service: `DeliveryRateProvider::Base#estimates(package)` returns an array (single-quote providers keep implementing `estimate` and are wrapped), every returned service becomes its own named `DeliveryRate` (`name` column; carrier + service by default), and the customer picks between them. Merchant controls follow Shopify's knob set plus label overrides (which Shopify lacks): **`Spree::DeliveryMethodService`** rows narrow the offered services and carry per-service `label`/`markup_flat`/`markup_percent`; method-level markup columns are the default; no rows = every service, current and future. Selection preservation and EasyPost label purchase key off the selected rate's carrier/service. Competitive grounding: Shopify (service selection + handling fee, dynamic quotes), Medusa (materialized per-service options), Saleor (fully app-delegated) — the two-entity method→services shape is the normalized version of Shopify's/Woo's config blobs. Supersedes the one-rate-per-method constraint and realizes the "one Carrier shipping method per market" aspiration from the 2026-07-29 survey.

### Provider wraps the Estimator; it does not replace it

`Stock::Estimator` **stays**. It owns responsibilities no provider should re-implement per-seller:
...
### Method rows are regional; the monorepo ships one reference provider (2026-07-29)

Competitor survey (Shopify zones + CarrierService participants, Medusa
service-zone-scoped shipping options, Saleor zone-scoped methods with
per-channel listings, Vendure channel-scoped methods) confirms the industry
shape: **the sellable method is regional, only the carrier integration is
shared.** "UPS (Europe)" and "UPS (North America)" as separate
`DeliveryMethod` rows is the sanctioned pattern — no cross-market method
entity will be added. See decisions.md 2026-07-29.

The same decision commits the monorepo to **one reference
`DeliveryRateProvider` backed by a multi-carrier aggregator** — settled
as **EasyPost** (decisions.md 2026-08-06 reversal entry: BYOCA fit for
Spree's larger-merchant profile + the only actively maintained official
Ruby SDK) — mirroring Stripe as the reference gateway.
It is the first real consumer of this plan's interface and the default
answer for installs that want live carrier rates without per-carrier
method sprawl; the `Internal` calculator path stays for manual rates.

### Selected-rate preservation is not the provider's business

`Fulfillment#refresh_rates` remembers the previously selected rate and re-selects it if still offered: the same carrier service, then the same method, then the estimator's own pick. The proposal rebuilds follow the same rule (`Cart#rebuild_fulfillments!`, and `Order#rebuild_fulfillments!` on an open order): each new proposal takes the earlier choice from its stock location whose method it is quoted for, a rebuild after a destination change keeps nothing, and every proposal is priced at the rate it ends up with (2026-09-24, V-3696). This is checkout UX, not rate estimation, and it stays on the fulfillment. **No provider or checkout service may unconditionally `min_by(&:cost).update(selected: true)`** — that would silently reset a customer's or admin's chosen rate on every refresh.
...
**Interaction with `SellerConcern`.** `ShippingMethod` conditionally includes `Spree::SellerConcern` (`shipping_method.rb:10-12`) for the marketplace. Store ownership and seller ownership are different axes: the store is the tenant boundary (and the credential scope); the seller is the seller within it. A seller's shipping methods belong to the marketplace's store. Coordinate with `6.0-multi-vendor-marketplace.md` before enforcing `null: false`.

### Seller-scoped methods filter in the Estimator (marketplace)

`6.0-multi-vendor-marketplace.md` requires per-seller delivery selection at checkout: line items partition into per-seller packages **before** the order split, and "a `DeliveryMethod` with `seller_id` is offered only on that seller's packages; `seller_id: nil` methods are marketplace-wide" — and it delegates that filtering to this plan.

Because the provider wraps the Estimator, this lands in exactly one place: `Estimator#shipping_methods(package, filter)` gains a seller gate — skip methods whose `seller_id` is present and differs from the package's seller; `nil` means marketplace-wide. The `seller_id` column itself is added by the marketplace plan's schema wave (it replaces today's Enterprise `SellerConcern` decorator seam at `shipping_method.rb:10-12`); this plan owns the filter. Per-package granularity is native here — the Estimator already runs per package, which is exactly the granularity per-seller selection needs. Single-seller stores are unaffected (`seller_id` is nil everywhere, the gate is a no-op).
...
- **One integration per (store, type)** — `validates :store, presence: true, uniqueness: { scope: :type }` (`integration.rb:16`). A store cannot hold two EasyPost credential sets. Multi-account support is a separate change; see the grooming note in `6.0-payment-method-rules.md`.
...
> **The snippets below are the original design sketch, kept for the reasoning
> that produced it.** Where they disagree with what shipped, the
> "Implementation status" section above is authoritative: the value object is
> `Spree::DeliveryRateProvider::Estimate` (not `DeliveryRateEstimate`), it
> carries no `estimated_transit_days`, and `DeliveryRate` uses a plain
> `metadata` column rather than the `Spree::Metadata` concern. Provider
> authors should follow
> `docs/v6/developer/how-to/custom-delivery-rate-provider.mdx`.
...
module Spree
  module DeliveryRateProvider
    ...
    # Shipped as Spree::DeliveryRateProvider::Estimate — an ActiveModel value
    # object, not the Struct this sketch originally showed (CLAUDE.md: never
    # use Struct for domain value objects).
    class Estimate
...
**Superseded (2026-08-09) — the index gate was exercised, not bypassed.** The unique index on `(fulfillment_id, delivery_method_id)` was dropped by a fresh decision (decisions.md 2026-08-09): a carrier method now yields **one rate per service** via `estimates(package)`, and a `DeliveryMethodService` row model narrows/renames/marks-up individual services. The original constraint text is kept below for history; the shared-API-call caching remains just as valuable (one call now feeds N rates).

> Historical: `spree_shipping_rates` had a *unique* index on `["shipment_id", "shipping_method_id"]`. One row per (shipment, method) was why each DeliveryMethod yielded at most one rate; a provider wanting Ground *and* Express had to be two DeliveryMethods.
...
### Phase 1 — store ownership (independent, ship first) ✅ shipped
...
- `ShippingMethod` includes `SingleStoreResource`, declares `belongs_to :store`.
...
- `null: false` is deferred to the 6.0 cleanup, after the seller interaction is settled.

### Phase 2 — provider interface + Internal provider ✅ shipped
...
### Phase 3 — enrich ShippingRate ✅ shipped
...
### Phase 4 — admin surface (React dashboard only — no Rails admin in 6.0) ✅ shipped
...
### Phase 5 — reference external provider ✅ shipped (initial scaffold)

- **Shipped 2026-08-06:** `spree/providers/easypost` (gem `spree_easypost`, module `SpreeEasyPost` — one Zeitwerk inflection) with `SpreeEasyPost::Integration` (password-preference API key, `can_connect?` via an address create — the one authenticated call EasyPost allows in both test and production modes; account-management endpoints reject test keys outright) and `SpreeEasyPost::DeliveryRateProvider` on the official `easypost` gem: per-method carrier/service mapping via method metadata, shared shipment call through `Spree::Current.provider_cache` (several methods = one API round-trip, covered by spec), weight converted to ounces from the store unit system, enriched estimates (carrier/service/date/rate ids). Specs stub the client — no network. The how-to guide shipped with Phase 2. Since completed: `SpreeEasyPost::FulfillmentProvider` (label purchase on ship with a stale-quote re-quote that buys only the exact sold carrier/service, refund on cancel, label via `documents` — never raises into the transition) and CI wiring; VCR-backed API-contract specs mirror spree_stripe. The gem stays out of the `spree` meta-gem (optional install).
...
- **Don't delete or deprecate `ShippingCalculator` or `Calculator::Shipping::*`.** They are load-bearing and staying. `CalculatedAdjustments` is shared with TaxRate and promotion actions; its fate belongs to `6.0-tax-provider.md` and `6.0-6.1-split-adjustments.md`, not here.
