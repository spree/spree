...
- **Shipped 2026-08-06:** `spree/easypost` (gem `spree_easypost`, module `SpreeEasyPost` — one Zeitwerk inflection) with `SpreeEasyPost::Integration` (password-preference API key, `can_connect?` via a cheap authenticated read) and `SpreeEasyPost::DeliveryRateProvider` on the official `easypost` gem: per-method carrier/service mapping via method metadata, shared shipment call through `Spree::Current.provider_cache` (several methods = one API round-trip, covered by spec), weight converted to ounces from the store unit system, enriched estimates (carrier/service/date/rate ids). Specs stub the client — no network. The how-to guide shipped with Phase 2. Not yet included: a `FulfillmentProvider` for label purchase/tracking (fulfillment stays manual), CI wiring for the new gem, and the gem is not part of the `spree` meta-gem (optional install).
