...
Each rename leaves the old name readable — a reverse `alias_attribute` (or a
constant alias for the model rename) that warns and is removed in 6.1, matching
what `Spree::Order` already does for `promo_total` / `item_count` and what
`Spree::StockItem = StockLevel` does for the class rename.
