...
- **Event naming after the split (settled 2026-07-29):** carts publish their own `cart.created` / `cart.updated` / `cart.deleted` lifecycle events (restoring the 5.x abandonment signal that incomplete orders' `order.*` events used to provide; Shopify `carts/*` parity). `order.completed` stays the placement event — `order.created` cannot take that role because it fires for every order row, including admin drafts and the pre-payment draft copy inside `Carts::Complete` (a failed payment then emits `order.deleted` for it). Renaming would also break every 5.x webhook consumer and the emails/metrics subscribers for no semantic gain.
