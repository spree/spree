...
1. **Extension migration window** — does `checkout_flow`/`insert_checkout_step` get a hard removal in 6.0 or a one-release shim that translates registrations into `Checkout::Registry` entries? (Enterprise checkout customizations are the main consumers.)
2. **`partially_canceled`** — today a quasi-state in the `canceled` scopes; proposed as a derived predicate. Confirm no report/export depends on it as a stored value.
3. **Reaper defaults** — 30/90-day tiers are OSS platform B's; confirm against Spree hosting expectations before hardcoding defaults.
