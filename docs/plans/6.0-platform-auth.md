...
**Status:** In Progress (RefreshToken shipped in 5.4 ✓; Phases 1–3 implemented on `feature/v-3514` — gem-owned `Spree::Customer`/`Spree::AdminUser` models + `Spree.customer_class` rename + `user_id`→`customer_id`; Phase 2 data migration implemented — `spree:upgrade:migrate_users_to_customers` (Path A) + custom-model recipe (Path B); Phases 4–6 pending)
...
### Phase 2: Data migration (for existing Devise-based installs) — implemented on `feature/v-3514`
