...
- New params types: `StockLocationCreateParams`, `StockLocationUpdateParams`. Both accept the address fields with two ergonomic shortcuts mirroring `Spree::Address`: `country_iso` (ISO-2 code, e.g. `'US'`) resolves to the right `Spree::Country` and `state_abbr` resolves to the matching state. Raw `country_id` / `state_id` are still accepted for parity with existing tooling.
