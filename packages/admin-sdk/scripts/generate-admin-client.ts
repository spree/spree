...
function classifyAction(
  ...
  path: string,
  basePath: string,
): { action: ResourceEndpoint['action']; customAction?: string } {
  ...
  for (const [fullPath, methods] of Object.entries(spec.paths)) {
    ...
    for (const [method, operation] of Object.entries(methods)) {
      ...
      if (segments.length >= 5 && segments[1].startsWith('{') && segments[3].startsWith('{')) {
        ...
        const hasDeepMember = Object.keys(spec.paths).some((p) =>
          p.startsWith(`${potentialDeepBasePath}/{`),
        )
...
function generateResourceMethods(resource: Resource, paramTypes: Map<string, string>): string {
  ...
  if (resource.grandparentParam) {
    parentParams.push({
      name: resource.grandparentParam,
      camel: camelCase(resource.grandparentParam),
    })
  }
  ...
  for (const endpoint of resource.endpoints) {
    ...
    switch (action) {
      case 'list': {
        ...
        lines.push(
          `      this.request<PaginatedResponse<${responseType}>>('GET', ${collectionPath()}, {`,
        )
        lines.push(`        ...options,`)
      ...
      case 'create': {
        ...
        const paramType = paramTypes.has(createTypeName)
          ? createTypeName
      ...
      case 'update': {
        ...
        const paramType = paramTypes.has(updateTypeName)
          ? updateTypeName
          : 'Record<string, unknown>'
        if (jsdoc) lines.push(jsdoc)
        lines.push(`    update: (`)
      ...
      case 'custom': {
        ...
        if (method === 'POST' || method === 'PATCH') {
          ...
          lines.push(`    ): Promise<${responseType}> =>`)
          lines.push(
            `      this.request<${responseType}>('${method}', ${memberPath(`/${customAction}`)}, {`,
          )
          lines.push(`        ...options,`)
          lines.push(`        body: params,`)
          lines.push(`      }),`)
...
function generateClient(spec: OpenApiSpec): { client: string; params: string } {
  ...
  out.push(`// Do not edit manually. Run: npm run generate:admin-client`)
  out.push(``)
  out.push(`import type { RequestFn, RequestOptions } from '@spree/sdk-core';`)
  out.push(`import { transformListParams, getParams } from '@spree/sdk-core';`)
  out.push(`import type { PaginatedResponse, ListParams } from '@spree/sdk-core';`)
  out.push(``)
  out.push(`export interface AuthTokens {`)
  out.push(`  token: string;`)
  out.push(`  refresh_token?: string;`)
  out.push(`  user: {`)
  out.push(`    id: string;`)
  out.push(`    email: string;`)
  out.push(`    first_name: string | null;`)
  out.push(`    last_name: string | null;`)
  out.push(`  };`)
  out.push(`}`)
  out.push(``)
  out.push(`export interface LoginCredentials {`)
  out.push(`  email: string;`)
  out.push(`  password: string;`)
  out.push(`}`)
  ...
  out.push(`  /** @internal */`)
  out.push(`  readonly request: RequestFn;`)
  out.push(``)
  ...
  out.push(`    refresh: (options?: RequestOptions): Promise<AuthTokens> =>`)
  out.push(`      this.request<AuthTokens>('POST', '/auth/refresh', options),`)
  out.push(`  };`)
  ...
  for (const resource of topLevel) {
    ...
    for (const nestedResource of nestedResources) {
      ...
      for (const deepResource of deepChildren) {
        ...
        out.push(
          `      /** Nested: ${resource.name}/{id}/${nestedResource.name}/{id}/${deepResource.name} */`,
        )
  ...
  const orphanNested = nested.filter((n) => !topLevel.some((t) => t.name === n.parentName))
...
const PARAMS_PATH = path.resolve(import.meta.dirname, '../src/params.ts')
...
for (const r of resources) {
  ...
  console.log(
    `${prefix}${r.name} (${r.endpoints.map((e) => `${e.method} ${e.action}`).join(', ')})`,
  )
}
