...
describe('credit card actions', () => {
  ...
  describe('listCreditCards', () => {
    it('returns credit cards list', async () => {
      ...
      expect(mockClient.customer.creditCards.list).toHaveBeenCalledWith(
        undefined,
  ...
  describe('deleteCreditCard', () => {
    it('deletes card and revalidates', async () => {
      mockClient.customer.creditCards.delete.mockResolvedValue(undefined);

      await deleteCreditCard('1');
      expect(mockClient.customer.creditCards.delete).toHaveBeenCalledWith(
        '1',
        expect.objectContaining({ token: expect.any(String) })
      );
