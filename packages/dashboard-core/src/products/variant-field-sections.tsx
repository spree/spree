...
// Weight and dimensions carry no inline errors on the operator's sheet, so
// this section takes the shared props but reads no `errors`.
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field>
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select value={field.value ?? ''} onValueChange={(v) => field.onChange(v || null)}>
                <SelectTrigger id={fieldId(prefix, 'purchase-unit')} className="w-full">
                  <SelectValue>
                    {(v) =>
                      v
      ...
      <Field>
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-col">
            <FieldLabel htmlFor={fieldId(prefix, 'preorderable')} className="cursor-pointer">
