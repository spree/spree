...
import { resolveNavDescription, resolveNavLabel } from '../lib/nav-registry'
import {
...
/**
 * Settings landing page — a card grid of every settings area the current
 * member can reach, grouped the same way the sidebar groups them.
 *
 * Shared by the operator dashboard and the marketplace seller panel. Each
 * host passes its tenant id so the cards link under that panel's prefix
 * (`/$storeId/settings/…` or `/$sellerId/settings/…`).
 *
 ...
 */
export function SettingsIndexPage({ tenantId }: { tenantId: string }) {
        ...
        groups.map(({ group, entries }) => (
          <section key={group.key} className="flex flex-col gap-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {entries.map((entry) => (
                <SettingsCard key={entry.key} entry={entry} tenantId={tenantId} />
              ))}
            </div>
          </section>
        ))
...
function SettingsCard({ entry, tenantId }: { entry: SettingsNavEntry; tenantId: string }) {
  const { t } = useTranslation()
  ...
  return (
    <Card className="p-0 transition-colors hover:bg-accent/80 hover:border-border">
      ...
      <Link
        to={`/${tenantId}/settings${entry.path}` as never}
