...
describe('paymentSetupSessions', () => {
  ...
  describe('get', () => {
    it('returns a payment setup session by ID', async () => {
      const result = await client.customer.paymentSetupSessions.get('pss_1', opts)
      expect(result.id).toBe('pss_1')
      expect(result.status).toBe('pending')
  ...
  describe('complete', () => {
    it('completes a payment setup session', async () => {
      const result = await client.customer.paymentSetupSessions.complete('pss_1', undefined, opts)
      expect(result.id).toBe('pss_1')
