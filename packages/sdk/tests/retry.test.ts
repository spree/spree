...
describe('Retry logic', () => {
  ...
  describe('POST requests (auto-idempotency)', () => {
    ...
    it('does not send Idempotency-Key on GET', async () => {
      const mockFetch = vi.fn().mockResolvedValueOnce(jsonResponse({ data: [], meta: {} }))
    ...
    it('does NOT retry POST when retries are disabled', async () => {
      ...
      await expect(client.auth.login({ email: 'a@b.com', password: 'pass' })).rejects.toThrow(
        SpreeError,
      )
      expect(mockFetch).toHaveBeenCalledTimes(1)
    })
    ...
    it('respects Retry-After header', async () => {
      ...
      const mockFetch = vi
        ...
        .mockResolvedValueOnce(
          errorResponse(429, 'rate_limited', 'Too many requests', { 'Retry-After': '1' }),
        )
        .mockResolvedValueOnce(jsonResponse(paginatedEmpty))
