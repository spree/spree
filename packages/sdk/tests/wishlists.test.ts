...
describe('wishlists', () => {
  ...
  it('creates a wishlist', async () => {
    const result = await client.wishlists.create({ name: 'My Wishlist' }, opts)
    expect(result.name).toBe('My Wishlist')
  })

  it('updates a wishlist', async () => {
    const result = await client.wishlists.update('wl_1', { name: 'Updated Wishlist' }, opts)
    expect(result.name).toBe('Updated Wishlist')
  })

  it('deletes a wishlist', async () => {
    await expect(client.wishlists.delete('wl_1', opts)).resolves.toBeUndefined()
  })
