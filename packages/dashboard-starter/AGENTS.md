...
This is the Spree admin dashboard for this project — a Vite + React app. The
shell (routes, chrome, resource pages) ships in `@spree/dashboard`; this
project holds your customizations.

Full docs: https://spreecommerce.org/docs/developer/dashboard/overview

## Imports

Import from `@spree/dashboard` — it re-exports the framework and the design
...
Organized by kind, one file per resource — the same layout `@spree/dashboard`
uses internally.

| Directory | Holds | Naming |
|---|---|---|
| `src/plugins.ts` | Registrations only: nav, routes, slots, tables | one file |
...
| `src/slots/` | Widgets injected into built-in pages | `product-brand-card.tsx` |
...
- **The Admin API is the only data source.** Never reach into the Rails app or
  import server-rendered HTML. A missing endpoint is added to the API first.
