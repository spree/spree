...
import { type CoverageStockItem, computeStockCoverage } from './use-stock-coverage'

function stockItem(overrides: Partial<CoverageStockItem>): CoverageStockItem {
  return {
...
describe('computeStockCoverage', () => {
  it('covers a location holding enough of every demanded variant', () => {
    const coverage = computeStockCoverage(
      [
        stockItem({ variant_id: 'variant_1', available_count: 5 }),
        stockItem({ variant_id: 'variant_2', available_count: 2 }),
      ],
      [
  ...
  it('does not cover a location short on one variant', () => {
    const coverage = computeStockCoverage(
      [
        stockItem({ variant_id: 'variant_1', available_count: 5 }),
        stockItem({ variant_id: 'variant_2', available_count: 1 }),
      ],
      [
  ...
  it('treats a backorderable row as unlimited', () => {
    const coverage = computeStockCoverage(
      [stockItem({ available_count: 0, backorderable: true })],
  ...
  it('does not cover a location missing a variant entirely', () => {
    const coverage = computeStockCoverage(
      [stockItem({ stock_location_id: 'sloc_1', variant_id: 'variant_1', available_count: 5 })],
      [
        { variantId: 'variant_1', quantity: 1 },
  ...
  it('keeps locations independent of one another', () => {
    const coverage = computeStockCoverage(
      [
        stockItem({ stock_location_id: 'sloc_1', available_count: 10 }),
        stockItem({ stock_location_id: 'sloc_2', available_count: 0 }),
      ],
