...
export function useUpdateProfile() {
  ...
  return useResourceMutation<MeResponse, Error, MeUpdateParams>({
    ...
    // Field-level 422s never toast — the dialog shows them inline — so this
    // covers everything else: a refusal, a server error, a lost connection.
    errorMessage: t('admin.errors.failed_to_update_profile'),
