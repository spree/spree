...
const BOOTSTRAP_RUBY = [
  ...
  // leave, and held by one checkout at the destination. The hold is written
  // the way Reserve writes it — a reservation row plus the level's counter —
  // because a real checkout needs reservations switched on store-wide, which
  // the order specs must not inherit.
  ...
  `unless inventory_level.stock_reservations.exists?; inventory_cart = Spree::Cart.create!(store: s, currency: s.default_currency, email: 'e2e-inventory@example.com'); inventory_line = inventory_cart.line_items.create!(variant: inventory_product.default_variant, quantity: ${FIXTURE_INVENTORY_RESERVED}); inventory_level.stock_reservations.create!(cart: inventory_cart, line_item: inventory_line, quantity: ${FIXTURE_INVENTORY_RESERVED}, expires_at: 10.years.from_now); inventory_level.adjust_reserved_count(${FIXTURE_INVENTORY_RESERVED}); end`,
