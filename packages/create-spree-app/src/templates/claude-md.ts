...
export function rootClaudeMdContent(hasStorefront: boolean): string {
  ...
  lines.push(
    ...
    '',
    'First-run setup mints a read-only secret key into `.spree/credentials.json`',
    '(gitignored), so the Admin API client works without further configuration.',
    'Use it to inspect live data instead of guessing at the schema:',
    '',
    '```bash',
