import type { AdminAddress, AdminOrder } from '@spree/sdk'
...
function OrderHeader({ order }: { order: AdminOrder }) {
  ...
  const cancelMutation = useOrderMutation(orderId, () =>
    spreeClient.admin.orders.cancel(orderId, {}, { token: token! }),
  )
  const resendMutation = useOrderMutation(orderId, () =>
    spreeClient.admin.orders.resendConfirmation(orderId, {}, { token: token! }),
  )
...
function AddLineItemDialog({
  ...
  const mutation = useOrderMutation(orderId, (params: { variant_id: string; quantity: number }) =>
    spreeClient.admin.orders.lineItems.create(orderId, params, { token: token! }),
  )
...
function EditQuantityDialog({
  ...
  const mutation = useOrderMutation(orderId, (params: { quantity: number }) =>
    spreeClient.admin.orders.lineItems.update(orderId, lineItemId, params, { token: token! }),
  )
...
function LineItemsCard({ order }: { order: AdminOrder }) {
  const { orderId } = Route.useParams()
  const { token } = useAuth()
  ...
  const deleteMutation = useOrderMutation(orderId, (lineItemId: string) =>
    spreeClient.admin.orders.lineItems.delete(orderId, lineItemId, { token: token! }),
  )
...
function EditTrackingDialog({
  ...
  const mutation = useOrderMutation(orderId, (params: { tracking: string }) =>
    spreeClient.admin.orders.shipments.update(orderId, shipmentId, params, { token: token! }),
  )
...
function ShipmentsCard({ order }: { order: AdminOrder }) {
  const { orderId } = Route.useParams()
  const { token } = useAuth()
  ...
  const shipMutation = useOrderMutation(orderId, (shipmentId: string) =>
    spreeClient.admin.orders.shipments.ship(orderId, shipmentId, {}, { token: token! }),
  )
  const cancelShipmentMutation = useOrderMutation(orderId, (shipmentId: string) =>
    spreeClient.admin.orders.shipments.cancel(orderId, shipmentId, {}, { token: token! }),
  )
...
function PaymentsCard({ order }: { order: AdminOrder }) {
  const { orderId } = Route.useParams()
  const { token } = useAuth()
  const payments = order.payments ?? []

  const captureMutation = useOrderMutation(orderId, (paymentId: string) =>
    spreeClient.admin.orders.payments.capture(orderId, paymentId, {}, { token: token! }),
  )
  const voidMutation = useOrderMutation(orderId, (paymentId: string) =>
    spreeClient.admin.orders.payments.void(orderId, paymentId, {}, { token: token! }),
  )
...
function AddAdjustmentDialog({
  ...
  const mutation = useOrderMutation(orderId, (params: { label: string; amount: number }) =>
    spreeClient.admin.orders.adjustments.create(orderId, params, { token: token! }),
  )
...
function AdjustmentsCard({ order }: { order: AdminOrder }) {
  const { orderId } = Route.useParams()
  const { token } = useAuth()
  ...
  const deleteMutation = useOrderMutation(orderId, (adjustmentId: string) =>
    spreeClient.admin.orders.adjustments.delete(orderId, adjustmentId, { token: token! }),
  )
...
function AddressBlock({
  ...
  title: string
  address: AdminAddress | null | undefined
}) {
  return (
    <div>
...
function EditAddressDialog({
  ...
  const mutation = useOrderMutation(orderId, (params: Record<string, unknown>) =>
    spreeClient.admin.orders.update(orderId, { [type]: params } as any, { token: token! }),
  )
...
function SpecialInstructionsCard({ order }: { order: AdminOrder }) {
  const { orderId } = Route.useParams()
  const { token } = useAuth()
  const [editing, setEditing] = useState(false)
  const mutation = useOrderMutation(orderId, (params: { special_instructions: string }) =>
    spreeClient.admin.orders.update(orderId, params, { token: token! }),
  )
...
function InternalNoteCard({ order }: { order: AdminOrder }) {
  const { orderId } = Route.useParams()
  const { token } = useAuth()
  const [editing, setEditing] = useState(false)
