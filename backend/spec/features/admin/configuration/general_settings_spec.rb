...
describe 'General Settings', type: :feature, js: true do
  ...
  context 'editing general settings (admin)' do
    it 'should be able to update the site name' do
      ...
      fill_in 'store_mail_from_address', with: 'spree-test@example.org'
      click_button 'Update'

      assert_successful_update_message(:general_settings)
      expect(find('#store_name').value).to eql('Spree Demo Site99')
      expect(find('#store_mail_from_address').value).to eql('spree-test@example.org')
    end
  end
