require 'spec_helper'

describe Spree::Admin::PromotionsController, type: :controller do
  stub_authorization!

  let!(:promotion_category) { create(:promotion_category) }

  let!(:promotion1) do
    create(
      :promotion,
      promotion_category: promotion_category,
      code:               'a',
      path:               '/a'
    )
  end

  let!(:promotion2) do
    create(
      :promotion,
      promotion_category: promotion_category,
      code:               'b',
      path:               '/b'
    )
  end

  context '#index' do
    it 'succeeds' do
      spree_get(:index)
      expect(assigns(:promotions).to_a).to match_array([promotion1, promotion2])
    end

    it 'assigns promotion categories' do
      spree_get(:index)
      expect(assigns(:promotion_categories).to_a).to eql([promotion_category])
    end

    context 'search' do
      it 'pages results' do
        spree_get(:index, per_page: '1')
        expect(assigns(:promotions).to_a).to eql([promotion2])
      end

      it 'filters by name' do
        spree_get(:index, q: { name_cont: promotion1.name })
        expect(assigns(:promotions).to_a).to eql([promotion1])
      end

      it 'filters by code' do
        spree_get(:index, q: { code_cont: promotion1.code })
        expect(assigns(:promotions).to_a).to eql([promotion1])
      end

      it 'filters by path' do
        spree_get(:index, q: { path_cont: promotion1.path })
        expect(assigns(:promotions).to_a).to eql([promotion1])
      end
    end
  end
end
