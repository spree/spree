module Spree
  class JwtToken
    ...
    def self.create_for_user(user)
      time = (Time.current + Spree::Api::Config[:graphql_expiration])
      payload = { spree_user_id: user.id }
      payload[:exp] = time.to_i
      token = JWT.encode(payload, Spree::Api::Config[:graphql_secret_key])
    ...
    def self.decode(token)
      decoded = ::JWT.decode(token, Spree::Api::Config[:graphql_secret_key])[0]
