...
describe "Serializer Cache", :type => :request, :caching => true do
  ...
  it "doesn't create a cache key collision for models with different serializers" do
    ...
    expect(response.status).to eq(200)
    variant_b = JSON.parse(response.body)['product']['variants'].last
