  ...
  describe 'variants#index' do
    context 'with no params' do
      ...
      it 'returns all variants' do
        expect(json_response['data'].count).to eq (store.variants.count - 1)
    ...
    context 'current store' do
      ...
      it 'returns variants from this store only' do
        ...
        expect(variant_ids).to match_array(store.variants.eligible.ids.map(&:to_s))
      end
    end
      ...
      context 'by updated_at' do
        context 'ascending order' do
          ...
          it 'returns variants sorted by updated_at' do
            expect(json_response['data'].count).to      eq (store.variants.count - 1)
            expect(json_response['data'].pluck(:id)).to eq store.variants.eligible.order(:updated_at).map(&:id).map(&:to_s)
          end
        end
        ...
        context 'descending order' do
          ...
          it 'returns variants sorted by updated_at with descending order' do
            expect(json_response['data'].count).to      eq (store.variants.count - 1)
            expect(json_response['data'].pluck(:id)).to eq store.variants.eligible.order(updated_at: :desc).map(&:id).map(&:to_s)
          end
        end
      end
      ...
      context 'by created_at' do
        context 'ascending order' do
          ...
          it 'returns variants sorted by created_at' do
            expect(json_response['data'].count).to      eq (store.variants.count - 1)
            expect(json_response['data'].pluck(:id)).to eq store.variants.eligible.order(:created_at).map(&:id).map(&:to_s)
          end
        end
        ...
        context 'descending order' do
          ...
          it 'returns variants sorted by created_at with descending order' do
            expect(json_response['data'].count).to      eq (store.variants.count - 1)
            expect(json_response['data'].pluck(:id)).to eq store.variants.eligible.order(created_at: :desc).map(&:id).map(&:to_s)
          end
        end
      end
    end
      ...
      context 'with specified pagination params' do
        context 'when per_page is between 1 and default value' do
          ...
          it 'returns proper meta data' do
            ...
            expect(json_response['meta']['total_count']).to eq (store.variants.count - 1)
          end
