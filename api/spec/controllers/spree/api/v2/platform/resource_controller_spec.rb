...
describe Spree::Api::V2::Platform::ResourceController, type: :controller do
  ...
  describe '#resource_serializer' do
    ...
    context 'when controller model class is nested more than 2 levels' do
      before do
        allow(dummy_controller).to receive(:model_class).and_return(Spree::Webhooks::Endpoint)
      end

      it { expect(subject).to eq(Spree::Api::V2::Platform::Webhooks::EndpointSerializer) }
    end
