    ...
    private
      ...
      def before_payment
        if @order.checkout_steps.include? "delivery"
          ...
          @order.reload
          # @order.reload sets @order.state back to "confirm,"
          # which we don't want. Set the state back to payment
          # so that the user can edit their payment after getting
          # to the confirm page (or on a failed payment attempt)
          @order.state = "payment"
        end
