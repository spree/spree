  ...
  describe 'change store' do
    ...
    context 'when show_store_selector preference is set to true' do
      ...
      context 'when there are more than one supported currencies' do
        ...
        let(:first_link_name) { "#{Spree::Store.locale_language_name('en', first_store.default_locale)} (#{first_store_currency_symbol})" }
        ...
        let(:second_link_name) { "#{Spree::Store.locale_language_name('en', second_store.default_locale)} (#{first_store_currency_symbol})" }
